/**
 * Next.js Desktop Integration Manager Component
...
export const NextjsDesktopManager: React.FC<NextjsIntegrationProps> = ({
  atomIngestionPipeline,
  onIngestionComplete,
  ...
  const requestNotificationPermission = async () => {
    if (isTauri()) {
      try {
        const permissionGranted = await window.__TAURI__!.notification.requestPermission();
  ...
  const sendNotification = async (title: string, body: string) => {
    if (isTauri() && config.enableNotifications && notificationsEnabled) {
      try {
  ...
  // Start Next.js OAuth flow (Desktop)
    ...
    try {
      ...
      if (data.ok) {
        ...
        const checkAuth = setInterval(async () => {
          try {
            const statusResponse = await fetch('/api/auth/nextjs/status');
    ...
    try {
      ...
      const ingestionResult = await atomIngestionPipeline.startIngestion({
        ...
        callback: (status: IngestionStatus) => {
          ...
          // Send progress notifications for desktop
          if (status.progress % 25 === 0) {
            sendNotification(
              'Next.js Ingestion Progress',
      ...
      if (ingestionResult.success) {
        ...
        await sendNotification(
          'Next.js Ingestion Complete',
          `${ingestionResult.projectsProcessed} projects processed`
        );
  ...
  }, [isTauri, config.backgroundSync, health]);
    ...
    <Card>
      <CardHeader>
        <HStack justify="space-between">
          <HStack>
            ...
            {isTauri() && <Icon as={DesktopIcon} w={4} h={4} color="green.500" />}
          </HStack>
          <HStack>
            <Badge
      ...
      <CardBody>
        <VStack spacing={6} align="stretch">
          ...
          {isTauri() && (
            <Alert status={notificationsEnabled ? 'success' : 'info'}>
              <BellIcon />
              <Box>
                <Text fontWeight="bold">Desktop Features Active</Text>
                <Text fontSize="sm">
                  {notificationsEnabled ? 'Desktop notifications enabled' : 'Enable notifications for updates'}
                </Text>
              </Box>
            </Alert>
          )}
              ...
              <VStack align="start">
                {isTauri() && (
                  <>
                    ...
                    <Checkbox
                      isChecked={config.backgroundSync}
            ...
            <Button
              ...
              onClick={() => {
                if (selectedProject?.deployment?.url) {
                  openExternal(selectedProject.deployment.url);
                }
              }}
            >
              View Live Site
