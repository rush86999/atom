...
interface GitLabSearchProps {
  ...
  onSearch: (
    results: any[],
    filters: GitLabSearchFilters,
    sort: GitLabSearchSort,
  ) => void;
...
const GitLabSearch: React.FC<GitLabSearchProps> = ({
  ...
  const performSearch = useCallback(() => {
    ...
    if (searchTerm.trim()) {
      results = results.filter((item) => {
        ...
        if ("id" in item) {
          return (
            (item as any).iid?.toString().includes(searchText) ||
  ...
  const filterProjects = (
    ...
    return projects.filter((project) => {
      ...
      // Star range filter
      if (
        project.star_count < filters.starRange.min ||
        project.star_count > filters.starRange.max
      ) {
        return false;
      }
      ...
      // Fork range filter
      if (
        project.forks_count < filters.forkRange.min ||
        project.forks_count > filters.forkRange.max
      ) {
        return false;
      }
      ...
      // Issue range filter
      if (
        project.open_issues_count < filters.issueRange.min ||
        project.open_issues_count > filters.issueRange.max
      ) {
        return false;
      }
  ...
  const filterPipelines = (
    ...
    return pipelines.filter((pipeline) => {
      ...
      if (
        ...
        new Date(pipeline.created_at) < new Date(filters.dateRange.from)
      ) {
        return false;
      }
      if (
        filters.dateRange.to &&
        new Date(pipeline.created_at) > new Date(filters.dateRange.to)
      ) {
        return false;
      }
  ...
  const filterIssues = (
    ...
    return issues.filter((issue) => {
      ...
      if (
        filters.milestone &&
        (!issue.milestone || issue.milestone.title !== filters.milestone)
      ) {
        return false;
      }
      ...
      if (
        filters.dateRange.from &&
        new Date(issue.created_at) < new Date(filters.dateRange.from)
      ) {
        return false;
      }
      if (
        filters.dateRange.to &&
        new Date(issue.created_at) > new Date(filters.dateRange.to)
      ) {
        return false;
      }
  ...
  const filterMergeRequests = (
    ...
    return mergeRequests.filter((mr) => {
      ...
      if (filters.assignee && mr.assignees.length > 0) {
        const hasAssignee = mr.assignees.some(
          (a) => a.username === filters.assignee,
        );
        if (!hasAssignee) {
          return false;
      ...
      if (
        filters.milestone &&
        (!mr.milestone || mr.milestone.title !== filters.milestone)
      ) {
        return false;
      }
      ...
      if (
        filters.dateRange.from &&
        new Date(mr.created_at) < new Date(filters.dateRange.from)
      ) {
        return false;
      }
      if (
        filters.dateRange.to &&
        new Date(mr.created_at) > new Date(filters.dateRange.to)
      ) {
        return false;
      }
  ...
  const updateFilters = useCallback(
    (newFilters: Partial<GitLabSearchFilters>) => {
                  ...
                  <AccordionPanel>
                    <SimpleGrid columns={{ base: 1, md: 2, lg: 3 }} spacing={4}>
                      ...
                      <FormControl>
                        ...
                        <Stack>
                          {["public", "private", "internal"].map(
                            (visibility) => (
                              <Checkbox
                                key={visibility}
                                isChecked={filters.visibility.includes(
                                  visibility,
                                )}
                                onChange={(e) => {
                      ...
                      <FormControl>
                        <FormLabel>
                          Stars: {filters.starRange.min} -{" "}
                          {filters.starRange.max}
                        </FormLabel>
                        <Slider
                      ...
                      <FormControl>
                        <FormLabel>
                          Forks: {filters.forkRange.min} -{" "}
                          {filters.forkRange.max}
                        </FormLabel>
                        <Slider
                      ...
                      <Stack>
                        ...
                        <Checkbox
                          ...
                          onChange={(e) =>
                            updateFilters({ excludeEmpty: e.target.checked })
                          }
                        >
                    ...
                    <AccordionButton>
                      <Box flex="1" textAlign="left">
                        <Text fontWeight="bold">
                          {dataType === "issues" ? "Issue" : "Merge Request"}{" "}
                          Filters
                        </Text>
                      </Box>
                      <AccordionIcon />
                    ...
                    <SimpleGrid columns={{ base: 1, md: 2, lg: 3 }} spacing={4}>
                      ...
                      <FormControl>
                        ...
                        <Select
                          ...
                          onChange={(e) =>
                            updateFilters({ milestone: e.target.value })
                          }
                        >
