/**
 * Linear Desktop OAuth Callback Page
...
interface LinearDesktopCallbackProps {
  isDesktop?: boolean;
}

interface LinearCallbackData {
  success: boolean;
  message?: string;
  user_info?: {
    id: string;
...
const LinearDesktopCallback: React.FC<LinearDesktopCallbackProps> = ({
  isDesktop = true,
}) => {
  const [loading, setLoading] = useState(true);
  const [callbackData, setCallbackData] = useState<LinearCallbackData | null>(null);
  ...
  // Parse URL parameters for Linear OAuth 2.0
  useEffect(() => {
    const handleCallback = async () => {
      try {
        ...
        const data: LinearCallbackData = await response.json();
        ...
        if (data.success) {
          ...
          if (isDesktop) {
            try {
              ...
              const callbackUrl = `atom://auth/linear/success?${new URLSearchParams({
                success: 'true',
          ...
          toast({
            ...
            description: `Welcome, ${data.user_info?.displayName || data.user_info?.name}!`,
            status: 'success',
            duration: 5000,
          });
        } else {
          ...
          toast({
            title: 'Linear Authentication Failed',
  ...
  const redirectToApp = () => {
    if (isDesktop) {
      ...
      try {
        window.location.href = `atom://auth/linear/close`;
      } catch (err) {
        ...
        <Card>
          <CardBody>
            <VStack spacing={6} align="center" py={10}>
              ...
              <Heading size="lg">Processing Linear Authentication</Heading>
              <Text color="gray.600" textAlign="center">
                Please wait while we complete your Linear connection...
              </Text>
              {isDesktop && (
        ...
        <Card>
          <CardBody>
            <VStack spacing={6} align="center" py={10}>
              ...
              <VStack spacing={3}>
                <Text color="gray.600" textAlign="center">
                  There was a problem connecting your Linear account. Please try again.
                </Text>
          ...
          <VStack spacing={6} align="center" py={10}>
            ...
            <VStack spacing={2}>
              ...
              <Text fontSize="xl" fontWeight="medium">
                Welcome, {callbackData?.user_info?.displayName || callbackData?.user_info?.name}!
              </Text>
              {isDesktop && (
                  ...
                  <VStack spacing={4} align="center">
                    <Avatar
                      src={callbackData.user_info.avatarUrl || 
                             `https://ui-avatars.com/api/?name=${encodeURIComponent(callbackData.user_info.displayName || callbackData.user_info.name)}&background=5E6AD2&color=fff&size=128`
                      }
                      alt={callbackData.user_info.displayName || callbackData.user_info.name}
                      size="xl"
                      border="3px solid"
                      borderColor="#5E6AD2"
                    />
                    ...
                    <VStack spacing={1} align="center">
                      <Text fontWeight="bold" fontSize="lg">
                        {callbackData.user_info.displayName || callbackData.user_info.name}
                      </Text>
                      <Text color="gray.600">
                        {callbackData.user_info.email}
                      </Text>
                      <Badge size="sm" colorScheme="blue">
                        {callbackData.user_info.role}
                      </Badge>
                      <Badge size="sm" colorScheme={callbackData.user_info.active ? 'green' : 'red'}>
                        {callbackData.user_info.active ? 'Active' : 'Inactive'}
                      </Badge>
                    </VStack>
                    
                    {callbackData.user_info.organization && (
                      <VStack spacing={1} align="center">
                        <Text fontWeight="medium" fontSize="sm" color="gray.600">
                          Organization
                        </Text>
                        <Text color="gray.800">
                          {callbackData.user_info.organization.name}
                        </Text>
                        <Text fontSize="sm" color="gray.500">
                          {callbackData.user_info.organization.urlKey}.linear.app
                        </Text>
                      </VStack>
                    )}
                  </VStack>
            ...
            <VStack spacing={4} w="full">
              ...
              <Text color="gray.600" textAlign="center">
                Your Linear account is now connected! You can:
              </Text>
...
export default LinearDesktopCallback;