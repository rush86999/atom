...
export class FigmaCreateFileSkill implements Skill {
  ...
  async execute(context: SkillContext): Promise<SkillResult> {
    try {
      ...
      const fileName = this.extractFileName(intent) || 
...
export class FigmaSearchComponentsSkill implements Skill {
  ...
  async execute(context: SkillContext): Promise<SkillResult> {
    try {
      ...
      const componentType = this.extractComponentType(intent) ||
      ...
      if (!componentType) {
        return {
          success: false,
          message: 'Please specify what type of component you want to search for',
  ...
  async execute(context: SkillContext): Promise<SkillResult> {
    try {
      ...
      if (!fileName && !comment) {
        return {
          success: false,
          message: 'Please specify which file and what feedback you want to add',
          error: 'Missing file or comment information'
        };
      }

      // For demo purposes, we'll create a mock file key
      const fileKey = fileName ? `FILE_KEY_${fileName.replace(/\s+/g, '_').toUpperCase()}` : 'MOCK_FILE_KEY';
      
      // Call Figma comment API
      const response = await fetch(`/api/integrations/figma/files/${fileKey}/comments`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
