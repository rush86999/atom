...
export class MetricsCollector extends EventEmitter {
  ...
  recordTaskStart(taskId: string, agentId: string, skillUsed: string, complexity: number = 1): void {
    ...
    this.systemMetrics.queueLength = Math.max(0, this.systemMetrics.queueLength - 1);
    this.logger.debug(`Task started: ${taskId} assigned to ${agentId}`);

    this.emit('task-started', { taskId, agentId, skillUsed, timestamp: metric.startTime });
  ...
  recordTaskComplete(taskId: string, duration?: number): void {
    ...
    if (metric) {
      ...
      metric.duration = duration || metric.startTime.getTime() - metric.completionTime.getTime();
      ...
      this.emit('task-completed', { taskId, duration: metric.duration, timestamp: metric.completionTime });
    }
    this.updateSystemMetrics();
  }
  ...
  recordTaskFailure(taskId: string, error?: string): void {
    ...
    if (metric) {
      ...
      this.emit('task-failed', { taskId, error, timestamp: metric.completionTime });
    }
    this.updateSystemMetrics();
  }
  ...
  calculateSuccessRate(): void {
    ...
    this.systemMetrics.successRate = total > 0 ? (completed - failed) / total : 0;
  }
  ...
  calculateAverageDuration(): void {
    ...
    this.systemMetrics.averageDuration = completedTasks.length > 0
      ? completedTasks.reduce((sum, duration) => sum + duration, 0) / completedTasks.length
      : 0;
  }
  ...
  detectBottlenecks(): string[] {
    ...
    if (this.systemMetrics.successRate < 0.7) {
      indicators.push(`High failure rate: ${(this.systemMetrics.successRate * 100).toFixed(1)}%`);
    }

    // Long queue
    if (this.systemMetrics.queueLength > 10) {
      indicators.push(`Queue overflow: ${this.systemMetrics.queueLength} pending tasks`);
    }

    // Slow tasks
    if (this.systemMetrics.averageDuration > 300
