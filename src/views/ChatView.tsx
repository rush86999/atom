...
export const ChatView = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // FIX: The `sendMessageStream` method expects a `message` property, not `contents`.
    ...
    <div className="chat-container">
      ...
      <footer className="chat-footer">
        <form onSubmit={handleSubmit} className="input-form">
          <textarea ref={textareaRef} value={inputValue} onChange={(e) => setInputValue(e.target.value)} onKeyDown={(e) => { if (e.key === "Enter" && !e.shiftKey) { e.preventDefault(); handleSubmit(e); } }} placeholder="Ask Atom anything..." rows={1} disabled={isStreaming || !chat} aria-label="Chat input" />
