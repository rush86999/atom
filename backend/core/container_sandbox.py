...
class ContainerSandbox:
    ...
    async def _execute_docker(
        ...
        try:
            ...
            try:
                ...
                # Killing the docker CLI client does NOT stop the container
                # (--rm only fires on a clean docker run exit) — it would run
                # forever. Read the container id from --cidfile and kill it.
                process.kill()
                await process.wait()
                await self._kill_docker_container(cid_path)
                return {
                    "status": "failed",
    ...
    async def _kill_docker_container(self, cid_path: str) -> None:
        """Best-effort kill of a container whose CLI client timed out."""
        try:
            import os
            ...
            logger.warning("Failed to kill timed-out sandbox container")
    ...
    async def _execute_subprocess(
        ...
        try:
            # Best-effort POSIX resource limits for the host subprocess
            # fallback (weaker isolation than Docker — cap CPU/address space
            # so runaway generated code can't pin the host).
            limit_fn = self._resource_limit_preexec()
            process = await asyncio.create_subprocess_exec(
                "python3",
                code_path,
                stdout=asyncio.subprocess.PIPE,
    ...
    def _resource_limit_preexec():
        """Return a preexec_fn applying POSIX rlimits (None on Windows)."""
        import sys
        ...
        def _apply_limits() -> None:
            try:
                ...
                resource.setrlimit(
                    resource.RLIMIT_CPU, (DEFAULT_TIMEOUT, DEFAULT_TIMEOUT)
                )
                resource.setrlimit(
                    resource.RLIMIT_AS,
