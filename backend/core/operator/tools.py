...
the loop, every action is audited (BrowserAudit) and guardrails hard-stop
risky (money/checkout) actions. Maturity gating also applies at the MCP
dispatch layer via the registered tool metadata.
...
async def _governance_allows(db, user_id: str,
    ...
    """Entry gate — same check browser_create_session runs (browser perms
    are INTERN+). Per-action policy lives inside the loop (guardrails +
    optional governance callback + audit)."""
    if not (FeatureFlags.should_enforce_governance("browser")
            and agent_id and db is not None):
        return True
    try:
...
async def _run_operator_task(state: OperatorRunState,
    ...
    try:
        ...
        await backend.start(start_url=start_url)
