...
class DeliveryGuard:
    ...
    def check_overdue_risk(self, contract_id: str, db: Session = None) -> Dict[str, Any]:
        ...
        # Use context manager if db not provided
        if db is None:
            with get_db_session() as db:
                return self._check_overdue_risk_impl(contract_id, db)
        else:
            return self._check_overdue_risk_impl(contract_id, db)

    def _check_overdue_risk_impl(self, contract_id: str, db: Session) -> Dict[str, Any]:
        """Implementation of overdue risk check."""
    ...
    def pause_high_risk_projects(self, workspace_id: str, db: Session = None) -> List[str]:
        ...
        if db is None:
            with get_db_session() as db:
                return self._pause_high_risk_projects_impl(workspace_id, db)
        else:
            return self._pause_high_risk_projects_impl(workspace_id, db)

    def _pause_high_risk_projects_impl(self, workspace_id: str, db: Session) -> List[str]:
        """Implementation of high-risk project pause logic."""
        paused_projects = []
        ...
        for project in projects:
            if project.contract_id:
                risk_data = self._check_overdue_risk_impl(project.contract_id, db)
