...
@router.post("/preseed")
async def preseed_cache(req: CachePreseedRequest, admin: User = Depends(get_platform_admin)) -> Dict[str, Any]:
...
@router.get("/stats")
async def cache_stats(admin: User = Depends(get_platform_admin)) -> Dict[str, Any]:
...
@router.get("/health")
async def cache_health(admin: User = Depends(get_platform_admin)) -> Dict[str, Any]:
