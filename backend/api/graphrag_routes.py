...
async def build_communities(user_id: str):
    ...
    return {"communities_built": count, "user_id": user_id}
...
async def query_graphrag(request: QueryRequest):
    ...
    return graphrag_engine.query(request.user_id, request.query, request.mode)
...
async def get_stats(user_id: str = None):
    ...
    return graphrag_engine.get_stats(user_id)
