...
try:
    ...
    SALESFORCE_INTEGRATION_AVAILABLE = True
except ImportError:
    SALESFORCE_INTEGRATION_AVAILABLE = False
...
@comprehensive_integration_bp.route(
    "/api/integrations/salesforce/add", methods=["POST"]
)
def add_salesforce_integration():
    """Add Salesforce integration for user"""
    try:
        ...
        if not SALESFORCE_INTEGRATION_AVAILABLE:
            return jsonify(
                {
                    "error": "Salesforce integration service not available",
                    "success": False,
                }
            ), 503

        # For Salesforce, we need to initiate OAuth flow
        # This endpoint would typically redirect to Salesforce OAuth
        return jsonify(
            {
                "success": True,
                "message": "Salesforce integration initiated",
                "oauth_required": True,
                "next_step": "Initiate OAuth flow with Salesforce",
                "service": "salesforce",
            }
        )

    except Exception as e:
        logger.error(f"Error adding Salesforce integration: {e}")
        return jsonify(
            {
                "error": f"Failed to add Salesforce integration: {str(e)}",
                "success": False,
            }
        ), 500


@comprehensive_integration_bp.route("/api/integrations/salesforce/status")
def get_salesforce_integration_status():
    ...
    try:
        ...
        try:
            ...
            if db_conn_pool:
                tokens = asyncio.run(get_user_salesforce_tokens(db_conn_pool, user_id))
                    ...
                    return jsonify(
                        {
                            ...
                            "user_id": user_id,
                            "has_tokens": True,
                            "last_sync": tokens.get("updated_at"),
                        }
                    )
                else:
                    return jsonify(
                        {
                            ...
                            "user_id": user_id,
                            "has_tokens": False,
                            "message": "No Salesforce OAuth tokens found",
                        }
                    )
            else:
                return jsonify(
            ...
            return jsonify(
                {
                    ...
                    "service": "salesforce",
                    "user_id": user_id,
                    "message": "Salesforce OAuth database handler not available",
                }
            )

    except Exception as e:
        logger.error(f"Error getting Salesforce integration status: {e}")
        return jsonify(
            {
                "error": f"Failed to get Salesforce integration status: {str(e)}",
                "success": False,
            }
        ), 500
...
@comprehensive_integration_bp.route(
    "/api/integrations/salesforce/sync", methods=["POST"]
)
def sync_salesforce_data():
    """Sync Salesforce data to LanceDB memory"""
    try:
        user_id = request.json.get("user_id")
        data_types = request.json.get(
        ...
        # This would typically trigger an async background sync
        return jsonify(
            {
                "success": True,
                "message": "Salesforce data sync initiated",
                "service": "salesforce",
                "user_id": user_id,
                "data_types": data_types,
                "sync_id": f"sf_sync_{user_id}_{datetime.now(timezone.utc).isoformat()}",
            }
        )
        ...
        return jsonify(
            {"error": f"Failed to sync Salesforce data: {str(e)}", "success": False}
        ), 500


@comprehensive_integration_bp.route("/api/integrations/salesforce/search")
def search_salesforce_data():
    """Search Salesforce data through LanceDB"""
    try:
        user_id = request.args.get("user_id")
        ...
        if not user_id or not query:
            return jsonify(
                {"error": "user_id and query parameters required", "success": False}
            ), 400

        # This would search LanceDB for Salesforce data
        return jsonify(
            {
                "success": True,
                "service": "salesforce",
                ...
                "data_type": data_type,
                "results": [],
                "message": "Salesforce search functionality coming soon",
            }
        )

    except Exception as e:
        logger.error(f"Error searching Salesforce data: {e}")
        return jsonify(
            {"error": f"Failed to search Salesforce data: {str(e)}", "success": False}
        ), 500
...
def add_gdrive_integration():
    ...
    try:
        ...
        result = asyncio.run(
            service.add_user_gdrive_integration(user_id, config_overrides)
        )
...
def add_onedrive_integration():
    ...
    try:
        ...
        result = asyncio.run(
            service.add_user_onedrive_integration(user_id, config_overrides)
        )
        ...
        return jsonify(
            {"error": f"Failed to add OneDrive integration: {str(e)}", "success": False}
        ), 500
...
@comprehensive_integration_bp.route(
    "/api/integrations/communication/add", methods=["POST"]
)
def add_communication_integration():
    ...
    try:
        ...
        if not user_id or not platform:
            return jsonify(
                {"error": "user_id and platform required", "success": False}
            ), 400
        ...
        result = asyncio.run(
            service.add_user_communication_integration(
...
def get_communication_integration_status():
    ...
    try:
        ...
        service = get_communication_integration_service()
        result = asyncio.run(service.get_user_communication_status(user_id, platform))
...
def get_all_integration_status():
    ...
    try:
        ...
        if ONEDRIVE_INTEGRATION_AVAILABLE:
            ...
            onedrive_status = asyncio.run(
                onedrive_service.get_user_onedrive_status(user_id)
            )
        ...
        if COMMUNICATION_INTEGRATION_AVAILABLE:
            ...
            comm_status = asyncio.run(
                comm_service.get_user_communication_status(user_id)
            )
...
def get_all_integration_statistics():
    ...
    try:
        ...
        total_services = sum(
            1
            for service in statistics["services"].values()
            if service.get("status") == "active"
        )
        statistics["overall"] = {
            "total_available_services": len(
                [
                    s
                    for s in [
                        NOTION_INTEGRATION_AVAILABLE,
                        GDRIVE_INTEGRATION_AVAILABLE,
                        ONEDRIVE_INTEGRATION_AVAILABLE,
                        COMMUNICATION_INTEGRATION_AVAILABLE,
                    ]
                    if s
                ]
            ),
...
def integration_health_check():
    ...
    try:
        ...
        health_status["overall"] = {
            ...
            "health_ratio": active_services / total_services
            if total_services > 0
            else 0,
        }
        ...
        return jsonify(
            {"error": f"Failed health check: {str(e)}", "success": False}
        ), 500
...
def remove_integration():
    ...
    try:
        ...
        result = {
            "status": "not_found",
            "message": f"Platform '{platform}' not recognized",
        }
        ...
        # Remove OneDrive
            ...
            onedrive_service = get_onedrive_integration_service()
            result = asyncio.run(
                onedrive_service.remove_user_onedrive_integration(user_id)
            )

        # Remove Communication
        elif (
            platform in ["slack", "teams", "gmail", "outlook"]
            and COMMUNICATION_INTEGRATION_AVAILABLE
        ):
            comm_service = get_communication_integration_service()
            result = asyncio.run(
                comm_service.remove_user_communication_integration(user_id, platform)
            )
        ...
        return jsonify(
            {"error": f"Failed to remove integration: {str(e)}", "success": False}
        ), 500
