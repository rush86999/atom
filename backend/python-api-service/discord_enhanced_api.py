...
def require_discord_auth(func):
    ...
    def wrapper(*args, **kwargs):
        if not DISCORD_AUTH_AVAILABLE:
            return jsonify(
                {"ok": False, "error": "Discord authentication not available"}
            ), 503
        ...
        try:
            ...
            if DISCORD_ENHANCED_AVAILABLE:
                ...
                if not tokens:
                    return jsonify(
                        {"ok": False, "error": "Discord authentication required"}
                    ), 401
...
def get_current_user():
    ...
    try:
        if not DISCORD_ENHANCED_AVAILABLE:
            return jsonify(
                {"ok": False, "error": "Enhanced Discord service not available"}
            ), 503
    ...
    try:
        ...
        guilds = loop.run_until_complete(
            discord_enhanced_service.get_user_guilds(
                user_id=user_id, limit=limit, before=before, after=after
            )
        )
        loop.close()
        ...
        return jsonify(
            {
                ...
                "count": len(guilds),
                "filters": {"limit": limit, "before": before, "after": after},
            }
        )
...
def get_guild_info():
    ...
    try:
        ...
        guild = loop.run_until_complete(
            discord_enhanced_service.get_guild_info(user_id=user_id, guild_id=guild_id)
        )
        loop.close()
...
def create_channel():
    ...
    try:
        ...
        if not all([guild_id, name]):
            return jsonify(
                {"ok": False, "error": "Guild ID and channel name are required"}
            ), 400
        ...
        if not channel:
            return jsonify({"ok": False, "error": "Failed to create channel"}), 500
...
def send_message():
    ...
    try:
        ...
        if not all([channel_id, content]):
            return jsonify(
                {"ok": False, "error": "Channel ID and content are required"}
            ), 400
        ...
        return jsonify({"ok": True, "message": message.to_dict(), "message_sent": True})
...
def get_bot_info():
    ...
    try:
        ...
        bot = loop.run_until_complete(
            discord_enhanced_service.get_bot_info(bot_token=bot_token)
        )
        loop.close()

        if not bot:
            return jsonify({"ok": False, "error": "Bot information not available"}), 404
...
def sync_data():
    ...
    try:
        ...
        if "guilds" in sync_types:
            ...
            loop.close()
            results["guilds"] = {"count": len(guilds), "synced": True}
        ...
        if "channels" in sync_types:
            ...
            results["channels"] = {"count": channels_synced, "synced": True}
        ...
        if user_id:
            ...
            try:
                if DISCORD_AUTH_AVAILABLE:
                    ...
                    status_data["user_authenticated"] = (
                        True  # Simplified for sync context
                    )
            except:
...
def handle_webhook():
    ...
    try:
        ...
        if webhook_type:
            ...
            # Handle Discord event asynchronously
            import asyncio

            asyncio.create_task(handle_discord_event(data, webhook_type))
        else:
...
# Export components
__all__ = ["discord_enhanced_bp", "register_discord_enhanced_bp"]
