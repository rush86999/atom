...
async def list_teams():
    ...
    try:
        ...
        if not tokens:
            return jsonify({
                'ok': False,
                'error': {'message': 'Teams tokens not found'}
            }), 401
            ...
            if response.status_code == 200:
                ...
                for team in teams_data.get('value', []):
                    ...
                    if team_response.status_code == 200:
                        ...
                        team_info = {
                            'id': team['id'],
                            'displayName': team_details.get('displayName', team['displayName']),
                            'description': team_details.get('description'),
                            'visibility': team_details.get('visibility', 'public'),
                            'specialization': team_details.get('specialization', 'none'),
                ...
                if not include_private:
                    all_teams = [t for t in all_teams if t.get('visibility') === 'public']
                
                if limit:
                    all_teams = all_teams[:limit]
            
            return jsonify(format_teams_response({
                'teams': all_teams,
        ...
        async with create_teams_client(tokens['access_token']) as client:
            ...
            if response.status_code == 200:
                ...
                for channel in channels_data.get('value', []):
                    ...
                    if channel.get('membershipType') === 'private' and not include_private:
                        continue
                    ...
                    channel_info = {
                        ...
                        'isFavoriteByDefault': channel.get('isFavoriteByDefault', False),
                ...
                return jsonify({
                    'ok': False,
                    'error': {'message': f'Teams API error: {response.status_code}'}
...
async def list_messages():
    ...
    try:
        ...
        if not channel_id and not team_id:
            return jsonify({
                'ok': False,
                'error': {'message': 'channel_id or team_id is required'}
            }), 400
        ...
        async with create_teams_client(tokens['access_token']) as client:
            ...
            for ch_id in channel_ids:
                ...
                params = {'$top': min(limit // len(channel_ids) if channel_ids else limit, 50)}
                ...
                if response.status_code == 200:
                    ...
                    for message in messages_data.get('value', []):
                        ...
                        message_info = {
                            ...
                            'fromMe': message.get('from', {}).get('user', {}).get('id') == user_id,
                            'channelId': ch_id,
                            'teamId': team_id,
                            'author': {
                                'id': message.get('from', {}).get('user', {}).get('id'),
                                'displayName': message.get('from', {}).get('user', {}).get('displayName'),
                                'email': message.get('from', {}).get('user', {}).get('email')
                            },
                            'body': {
        ...
        async with create_teams_client(tokens['access_token']) as client:
            ...
            if response.status_code == 200:
                ...
                profile = {
                    ...
                    'lastPasswordChangeDateTime': user_data.get('lastPasswordChangeDateTime')
                }
