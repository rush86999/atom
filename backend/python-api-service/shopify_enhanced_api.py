...
try:
    from shopify_service import ShopifyService, get_shopify_client
    ...
    if not SHOPIFY_DB_AVAILABLE:
        ...
        return {
            "access_token": os.getenv(
                "SHOPIFY_ACCESS_TOKEN", "shpat_mock_access_token"
            ),
            "token_type": "Bearer",
...
async def list_products():
    ...
    try:
        ...
        if not tokens:
            return jsonify(
                {"ok": False, "error": {"message": "Shopify tokens not found"}}
            ), 401
...
async def _create_product(user_id: str, data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if not product_data.get("title"):
            return jsonify(
                {"ok": False, "error": {"message": "Product title is required"}}
            ), 400
        ...
        if SHOPIFY_SERVICE_AVAILABLE:
            ...
            if result.get("ok"):
                return jsonify(
                    format_shopify_response(
                        {"product": result.get("product")}, "products", "create_product"
                    )
                )
            else:
        ...
        return jsonify(
            format_shopify_response(
                {"product": mock_product}, "products", "create_product"
            )
        )
...
async def list_orders():
    ...
    try:
        ...
        mock_orders = [
            {
                ...
                "current_subtotal_price_set": {
                    "shop_money": {"amount": "299.99", "currency_code": "USD"},
                    "presentment_money": {"amount": "299.99", "currency_code": "USD"},
                },
                ...
                "current_total_price_set": {
                    "shop_money": {"amount": "326.99", "currency_code": "USD"},
                    "presentment_money": {"amount": "326.99", "currency_code": "USD"},
                },
                "current_total_tax": "27.00",
                "current_total_tax_set": {
                    "shop_money": {"amount": "27.00", "currency_code": "USD"},
                    "presentment_money": {"amount": "27.00", "currency_code": "USD"},
                },
                "customer_locale": "en",
                "device_id": None,
                "discount_codes": [],
                ...
                "fulfillments": [
                    {
                        ...
                        "receipt": {"testcase": False, "authorization": "123456789"},
                        "name": "#1001F1",
                        "line_items": [
                            {
                                "id": 1,
        ...
        if status and status != "any":
            filtered_orders = [
                o for o in filtered_orders if o.get("financial_status") == status
            ]
        if fulfillment_status:
            filtered_orders = [
                o
                for o in filtered_orders
                if o.get("fulfillment_status") == fulfillment_status
            ]
    ...
    try:
        ...
        if email:
            filtered_customers = [
                ...
                if c["email"] and email.lower() in c["email"].lower()
            ]
...
async def _create_order(user_id: str, data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if not order_data.get("line_items") or len(order_data["line_items"]) == 0:
            return jsonify(
                {"ok": False, "error": {"message": "Order line items are required"}}
            ), 400
        ...
        return jsonify(
            format_shopify_response({"order": mock_order}, "orders", "create_order")
        )
...
async def _create_customer(user_id: str, data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        return jsonify(
            format_shopify_response(
                {"customer": mock_customer}, "customers", "create_customer"
            )
        )
