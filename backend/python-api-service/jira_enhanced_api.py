...
def handle_jira_errors(func):
    ...
    def wrapper(*args, **kwargs):
        try:
            ...
            return jsonify(
                {"ok": False, "error": str(e), "error_type": "api_error"}
            ), 500
...
def require_jira_auth(func):
    ...
    def wrapper(*args, **kwargs):
        if not JIRA_AUTH_AVAILABLE:
            return jsonify(
                {"ok": False, "error": "Jira authentication not available"}
            ), 503
        ...
        try:
            ...
            if not tokens:
                return jsonify(
                    {"ok": False, "error": "Jira authentication required"}
                ), 401
...
def health():
    ...
    try:
        service_info = (
            jira_enhanced_service.get_service_info() if jira_enhanced_service else {}
        )
...
def create_issue():
    ...
    try:
        if not JIRA_ENHANCED_AVAILABLE:
            return jsonify(
                {"ok": False, "error": "Enhanced Jira service not available"}
            ), 503
        ...
        if not issue:
            return jsonify({"ok": False, "error": "Failed to create issue"}), 500
...
def get_issue():
    ...
    try:
        ...
        # Validate required fields
        if not issue_key:
            return jsonify({"ok": False, "error": "Issue key is required"}), 400

        # Run async operation
        loop = asyncio.new_event_loop()
        asyncio.set_event_loop(loop)
        issue = loop.run_until_complete(
            jira_enhanced_service.get_issue(user_id=user_id, issue_key=issue_key)
        )
        loop.close()
        ...
        return jsonify({"ok": True, "issue": issue.to_dict()})
...
def search_issues():
    ...
    try:
        ...
        # Validate required fields
        if not jql:
            return jsonify({"ok": False, "error": "JQL query is required"}), 400
        ...
        issues = loop.run_until_complete(
            jira_enhanced_service.search_issues(
                user_id=user_id, jql=jql, limit=limit, start_at=start_at, fields=fields
            )
        )
        loop.close()
...
def update_issue():
    ...
    try:
        ...
        if not issue_key or not fields:
            return jsonify(
                {"ok": False, "error": "Issue key and fields are required"}
            ), 400
        ...
        issue = loop.run_until_complete(
            jira_enhanced_service.update_issue(
                user_id=user_id, issue_key=issue_key, fields=fields
            )
        )
        loop.close()

        if not issue:
            return jsonify({"ok": False, "error": "Failed to update issue"}), 500
...
def list_projects():
    ...
    try:
        ...
        projects = loop.run_until_complete(
            jira_enhanced_service.list_projects(user_id=user_id, limit=limit)
        )
        loop.close()
    ...
    try:
        ...
        boards = loop.run_until_complete(
            jira_enhanced_service.list_boards(
                user_id=user_id, project_key=project_key, limit=limit
            )
        )
        loop.close()
        ...
        return jsonify(
            {
                ...
                "count": len(boards),
                "filters": {"project_key": project_key, "limit": limit},
            }
        )
...
def list_sprints():
    ...
    try:
        ...
        # Validate required fields
        if not board_id:
            return jsonify({"ok": False, "error": "Board ID is required"}), 400
        ...
        sprints = loop.run_until_complete(
            jira_enhanced_service.list_sprints(
                user_id=user_id, board_id=board_id, state=state
            )
        )
        loop.close()
        ...
        return jsonify(
            {
                ...
                "count": len(sprints),
                "filters": {"board_id": board_id, "state": state},
            }
        )
...
def sync_data():
    ...
    try:
        ...
        if "projects" in sync_types:
            ...
            loop.close()
            results["projects"] = {"count": len(projects), "synced": True}
        ...
        if "issues" in sync_types:
            ...
            issues = loop.run_until_complete(
                jira_enhanced_service.search_issues(
                    user_id=user_id, jql="order by created DESC", limit=1000
                )
            )
            loop.close()
            results["issues"] = {"count": len(issues), "synced": True}
        ...
        if "boards" in sync_types:
            ...
            loop.close()
            results["boards"] = {"count": len(boards), "synced": True}
...
def handle_webhook():
    ...
    try:
        ...
        if webhook_event:
            ...
            handle_jira_event(data, webhook_event)
        else:
...
def handle_jira_event(event_data: Dict[str, Any], event_type: str):
    """Handle Jira event"""
    try:
        ...
        if event_type == "jira:issue_created":
            handle_issue_created_event(event_data)
        elif event_type == "jira:issue_updated":
            handle_issue_updated_event(event_data)
        elif event_type == "jira:issue_deleted":
            handle_issue_deleted_event(event_data)
        elif event_type == "jira:sprint_started":
            handle_sprint_started_event(event_data)
        elif event_type == "jira:sprint_closed":
            handle_sprint_closed_event(event_data)

    except Exception as e:
        logger.error(f"Error processing Jira event: {e}")


def handle_issue_created_event(event_data: Dict[str, Any]):
...
def handle_issue_updated_event(event_data: Dict[str, Any]):
...
def handle_issue_deleted_event(event_data: Dict[str, Any]):
...
def handle_sprint_started_event(event_data: Dict[str, Any]):
...
def handle_sprint_closed_event(event_data: Dict[str, Any]):
...
# Export components
__all__ = ["jira_enhanced_bp", "register_jira_enhanced_bp"]
