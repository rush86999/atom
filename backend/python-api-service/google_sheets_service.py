...
class GoogleSheetsService:
    ...
    async def initialize(self, db_pool: Pool):
        ...
        try:
            ...
            if tokens and tokens.get("access_token"):
                ...
                logger.info(
                    f"Google Sheets service initialized for user {self.user_id}"
                )
                return True
            else:
    ...
    async def get_spreadsheets(
        self, query: str = None, page_size: int = 50
    ) -> Dict[str, Any]:
                    ...
                    spreadsheets.append(
                        {
                            ...
                            "owner": file.get("owners", [{}])[0].get(
                                "displayName", "Unknown"
                            ),
                            "ownerEmail": file.get("owners", [{}])[0].get(
                                "emailAddress", ""
                            ),
                            ...
                            "spreadsheetId": sheet_details.get(
                            ...
                            "timeZone": sheet_details.get("properties", {}).get(
                                "timeZone"
                            ),
                            "autoRecalc": sheet_details.get("properties", {}).get(
                                "autoRecalc"
                            ),
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    ...
                    headers=headers,
                    params={
                        "fields": "spreadsheetId,title,sheets(properties,gridProperties,charts),properties,spreadsheetUrl"
                    },
                )
                response.raise_for_status()
    ...
    async def get_sheet_data(
        self, spreadsheet_id: str, sheet_name: str = None, range: str = "A1:Z1000"
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                response = await client.put(
                    ...
                    headers=headers,
                    json={"values": data, "valueInputOption": "USER_ENTERED"},
                )
                response.raise_for_status()
            ...
            async with httpx.AsyncClient() as client:
                ...
                await self.log_activity(
                    "append_sheet_data",
                    {
                        ...
                        "sheet_name": sheet_name,
                        "range": range,
                        "rows_appended": result_data.get("updates", {}).get(
                            "updatedRows", 0
                        ),
                    },
                )
    ...
    async def clear_sheet_data(
        self, spreadsheet_id: str, sheet_name: str, range: str = None
    ) -> Dict[str, Any]:
    ...
    async def add_sheet(
        self, spreadsheet_id: str, sheet_properties: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                response = await client.post(
                    ...
                    headers=headers,
                    json={"requests": [{"addSheet": {"properties": sheet_properties}}]},
                )
                response.raise_for_status()
                ...
                return {"success": True, "data": result_data.get("replies", [{}])[0]}
    ...
    async def add_sheets(
        self, spreadsheet_id: str, sheets_data: List[Dict[str, Any]]
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for sheet_data in sheets_data:
                requests.append({"addSheet": {"properties": sheet_data}})
            ...
            async with httpx.AsyncClient() as client:
                ...
                return {"success": True, "data": result_data.get("replies", [])}
    ...
    async def search_spreadsheets(
        self, query: str, search_scope: str = "all"
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            if search_scope == "title":
                ...
                search_query += (
                    f" and name contains '{query}' and fullText contains '{query}'"
                )
    ...
    async def share_spreadsheet(
        self, spreadsheet_id: str, email_addresses: List[str], role: str = "reader"
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            for email in email_addresses:
                ...
                async with httpx.AsyncClient() as client:
                    ...
                    results.append({"email": email, "role": role, "success": True})
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                response = await client.delete(
                    f"{GOOGLE_DRIVE_API_BASE}/files/{spreadsheet_id}", headers=headers
                )
                response.raise_for_status()

                # Log activity
                await self.log_activity(
                    "delete_spreadsheet", {"spreadsheet_id": spreadsheet_id}
                )

                return {"success": True, "message": "Spreadsheet deleted successfully"}
