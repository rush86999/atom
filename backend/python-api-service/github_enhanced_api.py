...
try:
    from db_oauth_github import (
        get_user_github_tokens,
        save_user_github_tokens,
        delete_user_github_tokens,
    )
...
async def get_user_tokens(user_id: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        tokens = await get_user_github_tokens(
            None, user_id
...
async def list_repositories():
    ...
    try:
        ...
        if not tokens:
            return jsonify(
                {"ok": False, "error": {"message": "GitHub tokens not found"}}
            ), 401
...
async def _create_repository(user_id: str, data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if not repo_data.get("name"):
            return jsonify(
                {"ok": False, "error": {"message": "Repository name is required"}}
            ), 400
            ...
            if result:
                repo_response = {
                    ...
                    "owner": {
                        ...
                        "avatar_url": result.owner.get("avatar_url")
                        if result.owner
                        else None,
                    },
...
async def _create_issue(user_id: str, data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if not issue_data.get("title"):
            return jsonify(
                {"ok": False, "error": {"message": "Issue title is required"}}
            ), 400
        ...
        mock_issue = {
            ...
            "assignees": [
                {"login": assignee, "avatar_url": None}
                for assignee in (issue_data.get("assignees", [])[:1])
            ]
            if issue_data.get("assignees")
            else [],
            "labels": [
                {"name": label, "color": "c1c9e5"}
                for label in (issue_data.get("labels", [])[:1])
            ]
...
async def _create_pull_request(user_id: str, data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if not pr_data.get("title"):
            return jsonify(
                {"ok": False, "error": {"message": "Pull request title is required"}}
            ), 400
...
async def get_user_profile():
    ...
    try:
        ...
        return jsonify(
            format_github_response({"user": tokens["user_info"]}, "get_user_profile")
        )
...
def create_github_client(access_token: str) -> httpx.AsyncClient:
    ...
    return httpx.AsyncClient(
        base_url=GITHUB_API_BASE_URL, headers=headers, timeout=REQUEST_TIMEOUT
    )
    ...
    try:
        ...
        async with create_github_client(tokens["access_token"]) as client:
            ...
            params = {
                ...
                "per_page": min(
                    limit // len(repositories) if repositories else limit, 50
                ),
            }
                    ...
                    format_github_response(
                        {
                            ...
                            "incomplete_results": search_results.get(
        ...
        async with create_github_client(tokens["access_token"]) as client:
            ...
            if response.status_code == 200:
                ...
                profile = {
                    ...
                    "two_factor_authentication": user_data.get(
    ...
    try:
        ...
        async with create_github_client(tokens["access_token"]) as client:
            ...
            params = {
                "sha": branch,
                "per_page": min(
                    limit // len(repositories) if repositories else limit, 100
                ),
            }
