...
LANCEDB_CHUNKS_TABLE_NAME = os.environ.get("LANCEDB_CHUNKS_TABLE_NAME", "document_chunks")
...
async def create_generic_document_tables_if_not_exist(db_conn):
    ...
    try:
        ...
        document_schema = pa.schema([
            ...
            pa.field("vector_embedding", pa.list_(pa.float32(), 1536))  # OpenAI embedding dimension
        ])
...
async def add_processed_document(db_conn, doc_meta: Dict[str, Any], chunks_with_embeddings: List[Dict[str, Any]]) -> Dict[str, Any]:
    """
    ...
    """
    if not LANCEDB_AVAILABLE or db_conn is None:
        return {"status": "error", "message": "LanceDB not available", "code": "LANCEDB_UNAVAILABLE"}
    ...
    try:
        ...
        doc_data = {
            ...
            "ingested_at": doc_meta.get("ingested_at", datetime.now(timezone.utc).isoformat()),
            "processing_status": doc_meta.get("processing_status", "completed"),
            "error_message": doc_meta.get("error_message", ""),
            "total_chunks": len(chunks_with_embeddings),
            "vector_embedding": [0.0] * 1536  # Placeholder embedding for document level
        }
        ...
        logger.info(f"Successfully stored document {doc_meta.get('doc_id')} with {len(chunks_data)} chunks")
        return {"status": "success", "message": "Document stored successfully"}

    except Exception as e:
        logger.error(f"Failed to store document in LanceDB: {e}")
        return {"status": "error", "message": f"LanceDB storage failed: {str(e)}", "code": "LANCEDB_STORAGE_ERROR"}

async def search_documents(db_conn, query_vector: List[float], user_id: str, limit: int = 10) -> Dict[str, Any]:
    """
    ...
    try:
        ...
        results = chunks_table.search(query_vector) \
        ...
        for doc_id, doc_data in document_results.items():
            doc_info = docs_table.search([0.0] * 1536) \
        ...
        return {"status": "success", "results": final_results, "count": len(final_results)}

    except Exception as e:
        logger.error(f"Failed to search documents in LanceDB: {e}")
        return {"status": "error", "message": f"LanceDB search failed: {str(e)}", "code": "LANCEDB_SEARCH_ERROR"}
...
async def get_document_chunks(db_conn, doc_id: str, user_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        results = chunks_table.search([0.0] * 1536) \
            .where(f"doc_id = '{doc_id}' AND user_id = '{user_id}'") \
            .to_list()
        ...
        return {"status": "error", "message": f"LanceDB retrieval failed: {str(e)}", "code": "LANCEDB_RETRIEVAL_ERROR"}
...
async def delete_document(db_conn, doc_id: str, user_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        return {"status": "error", "message": f"LanceDB deletion failed: {str(e)}", "code": "LANCEDB_DELETION_ERROR"}
...
async def get_document_stats(db_conn, user_id: str) -> Dict[str, Any]:
    ...
    try:
        ...
        # Get document count
        docs = docs_table.search([0.0] * 1536) \
            .where(f"user_id = '{user_id}'") \
            .to_list()

        # Get chunk count
        chunks = chunks_table.search([0.0] * 1536) \
            .where(f"user_id = '{user_id}'") \
            .to_list()
        ...
        return {"status": "error", "message": f"LanceDB stats retrieval failed: {str(e)}", "code": "LANCEDB_STATS_ERROR"}
