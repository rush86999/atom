...
class TrelloEnhancedService:
    ...
    def _get_auth_params(self) -> Dict[str, str]:
        ...
        return {"key": self.api_key, "token": self.oauth_token}
    ...
    async def _make_request(
        ...
        try:
            ...
            logger.error(
                f"Trello API HTTP error: {e.response.status_code} - {e.response.text}"
            )
            return {
    ...
    async def get_boards(self, user_id: str, filter: str = None) -> List[TrelloBoard]:
        ...
        try:
            ...
            result = await self._make_request(
                "GET", f"members/{user_id}/boards", params
            )
            ...
            for board_data in result:
                board = TrelloBoard(
                    ...
                    background_image=board_data.get("prefs", {}).get(
                        "backgroundImage", ""
                    ),
                    background_image_scaled=board_data.get("prefs", {}).get(
                        "backgroundImageScaled", []
                    ),
                    background_tile=board_data.get("prefs", {}).get(
                        "backgroundTile", False
                    ),
                    background_brightness=board_data.get("prefs", {}).get(
                        "backgroundBrightness", "light"
                    ),
                    background_bottom_color=board_data.get("prefs", {}).get(
                        "backgroundBottomColor", ""
                    ),
                    background_top_color=board_data.get("prefs", {}).get(
                        "backgroundTopColor", ""
                    ),
                    card_cover_images=board_data.get("prefs", {}).get(
                        "cardCovers", True
                    ),
                    card_Aging=board_data.get("prefs", {}).get("cardAging", "regular"),
                    calendar_feed_enabled=board_data.get("prefs", {}).get(
                        "calendarFeedEnabled", False
                    ),
                    comment_permissions=board_data.get("prefs", {}).get(
                        "comments", "members"
                    ),
                    hidden_cards=board_data.get("prefs", {}).get("hiddenCards", ""),
                    hide_votes=board_data.get("prefs", {}).get("hideVotes", False),
                    invitations_enabled=board_data.get("prefs", {}).get(
                        "invitations", "members"
                    ),
                    invitees=board_data.get("prefs", {}).get("invitees", []),
                    level=board_data.get("level", ""),
                    limit_attachments=board_data.get("prefs", {}).get(
                        "limitAttachments", ""
                    ),
                    limited_attachments=board_data.get("prefs", {}).get(
                        "limitedAttachments", ""
                    ),
                    list_limit=board_data.get("prefs", {}).get("listLimit", 200),
                    max_attachment_size=board_data.get("prefs", {}).get(
                        "maxAttachmentSize", 10485760
                    ),
                    max_card_size=board_data.get("prefs", {}).get("maxCardSize", 16384),
                    max_description_length=board_data.get("prefs", {}).get(
                        "maxDescriptionLength", 16384
                    ),
                    max_name_length=board_data.get("prefs", {}).get(
                        "maxNameLength", 16384
                    ),
                    max_title_length=board_data.get("prefs", {}).get(
                        "maxTitleLength", 16384
                    ),
                    member_level=board_data.get("member_level", ""),
                    members_everywhere=board_data.get("prefs", {}).get(
                        "membersEverywhere", False
                    ),
                    members_omitted=board_data.get("prefs", {}).get(
                        "membersOmitted", []
                    ),
                    minutes_between_summaries=board_data.get("prefs", {}).get(
                        "minutesBetweenSummaries", 60
                    ),
                    need_a_board_or_card_to_email_to=board_data.get("prefs", {}).get(
                        "needABoardOrCardToEmailTo", ""
                    ),
                    need_an_email_to_see_comments=board_data.get("prefs", {}).get(
                        "needAnEmailToSeeComments", ""
                    ),
                    need_to_see_about_edits=board_data.get("prefs", {}).get(
                        "needToSeeAboutEdits", ""
                    ),
                    need_to_see_about_moves=board_data.get("prefs", {}).get(
                        "needToSeeAboutMoves", ""
                    ),
                    need_to_see_about_voting=board_data.get("prefs", {}).get(
                        "needToSeeAboutVoting", ""
                    ),
                    oauth_token_id=board_data.get("prefs", {}).get("oauthTokenId", ""),
                    preflight_board=board_data.get("prefs", {}).get(
                        "preflightBoard", ""
                    ),
                    preflight_card=board_data.get("prefs", {}).get("preflightCard", ""),
                    preflight_card_position=board_data.get("prefs", {}).get(
                        "preflightCardPosition", ""
                    ),
                    preflight_list=board_data.get("prefs", {}).get("preflightList", ""),
                    preflight_self_join=board_data.get("prefs", {}).get(
                        "preflightSelfJoin", ""
                    ),
                    preflight_share_board=board_data.get("prefs", {}).get(
                        "preflightShareBoard", ""
                    ),
                    preflight_share_card=board_data.get("prefs", {}).get(
                        "preflightShareCard", ""
                    ),
                    preflight_share_list=board_data.get("prefs", {}).get(
                        "preflightShareList", ""
                    ),
                    preflight_tag_card=board_data.get("prefs", {}).get(
                        "preflightTagCard", ""
                    ),
                    preflight_tag_list=board_data.get("prefs", {}).get(
                        "preflightTagList", ""
                    ),
                    preflight_tag_member=board_data.get("prefs", {}).get(
                        "preflightTagMember", ""
                    ),
                    preflight_unassign=board_data.get("prefs", {}).get(
                        "preflightUnassign", ""
                    ),
                    show_board_invite_ui=board_data.get("prefs", {}).get(
                        "showBoardInviteUi", ""
                    ),
                    show_card_cover_images=board_data.get("prefs", {}).get(
                        "showCardCoverImages", ""
                    ),
                    show_members_only_on_cards=board_data.get("prefs", {}).get(
                        "showMembersOnlyOnCards", False
                    ),
                    show_sidebar=board_data.get("prefs", {}).get("showSidebar", True),
                    show_sidebar_activity=board_data.get("prefs", {}).get(
                        "showSidebarActivity", True
                    ),
                    show_sidebar_board_actions=board_data.get("prefs", {}).get(
                        "showSidebarBoardActions", True
                    ),
                    show_sidebar_card_actions=board_data.get("prefs", {}).get(
                        "showSidebarCardActions", True
                    ),
                    show_sidebar_members=board_data.get("prefs", {}).get(
                    ...
                    use_limited_attachments=board_data.get("prefs", {}).get(
                        "useLimitedAttachments", ""
                    ),
                    voting_permitted=board_data.get("prefs", {}).get(
                        "votingPermission", "members"
                    ),
    ...
    async def get_board(
        ...
        try:
            ...
            board = TrelloBoard(
                ...
                background_image_scaled=result.get("prefs", {}).get(
                    "backgroundImageScaled", []
                ),
                background_tile=result.get("prefs", {}).get("backgroundTile", False),
                background_brightness=result.get("prefs", {}).get(
                    "backgroundBrightness", "light"
                ),
                background_bottom_color=result.get("prefs", {}).get(
                    "backgroundBottomColor", ""
                ),
                background_top_color=result.get("prefs", {}).get(
                    "backgroundTopColor", ""
                ),
                card_cover_images=result.get("prefs", {}).get("cardCovers", True),
                card_Aging=result.get("prefs", {}).get("cardAging", "regular"),
                calendar_feed_enabled=result.get("prefs", {}).get(
                    "calendarFeedEnabled", False
                ),
                ...
                members_everywhere=result.get("prefs", {}).get(
                    "membersEverywhere", False
                ),
                members_omitted=result.get("prefs", {}).get("membersOmitted", []),
                minutes_between_summaries=result.get("prefs", {}).get(
                    "minutesBetweenSummaries", 60
                ),
                need_a_board_or_card_to_email_to=result.get("prefs", {}).get(
                    "needABoardOrCardToEmailTo", ""
                ),
                need_an_email_to_see_comments=result.get("prefs", {}).get(
                    "needAnEmailToSeeComments", ""
                ),
                need_to_see_about_edits=result.get("prefs", {}).get(
                    "needToSeeAboutEdits", ""
                ),
                need_to_see_about_moves=result.get("prefs", {}).get(
                    "needToSeeAboutMoves", ""
                ),
                need_to_see_about_voting=result.get("prefs", {}).get(
                    "needToSeeAboutVoting", ""
                ),
                ...
                preflight_card_position=result.get("prefs", {}).get(
                    "preflightCardPosition", ""
                ),
                preflight_list=result.get("prefs", {}).get("preflightList", ""),
                preflight_self_join=result.get("prefs", {}).get(
                    "preflightSelfJoin", ""
                ),
                preflight_share_board=result.get("prefs", {}).get(
                    "preflightShareBoard", ""
                ),
                preflight_share_card=result.get("prefs", {}).get(
                    "preflightShareCard", ""
                ),
                preflight_share_list=result.get("prefs", {}).get(
                    "preflightShareList", ""
                ),
                preflight_tag_card=result.get("prefs", {}).get("preflightTagCard", ""),
                preflight_tag_list=result.get("prefs", {}).get("preflightTagList", ""),
                preflight_tag_member=result.get("prefs", {}).get(
                    "preflightTagMember", ""
                ),
                preflight_unassign=result.get("prefs", {}).get("preflightUnassign", ""),
                show_board_invite_ui=result.get("prefs", {}).get(
                    "showBoardInviteUi", ""
                ),
                show_card_cover_images=result.get("prefs", {}).get(
                    "showCardCoverImages", ""
                ),
                show_members_only_on_cards=result.get("prefs", {}).get(
                    "showMembersOnlyOnCards", False
                ),
                show_sidebar=result.get("prefs", {}).get("showSidebar", True),
                show_sidebar_activity=result.get("prefs", {}).get(
                    "showSidebarActivity", True
                ),
                show_sidebar_board_actions=result.get("prefs", {}).get(
                    "showSidebarBoardActions", True
                ),
                show_sidebar_card_actions=result.get("prefs", {}).get(
                    "showSidebarCardActions", True
                ),
                show_sidebar_members=result.get("prefs", {}).get(
                    "showSidebarMembers", True
                ),
                ...
                use_limited_attachments=result.get("prefs", {}).get(
                    "useLimitedAttachments", ""
                ),
                voting_permitted=result.get("prefs", {}).get(
                    "votingPermission", "members"
                ),
    ...
    async def get_cards(
        self, board_id: str, list_id: str = None, filter: str = None
    ) -> List[TrelloCard]:
        """Get Trello cards from board or list"""
        try:
            # Check cache first
            cache_key = self._generate_cache_key(
                "cards", f"{board_id}:{list_id or 'all'}"
            )
            ...
            for card_data in result:
                card = TrelloCard(
                    ...
                    manual_cover_attachment=card_data.get(
                    ...
                    id_attachment_background=card_data.get(
                        "idAttachmentBackground", ""
                    ),
    ...
    async def get_lists(
        self, board_id: str, include_cards: bool = False
    ) -> List[TrelloList]:
    ...
    async def get_members(
        self, board_id: str = None, include_boards: bool = False
    ) -> List[TrelloMember]:
        ...
        try:
            ...
            for member_data in result:
                ...
                member = TrelloMember(
                    ...
                    one_time_messages_dismissed=member_data.get(
                    ...
                    available_organizations=member_data.get(
                    ...
                    login_blocked_expiration=member_data.get(
                        "loginBlockedExpiration", ""
                    ),
                    login_blocked_for_header=member_data.get(
                        "loginBlockedForHeader", False
                    ),
                    ...
                    md_profile_background_color=member_data.get(
                        "mdProfileBackgroundColor", ""
                    ),
                    md_profile_background_url=member_data.get(
                        "mdProfileBackgroundUrl", ""
                    ),
                    ...
                    md_profile_link_color_preset=member_data.get(
                        "mdProfileLinkColorPreset", ""
                    ),
                    md_profile_link_color_locked=member_data.get(
                        "mdProfileLinkColorLocked", False
                    ),
                    md_profile_link_color_preset_locked=member_data.get(
                        "mdProfileLinkColorPresetLocked", False
                    ),
                    md_profile_link_color_name=member_data.get(
                        "mdProfileLinkColorName", ""
                    ),
                    md_profile_link_color_name_locked=member_data.get(
                        "mdProfileLinkColorNameLocked", False
                    ),
                    md_profile_link_color_value=member_data.get(
                        "mdProfileLinkColorValue", ""
                    ),
                    md_profile_link_color_value_locked=member_data.get(
                        "mdProfileLinkColorValueLocked", False
                    ),
                    md_profile_link_color_css=member_data.get(
                        "mdProfileLinkColorCss", ""
                    ),
                    md_profile_link_color_css_locked=member_data.get(
                        "mdProfileLinkColorCssLocked", False
                    ),
                    md_profile_link_color_hex=member_data.get(
                        "mdProfileLinkColorHex", ""
                    ),
                    md_profile_link_color_hex_locked=member_data.get(
                        "mdProfileLinkColorHexLocked", False
                    ),
                    md_profile_link_color_rgb=member_data.get(
                        "mdProfileLinkColorRgb", ""
                    ),
                    md_profile_link_color_rgb_locked=member_data.get(
                        "mdProfileLinkColorRgbLocked", False
                    ),
                    md_profile_link_color_rgba=member_data.get(
                        "mdProfileLinkColorRgba", ""
                    ),
                    md_profile_link_color_rgba_locked=member_data.get(
                        "mdProfileLinkColorRgbaLocked", False
                    ),
                    md_profile_link_color_hsl=member_data.get(
                        "mdProfileLinkColorHsl", ""
                    ),
                    md_profile_link_color_hsl_locked=member_data.get(
                        "mdProfileLinkColorHslLocked", False
                    ),
                    md_profile_link_color_hsla=member_data.get(
                        "mdProfileLinkColorHsla", ""
                    ),
                    md_profile_link_color_hsla_locked=member_data.get(
                        "mdProfileLinkColorHslaLocked", False
                    ),
                    md_profile_link_color_hsv=member_data.get(
                        "mdProfileLinkColorHsv", ""
                    ),
                    md_profile_link_color_hsv_locked=member_data.get(
                        "mdProfileLinkColorHsvLocked", False
                    ),
                    md_profile_link_color_hsva=member_data.get(
                        "mdProfileLinkColorHsva", ""
                    ),
                    md_profile_link_color_hsva_locked=member_data.get(
                        "mdProfileLinkColorHsvaLocked", False
                    ),
                    md_profile_link_color_hwb=member_data.get(
                        "mdProfileLinkColorHwb", ""
                    ),
                    md_profile_link_color_hwb_locked=member_data.get(
                        "mdProfileLinkColorHwbLocked", False
                    ),
                    md_profile_link_color_hwba=member_data.get(
                        "mdProfileLinkColorHwba", ""
                    ),
                    md_profile_link_color_hwba_locked=member_data.get(
                        "mdProfileLinkColorHwbaLocked", False
                    ),
                    md_profile_link_color_cmyk=member_data.get(
                        "mdProfileLinkColorCmyk", ""
                    ),
                    md_profile_link_color_cmyk_locked=member_data.get(
                        "mdProfileLinkColorCmykLocked", False
                    ),
                    md_profile_link_color_cmyka=member_data.get(
                        "mdProfileLinkColorCmyka", ""
                    ),
                    md_profile_link_color_cmyka_locked=member_data.get(
                        "mdProfileLinkColorCmykaLocked", False
                    ),
                    md_profile_link_color_ncol=member_data.get(
                        "mdProfileLinkColorNcol", ""
                    ),
                    md_profile_link_color_ncol_locked=member_data.get(
                        "mdProfileLinkColorNcolLocked", False
                    ),
                    md_profile_link_color_ncola=member_data.get(
                        "mdProfileLinkColorNcola", ""
                    ),
                    md_profile_link_color_ncola_locked=member_data.get(
                        "mdProfileLinkColorNcolaLocked", False
                    ),
                    md_profile_link_color_xyz=member_data.get(
                        "mdProfileLinkColorXyz", ""
                    ),
                    md_profile_link_color_xyz_locked=member_data.get(
                        "mdProfileLinkColorXyzLocked", False
                    ),
                    md_profile_link_color_xyza=member_data.get(
                        "mdProfileLinkColorXyza", ""
                    ),
                    md_profile_link_color_xyza_locked=member_data.get(
                        "mdProfileLinkColorXyzaLocked", False
                    ),
                    md_profile_link_color_lab=member_data.get(
                        "mdProfileLinkColorLab", ""
                    ),
                    md_profile_link_color_lab_locked=member_data.get(
                        "mdProfileLinkColorLabLocked", False
                    ),
                    md_profile_link_color_laba=member_data.get(
                        "mdProfileLinkColorLaba", ""
                    ),
                    md_profile_link_color_laba_locked=member_data.get(
                        "mdProfileLinkColorLabaLocked", False
                    ),
                    md_profile_link_color_lch=member_data.get(
                        "mdProfileLinkColorLch", ""
                    ),
                    md_profile_link_color_lch_locked=member_data.get(
                        "mdProfileLinkColorLchLocked", False
                    ),
                    md_profile_link_color_lcha=member_data.get(
                        "mdProfileLinkColorLcha", ""
                    ),
                    md_profile_link_color_lcha_locked=member_data.get(
                        "mdProfileLinkColorLchaLocked", False
                    ),
                    md_profile_link_color_luv=member_data.get(
                        "mdProfileLinkColorLuv", ""
                    ),
                    md_profile_link_color_luv_locked=member_data.get(
                        "mdProfileLinkColorLuvLocked", False
                    ),
                    md_profile_link_color_luva=member_data.get(
                        "mdProfileLinkColorLuva", ""
                    ),
                    md_profile_link_color_luva_locked=member_data.get(
                        "mdProfileLinkColorLuvaLocked", False
                    ),
                    md_profile_link_color_hsluv=member_data.get(
                        "mdProfileLinkColorHsluv", ""
                    ),
                    md_profile_link_color_hsluv_locked=member_data.get(
                        "mdProfileLinkColorHsluvLocked", False
                    ),
                    md_profile_link_color_hsluva=member_data.get(
                        "mdProfileLinkColorHsluva", ""
                    ),
                    md_profile_link_color_hsluva_locked=member_data.get(
                        "mdProfileLinkColorHsluvaLocked", False
                    ),
                    metadata={
                        "accessed_at": datetime.utcnow().isoformat(),
                        "source": "trello_api",
    ...
    async def create_card(
        ...
        desc: str = None,
        due: str = None,
        id_members: List[str] = None,
        id_labels: List[str] = None,
        pos: str = None,
        address: str = None,
        url_source: str = None,
        file_source: str = None,
        id_card_source: str = None,
    ) -> Optional[TrelloCard]:
        """Create Trello card"""
        try:
            # Build data
    ...
    async def create_checklist(
        self, name: str, id_card: str, check_items: List[str] = None
    ) -> Optional[TrelloChecklist]:
        """Create Trello checklist"""
        try:
            # Build data
            data = {"name": name, "idCard": id_card, "checkItems": check_items or []}
    ...
    async def create_label(
        self, name: str, color: str, id_board: str
    ) -> Optional[TrelloLabel]:
        """Create Trello label"""
        try:
            # Build data
    ...
    async def search_cards(
        self,
        query: str,
        id_boards: List[str] = None,
        id_cards: List[str] = None,
        id_labels: List[str] = None,
        id_members: List[str] = None,
        limit: int = 50,
    ) -> List[TrelloCard]:
    ...
    async def get_board_activities(
        self, board_id: str, limit: int = 50, before: str = None, since: str = None
    ) -> List[TrelloActivity]:
        ...
        try:
            ...
            # Make request
            result = await self._make_request(
                "GET", f"boards/{board_id}/actions", params
            )
