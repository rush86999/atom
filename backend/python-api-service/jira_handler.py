...
async def get_jira_client(user_id: str, db_conn_pool):
    """Get real Jira client with user's OAuth credentials"""
    try:
        ...
        if tokens:
            ...
            # For Jira, we need server URL and token
            server_url = os.getenv("JIRA_SERVER_URL")
            if server_url and access_token:
                client = get_real_jira_client(server_url, token=access_token)
                return RealJiraService(client)

        # Fallback to environment variables if no OAuth tokens
        server_url = os.getenv("JIRA_SERVER_URL")
        username = os.getenv("JIRA_USERNAME")
        api_token = os.getenv("JIRA_API_TOKEN")

        if all([server_url, username, api_token]):
            client = get_real_jira_client(server_url, username=username, password=api_token)
            return RealJiraService(client)
...
async def search_jira_route():
    ...
    try:
        client = await get_jira_client(user_id, current_app.config['DB_CONNECTION_POOL'])
        if not client:
...
async def list_issues():
    ...
    try:
        ...
        issues = client.list_files(project_id=project_id, page_size=page_size, page_token=page_token)
...
async def get_issue():
    ...
    if not user_id or not issue_id:
        return jsonify({"ok": False, "error": {"code": "VALIDATION_ERROR", "message": "user_id and issue_id are required."}}), 400
    ...
    try:
        ...
        logger.error(f"Error getting Jira issue {issue_id} for user {user_id}: {e}", exc_info=True)
        return jsonify({"ok": False, "error": {"code": "GET_ISSUE_FAILED", "message": str(e)}}), 500
...
async def create_issue():
    ...
    if not user_id or not project_id or not summary:
        return jsonify({"ok": False, "error": {"code": "VALIDATION_ERROR", "message": "user_id, project_id, and summary are required."}}), 400
    ...
    try:
        ...
        logger.error(f"Error creating Jira issue for user {user_id}: {e}", exc_info=True)
        return jsonify({"ok": False, "error": {"code": "CREATE_ISSUE_FAILED", "message": str(e)}}), 500
...
async def update_issue():
    ...
    try:
        ...
        logger.error(f"Error updating Jira issue {issue_id} for user {user_id}: {e}", exc_info=True)
        return jsonify({"ok": False, "error": {"code": "UPDATE_ISSUE_FAILED", "message": str(e)}}), 500
...
async def list_projects():
    ...
    try:
        ...
        logger.error(f"Error listing Jira projects for user {user_id}: {e}", exc_info=True)
        return jsonify({"ok": False, "error": {"code": "LIST_PROJECTS_FAILED", "message": str(e)}}), 500
