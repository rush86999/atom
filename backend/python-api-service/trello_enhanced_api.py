...
def list_boards():
    ...
    try:
        ...
        if not tokens:
            return jsonify(
                {"ok": False, "error": {"message": "Trello tokens not found"}}
            ), 401
        ...
        if TRELLO_SERVICE_AVAILABLE:
            ...
            return jsonify(
                format_trello_response(
                    {"boards": boards_data[:limit], "total_count": len(boards_data)},
                    "list_boards",
                )
            )
        ...
        return jsonify(
            format_trello_response(
                {"boards": mock_boards[:limit], "total_count": len(mock_boards)},
                "list_boards",
            )
        )
...
def list_lists():
    ...
    try:
        ...
        if TRELLO_SERVICE_AVAILABLE:
            ...
            return jsonify(
                format_trello_response(
                    {"lists": lists_data[:limit], "total_count": len(lists_data)},
                    "list_lists",
                )
            )
        ...
        return jsonify(
            format_trello_response(
                {"lists": mock_lists[:limit], "total_count": len(mock_lists)},
                "list_lists",
            )
        )
...
def list_cards():
    ...
    try:
        ...
        if TRELLO_SERVICE_AVAILABLE:
            ...
            return jsonify(
                format_trello_response(
                    {"cards": cards_data[:limit], "total_count": len(cards_data)},
                    "list_cards",
                )
            )
        ...
        return jsonify(
            format_trello_response(
                {"cards": mock_cards[:limit], "total_count": len(mock_cards)},
                "list_cards",
            )
        )
...
def _create_card(
    user_id: str, tokens: Dict[str, Any], data: Dict[str, Any]
) -> Dict[str, Any]:
    ...
    try:
        ...
        if not card_data.get("name"):
            return jsonify(
                {"ok": False, "error": {"message": "Card name is required"}}
            ), 400
...
def list_members():
    ...
    try:
        ...
        if TRELLO_SERVICE_AVAILABLE:
            ...
            return jsonify(
                format_trello_response(
                    {"members": members_data[:limit], "total_count": len(members_data)},
                    "list_members",
                )
            )
        ...
        mock_members = [
            {
                ...
                "premiumFeatures": [],
                "idEnterprises": [tokens.get("enterpriseId")]
                if tokens.get("enterpriseId")
                else [],
                "idTeams": [],
            },
            {
        ...
        return jsonify(
            format_trello_response(
                {"members": mock_members[:limit], "total_count": len(mock_members)},
                "list_members",
            )
        )
            ...
            format_trello_response(
                {
                    "user": {
                        ...
                        "avatarUrl": tokens["memberAvatar"]
                        or f"https://trello-members.s3.amazonaws.com/{tokens['memberId']}/170x170.png",
                        "memberType": "admin",
...
def search_trello():
    ...
    try:
        ...
        # Use Trello service
        if TRELLO_SERVICE_AVAILABLE:
            result = trello_service.search_trello(user_id, query, search_type, limit)
            return jsonify(result)
