...
try:
    from db_oauth_figma import (
        save_tokens,
        get_tokens,
        delete_tokens,
        refresh_figma_tokens,
    )
...
ATOM_OAUTH_ENCRYPTION_KEY = os.getenv(
    "ATOM_OAUTH_ENCRYPTION_KEY", "dev_encryption_key_change_in_production"
)
...
def callback():
    ...
    try:
        ...
        if not user_info:
            return jsonify(
                {"ok": False, "error": "Failed to retrieve user information"}
            ), 400
        ...
        if FIGMA_DB_AVAILABLE:
            ...
            encrypted_access_token = encrypt_data(
                access_token, ATOM_OAUTH_ENCRYPTION_KEY
            )
            encrypted_refresh_token = encrypt_data(
                token_response.get("refresh_token", ""), ATOM_OAUTH_ENCRYPTION_KEY
            )
            ...
            save_tokens(
                ...
                token_response.get("workspace_info", {}).get("team_id"),
            )
...
def disconnect():
    ...
    try:
        ...
        if FIGMA_DB_AVAILABLE:
            delete_tokens(None, user_id)  # db_conn_pool - will be passed in production

        return jsonify(
            {"ok": True, "message": "Figma integration disconnected successfully"}
        )
...
def refresh():
    ...
    try:
        ...
        if FIGMA_DB_AVAILABLE:
            encrypted_access_token = encrypt_data(
                token_response.get("access_token"), ATOM_OAUTH_ENCRYPTION_KEY
            )
            encrypted_refresh_token = encrypt_data(
        ...
        return jsonify({"ok": False, "error": "Failed to refresh tokens"}), 500
    except Exception as e:
    ...
    try:
        ...
        return {
            ...
            "username": user_data.get(
                "handle", user_data.get("email", "").split("@")[0]
            ),
...
def handle_file_comment(data: dict):
...
def handle_file_update(data: dict):
...
def handle_file_version(data: dict):
...
def handle_library_publish(data: dict):
