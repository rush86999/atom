...
try:
    from db_oauth_google import get_tokens, save_tokens, delete_tokens, get_user_google_data, save_google_data
...
async def list_gmail_messages():
    ...
    try:
        ...
        if not tokens:
            return jsonify({
                'ok': False,
                'error': {'message': 'Google tokens not found'}
            }), 401
        ...
        if GOOGLE_SERVICE_AVAILABLE:
            ...
            return jsonify(format_google_response({
        ...
        mock_messages = [
            {
                ...
                'size_estimate': 15234,
                'history_id': '12345',
                'internal_date': str(int((datetime.utcnow() - timedelta(days=1)).timestamp() * 1000)),
                'is_unread': True,
                'is_important': False,
            ...
            },
            {
                ...
                'size_estimate': 45678,
                'history_id': '12346',
                'internal_date': str(int((datetime.utcnow() - timedelta(days=3)).timestamp() * 1000)),
                'is_unread': False,
                'is_important': True,
...
async def _send_gmail_message(user_id: str, data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if not message_data.get('to') and not message_data.get('thread_id'):
            return jsonify({
                'ok': False,
                'error': {'message': 'To email or thread_id is required'}
            }), 400
        ...
        mock_message = {
            ...
            'thread_id': message_data.get('thread_id') or 'thread_new_' + str(int(datetime.utcnow().timestamp())),
    ...
    try:
        ...
        if GOOGLE_SERVICE_AVAILABLE:
            events = await google_service.get_calendar_events(
                user_id, calendar_id, time_min, time_max, q, max_results, single_events, order_by
            )
        ...
        mock_events = [
            ...
            },
            {
                ...
                'visibility': 'public',
                'attendees': [
                    {'email': 'client@company.com', 'displayName': 'Client', 'responseStatus': 'needsAction'}
                ],
                'creator': {'email': 'user@gmail.com', 'displayName': 'User'},
                'organizer': {'email': 'user@gmail.com', 'displayName': 'User'},
                ...
                'conference_data': {
                    ...
                    'entryPoints': [
                        {'entryPointType': 'video', 'uri': 'https://meet.google.com/jkl-mno-pqr'}
                    ]
                },
                'reminders': {
                    'useDefault': True
...
async def _create_calendar_event(user_id: str, data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        if not event_data.get('summary'):
            return jsonify({
                'ok': False,
                'error': {'message': 'Event summary is required'}
            }), 400
...
async def list_drive_files():
    ...
    try:
        ...
        fields = data.get('fields', 'nextPageToken, files(id, name, mimeType, size, createdTime, modifiedTime, parents, webViewLink)')
        ...
        if GOOGLE_SERVICE_AVAILABLE:
            ...
            files_data = [{
                ...
                'is_google_doc': file.mimeType.startswith('application/vnd.google-apps.'),
...
async def _create_drive_file(user_id: str, data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        mock_file = {
            ...
            'name': file_data['name'],
            'mimeType': file_data.get('mimeType', 'application/vnd.google-apps.document'),
            'size': '0',
            ...
            'is_google_doc': file_data.get('mimeType', '').startswith('application/vnd.google-apps.'),
    ...
    try:
        ...
        if 'gmail' in services:
            mock_results.append({
                ...
                'created_time': (datetime.utcnow() - timedelta(hours=2)).isoformat()
            })
        ...
        if 'calendar' in services:
            mock_results.append({
                ...
                'created_time': (datetime.utcnow() + timedelta(days=1, hours=14)).isoformat()
            })
