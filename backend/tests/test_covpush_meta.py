...
class TestModuleLevelRouting:
    @pytest.mark.asyncio
    async def test_module_check_governance_denied(self, monkeypatch):
        from core import atom_meta_agent as ama
        decision = SimpleNamespace(allowed=False, reason="denied")
        gov = MagicMock()
        gov.canPerformAction = AsyncMock(return_value=decision)
        with patch("core.atom_meta_agent.AgentGovernanceService", return_value=gov), \
             patch("core.database.SessionLocal"):
            allowed, reason = await ama._check_governance(None, "u1", "a1", "workflow")
        assert (allowed, reason) == (False, "denied")

    @pytest.mark.asyncio
    async def test_module_check_governance_allowed(self, monkeypatch):
        from core import atom_meta_agent as ama
        decision = SimpleNamespace(allowed=True, reason=None)
        gov = MagicMock()
        gov.canPerformAction = AsyncMock(return_value=decision)
        with patch("core.atom_meta_agent.AgentGovernanceService", return_value=gov), \
             patch("core.database.SessionLocal"):
            allowed, reason = await ama._check_governance(None, "u1", "a1", "chat")
        assert (allowed, reason) == (True, None)

    @pytest.mark.asyncio
    async def test_module_route_chat(self, monkeypatch):
        from core import atom_meta_agent as ama
        fake = SimpleNamespace()
        fake.llm = MagicMock()
        fake.llm.generate_response = AsyncMock(return_value="resp")
        result = await ama._route_to_chat(fake, "hello", "u1")
        assert result["route"] == "CHAT"
        assert result["response"] == "resp"

    @pytest.mark.asyncio
    async def test_module_route_with_governance_chat(self, monkeypatch):
        from core import atom_meta_agent as ama
        fake = SimpleNamespace()
        fake._route_to_chat = AsyncMock(return_value={"route": "CHAT", "status": "chat_complete"})
        result = await ama.route_with_governance(
            fake, "hi", _route_cls(IntentCategory.CHAT), "u1"
        )
        assert result["route"] == "CHAT"

    @pytest.mark.asyncio
    async def test_module_route_workflow(self, monkeypatch):
        from core import atom_meta_agent as ama
        fake = SimpleNamespace()
        fake._check_governance = AsyncMock(return_value=(True, None))
        fake._route_to_workflow = AsyncMock(return_value={"route": "WORKFLOW"})
        result = await ama.route_with_governance(
            fake, "wf", _route_cls(IntentCategory.WORKFLOW), "u1"
        )
        assert result["route"] == "WORKFLOW"

    @pytest.mark.asyncio
    async def test_module_route_denied(self, monkeypatch):
        from core import atom_meta_agent as ama
        fake = SimpleNamespace()
        fake._check_governance = AsyncMock(return_value=(False, "nope"))
        fake._propose_chat_alternative = AsyncMock(return_value={"route": "CHAT", "auto_takeover": True})
        result = await ama.route_with_governance(
            fake, "tasky", _route_cls(IntentCategory.TASK), "u1"
        )
        assert result["auto_takeover"] is True

    @pytest.mark.asyncio
    async def test_module_route_task(self, monkeypatch):
        from core import atom_meta_agent as ama
        fake = SimpleNamespace()
        fake._check_governance = AsyncMock(return_value=(True, None))
        fake._route_to_task = AsyncMock(return_value={"route": "TASK"})
        result = await ama.route_with_governance(
            fake, "tasky", _route_cls(IntentCategory.TASK), "u1"
        )
        assert result["route"] == "TASK"
    ...
    @pytest.mark.asyncio
    async def test_module_route_to_task(self, monkeypatch):
        from core import atom_meta_agent as ama
        fake = SimpleNamespace()
        fake.llm = MagicMock()
        admiral = MagicMock()
        admiral.recruit_and_execute = AsyncMock(return_value={"chain_id": "c1", "specialists_count": 1})
        with patch("core.fleet_admiral.FleetAdmiral", return_value=admiral), \
             patch("core.database.SessionLocal") as sl:
            db = sl.return_value.__enter__.return_value
            result = await ama._route_to_task(fake, "task", "u1")
        assert result["route"] == "TASK"

    @pytest.mark.asyncio
    async def test_module_propose_chat_alternative(self, monkeypatch):
        from core import atom_meta_agent as ama
        fake = SimpleNamespace()
        fake.llm = MagicMock()
        fake.llm.generate_response = AsyncMock(return_value="proposal")
        result = await ama._propose_chat_alternative(fake, "req", "workflow", "reason", "u1")
        assert result["auto_takeover"] is True


class TestModuleRouteWorkflow:
    """Covers module-level _route_to_workflow — was raising TypeError via
    generate_blueprint(user_id=...), which doesn't exist (bug)."""

    @pytest.mark.asyncio
    async def test_module_route_to_workflow(self, monkeypatch):
        from core import atom_meta_agent as ama
        from core.agents.queen_agent import QueenAgent

        fake = SimpleNamespace()
        fake.llm = MagicMock()
        fake.llm.generate_response = AsyncMock(return_value=json.dumps({
        ...
        with patch("core.database.SessionLocal") as sl:
            db = sl.return_value.__enter__.return_value
            result = await ama._route_to_workflow(fake, "build x", "u1")
        assert result["route"] == "WORKFLOW"
        assert result["architecture_name"] == "Arch"
        assert result["node_count"] == 1

    @pytest.mark.asyncio
    async def test_module_route_to_workflow_reuses_queen(self, monkeypatch):
        from core import atom_meta_agent as ama
        from core.agents.queen_agent import QueenAgent

        queen = QueenAgent(MagicMock(), MagicMock())
        queen.llm.generate_response = AsyncMock(return_value=json.dumps({
            "architecture_name": "Arch",
            "description": "d",
            "nodes": [],
        }))
        ...
        fake.queen = queen
        with patch("core.database.SessionLocal") as sl:
            result = await ama._route_to_workflow(fake, "build x", "u1", execution_mode="recurring_automation")
        assert result["node_count"] == 0


class TestModuleRouteWorkflowClassMethod:
    """The AtomMetaAgent._route_to_workflow method — was raising TypeError:
    QueenAgent(db, llm, user_id=...) and generate_blueprint(user_id=...)
    don't accept user_id (bug)."""

    @pytest.mark.asyncio
    async def test_class_route_to_workflow(self, meta_agent, monkeypatch):
        from core.agents.queen_agent import QueenAgent

        agent, sl = meta_agent
        agent.llm.generate_response = AsyncMock(return_value=json.dumps({
            "architecture_name": "Arch",
            "description": "d",
            "nodes": [{"id": "n1", "type": "agent", "name": "Agent"}],
        }))
        ...
        queen.llm = agent.llm
        with patch("core.atom_meta_agent.QueenAgent", return_value=queen):
            result = await agent._route_to_workflow("build x", "u1")
        assert result["route"] == "WORKFLOW"
        assert result["architecture_name"] == "Arch"
        assert result["node_count"] == 1

    @pytest.mark.asyncio
    async def test_class_route_to_workflow_reuses_queen(self, meta_agent):
        from core.agents.queen_agent import QueenAgent
        ...
        queen.llm = agent.llm
        agent.queen = queen
        result = await agent._route_to_workflow("build x", "u1", execution_mode="recurring_automation")
        assert result["node_count"] == 0
