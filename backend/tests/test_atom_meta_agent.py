                ...
                with patch('core.atom_meta_agent.AgentFleetService'):
                    with patch('core.atom_meta_agent.CapabilityGraduationService'):
                        with patch('core.atom_meta_agent.AdvancedWorkflowOrchestrator'):
                            ...
                            assert result.name == "Finance Analyst"
                ...
                with patch('core.atom_meta_agent.AgentFleetService'):
                    with patch('core.atom_meta_agent.CapabilityGraduationService'):
                        with patch('core.atom_meta_agent.AdvancedWorkflowOrchestrator'):
                            agent = AtomMetaAgent()
                            agent.llm = AsyncMock()
                            agent.llm.generate_response = AsyncMock(return_value="Sunny and 72 degrees")
                    ...
                    with patch('core.atom_meta_agent.CapabilityGraduationService'):
                        with patch('core.atom_meta_agent.AdvancedWorkflowOrchestrator'):
                            with patch('core.atom_meta_agent.QueenAgent') as mock_queen:
                                mock_gov.return_value.canPerformAction = AsyncMock(
                                    return_value=Mock(allowed=True, reason=None)
                                )
                                mock_queen.return_value.generate_blueprint = AsyncMock(
                                    return_value={
                                        "blueprint_id": "bp-1",
                                        "architecture_name": "Daily Sales Report",
                                        "nodes": [{"id": "n1"}, {"id": "n2"}]
                                    }
                                )
                                ...
                                agent = AtomMetaAgent()
                                result = await agent.route_with_governance(
                                    request="Run daily sales report",
                                    intent=IntentClassification(
        ...
        with patch('core.atom_meta_agent.WorldModelService'):
            with patch('core.atom_meta_agent.AgentGovernanceService') as mock_gov:
                with patch('core.atom_meta_agent.AgentFleetService'):
                    with patch('core.atom_meta_agent.CapabilityGraduationService'):
                        with patch('core.atom_meta_agent.AdvancedWorkflowOrchestrator'):
                            with patch('core.fleet_admiral.FleetAdmiral') as mock_admiral:
                                mock_gov.return_value.canPerformAction = AsyncMock(
                                    return_value=Mock(allowed=True, reason=None)
                                )
                                mock_admiral.return_value.recruit_and_execute = AsyncMock(
                                    return_value={"chain_id": "chain-1", "specialists_count": 2}
                                )

                                agent = AtomMetaAgent()
                                result = await agent.route_with_governance(
                                    request="Research competitors and create strategy",
                                    intent=IntentClassification(
                ...
                with patch('core.atom_meta_agent.AgentFleetService') as mock_fleet:
                    with patch('core.atom_meta_agent.CapabilityGraduationService'):
                        with patch('core.atom_meta_agent.AdvancedWorkflowOrchestrator'):
                            ...
                            assert result is not None
                            assert "agents" in str(result).lower() or "fleet" in str(result).lower()
                ...
                with patch('core.atom_meta_agent.AgentFleetService'):
                    with patch('core.atom_meta_agent.CapabilityGraduationService'):
                        with patch('core.atom_meta_agent.AdvancedWorkflowOrchestrator'):
                            ...
                            # Task distribution happens via fleet recruitment
                            result = await agent._recruit_fleet(
                ...
                with patch('core.atom_meta_agent.AgentFleetService') as mock_fleet:
                    with patch('core.atom_meta_agent.CapabilityGraduationService') as mock_grad:
                        with patch('core.atom_meta_agent.AdvancedWorkflowOrchestrator') as mock_orch:
                            ...
                            assert agent.graduation_service is not None or mock_grad.called
                ...
                with patch('core.atom_meta_agent.AgentFleetService'):
                    with patch('core.atom_meta_agent.CapabilityGraduationService'):
                        with patch('core.atom_meta_agent.AdvancedWorkflowOrchestrator'):
                            agent = AtomMetaAgent()
                            agent.llm = AsyncMock()
                            agent.llm.generate_response = AsyncMock(return_value="Hello there!")
                ...
                with patch('core.atom_meta_agent.AgentFleetService'):
                    with patch('core.atom_meta_agent.CapabilityGraduationService'):
                        with patch('core.atom_meta_agent.AdvancedWorkflowOrchestrator'):
                            agent = AtomMetaAgent()
                            agent.llm = AsyncMock()
                            agent.llm.generate_response = AsyncMock(return_value="Test response")
                ...
                with patch('core.atom_meta_agent.AgentFleetService'):
                    with patch('core.atom_meta_agent.CapabilityGraduationService'):
                        with patch('core.atom_meta_agent.AdvancedWorkflowOrchestrator'):
                            ...
                            # Check governance for a route
                            allowed, reason = await agent._check_governance(
                                user_id="user-123",
                                agent_id="atom-1",
                                route_category="workflow"
                            )
                    ...
                    with patch('core.atom_meta_agent.CapabilityGraduationService'):
                        with patch('core.atom_meta_agent.AdvancedWorkflowOrchestrator'):
                            mock_gov.return_value.canPerformAction = AsyncMock(
                                return_value=Mock(
                                    allowed=False,
                                    reason="STUDENT agents cannot execute deletions"
                                )
                            )
                            ...
                            # Governance denial reason should surface
                ...
                with patch('core.atom_meta_agent.AgentFleetService'):
                    with patch('core.atom_meta_agent.CapabilityGraduationService'):
                        with patch('core.atom_meta_agent.AdvancedWorkflowOrchestrator'):
                            ...
                            # Simulate spawn failure during capability registration
                            agent.graduation_service.reset_maturity = Mock(
        ...
        with patch('core.atom_meta_agent.WorldModelService') as mock_wm:
            with patch('core.atom_meta_agent.AgentGovernanceService'):
                with patch('core.atom_meta_agent.AgentFleetService'):
                    with patch('core.atom_meta_agent.CapabilityGraduationService'):
                        with patch('core.atom_meta_agent.AdvancedWorkflowOrchestrator'):
                            mock_wm.return_value.recall_experiences = AsyncMock(
                                return_value={"experiences": ["memory1", "memory2"]}
                            )
