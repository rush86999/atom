...
class TestWorkflowEngineExecutionGraph:
    ...
    def test_build_execution_graph_simple(self):
        ...
        # adjacency holds connection dicts (source/target), not node ids
...
class TestWorkflowEnginePauseResumeCancel:
    ...
    async def test_cancel_execution_adds_to_set(self):
        ...
        # Mock the state manager: persistence goes through the async engine,
        # which has no tables in the test environment.
        ...
        with patch('core.workflow_engine.get_connection_manager') as mock_ws:
            ...
            engine.state_manager.update_execution_status.assert_awaited_once_with(
                execution_id, "CANCELLED"
            )
            mock_manager.notify_workflow_status.assert_awaited_once()
    ...
    async def test_resume_workflow_invalid_state(self):
        ...
        # Mock an execution persisted in RUNNING state
        engine.state_manager = MagicMock()
        engine.state_manager.get_execution_state = AsyncMock(return_value={
        ...
        workflow = {"id": "test-wf"}
        result = await engine.resume_workflow("exec-running", workflow, {})
...
class TestWorkflowEngineStepExecution:
    ...
    async def test_execute_step_unknown_service(self):
        ...
        # Unknown services surface as errors (no fallback configured)
...
class TestWorkflowExecutionLogging:
    ...
    async def test_workflow_execution_log_created(self):
        ...
        # Mock persistence: the async engine has no tables in the test env.
        engine.state_manager = MagicMock()
        engine.state_manager.create_execution = AsyncMock(return_value="exec-log-1")
        engine.state_manager.update_execution_status = AsyncMock()
        engine.state_manager.update_step_status = AsyncMock()
        engine.state_manager.get_execution_state = AsyncMock(return_value={
            "execution_id": "exec-log-1",
            "workflow_id": "test-log-wf",
            "status": "RUNNING",
            "version": 1,
            "input_data": {},
            "steps": {},
        ...
        with patch.object(engine, '_execute_step', new=AsyncMock(return_value={"status": "success"})) as mock_execute:
            with patch('core.workflow_engine.get_connection_manager') as mock_ws:
                ...
                with patch('core.workflow_engine.get_db_session') as mock_db:
                    ...
                    # Execution was registered and the step actually ran
                    engine.state_manager.create_execution.assert_awaited_once_with(
                        "test-log-wf", {}
                    )
                    mock_execute.assert_awaited_once()
                    assert execution_id == "exec-log-1"
