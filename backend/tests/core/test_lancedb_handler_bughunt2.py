...
def test_get_document_by_id_escapes_doc_id():
    """BUG: get_document_by_id builds `f"id = '{doc_id}'"` without escaping
    doc_id, unlike search() which doubles single quotes. A doc_id with a
    quote breaks the filter (or injects SQL). The escaping must match search()."""
    ...
    def fake_where(filt):
        ...
        rv = MagicMock()
        rv.limit.return_value.to_pandas.return_value = empty_df
        return rv

    fake_table.search.return_value.where = fake_where
    with patch.object(h, "get_table", return_value=fake_table):
        h.get_document_by_id("docs", "it's a'doc")

    # The injected single quotes must be escaped (doubled), not passed through.
    assert captured["filter"] == "id = 'it''s a''doc'", (
        f"doc_id single quotes must be escaped via doubling, got: "
        f"{captured.get('filter')!r}"
    )
