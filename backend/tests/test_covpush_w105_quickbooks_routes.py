"""Coverage wave 105 — integrations/quickbooks_routes.py (TDD, 0% baseline).
...
from integrations import quickbooks_routes as qr


@pytest.fixture
def user():
    u = MagicMock(spec=User)
    u.id = "quickbooks105-user"
    u.email = "quickbooks105@x.com"
    return u
...
class TestAuthUrl:
    def test_success(self, anon_client):
        response = anon_client.get("/api/quickbooks/auth/url")
        assert response.status_code == 200
        body = response.json()
        assert "url" in body and "timestamp" in body
        assert "intuit.com" in body["url"]


class TestCallback:
    def test_success(self, anon_client):
        response = anon_client.get("/api/quickbooks/callback", params={"code": "abc"})
        assert response.status_code == 200
        body = response.json()
...
class TestStatus:
    def test_success(self, anon_client):
        response = anon_client.get("/api/quickbooks/status")
        assert response.status_code == 200
        body = response.json()
        assert body["ok"] is True
        assert body["service"] == "quickbooks"
        assert body["status"] == "connected"
        assert body["user_id"] == "test_user"

    def test_custom_user_id(self, anon_client):
        response = anon_client.get("/api/quickbooks/status", params={"user_id": "u1"})
        assert response.json()["user_id"] == "u1"


class TestSearch:
    def test_success(self, client):
        response = client.post("/api/quickbooks/search", json={"query": "invoice"})
        assert response.status_code == 200
        body = response.json()
        assert body["ok"] is True
        assert body["query"] == "invoice"
        assert body["results"][0]["title"] == "Sample Quickbooks Result - invoice"
        assert "timestamp" in body

    def test_custom_user_id_default(self, client):
        response = client.post("/api/quickbooks/search",
                               json={"query": "x", "user_id": "u9"})
        assert response.status_code == 200
        assert response.json()["query"] == "x"

    def test_missing_query_422(self, client):
        response = client.post("/api/quickbooks/search", json={})
        assert response.status_code == 422

    def test_anonymous_401(self, anon_client):
        response = anon_client.post("/api/quickbooks/search", json={"query": "invoice"})
        assert response.status_code == 401


class TestItems:
    def test_success(self, client):
        response = client.get("/api/quickbooks/items")
        assert response.status_code == 200
        body = response.json()
        assert body["ok"] is True
    ...
    def test_custom_user_id(self, client):
        response = client.get("/api/quickbooks/items", params={"user_id": "u2"})
        assert response.status_code == 200

    def test_anonymous_401(self, anon_client):
        response = anon_client.get("/api/quickbooks/items")
        assert response.status_code == 401
