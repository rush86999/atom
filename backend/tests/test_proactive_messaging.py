"""
Tests for Proactive Messaging Service
...
# Test database setup
TEST_DATABASE_URL = "sqlite:///./test_proactive_messaging.db"
...
class TestProactiveMessagingGovernance:
    ...
    def test_supervised_agent_auto_approved(self, db, supervised_agent):
        """SUPERVISED agents should be auto-approved but not immediately sent."""
        service = ProactiveMessagingService(db)
        ...
        message = service.create_proactive_message(
            ...
            platform="slack",
            recipient_id="C12345",
            content="Hello from supervised agent!",
            send_now=False,  # Don't send immediately
        )

        assert message.status == ProactiveMessageStatus.APPROVED.value
        assert message.agent_maturity_level == AgentStatus.SUPERVISED.value
...
class TestProactiveMessageApproval:
    ...
    def test_cancel_scheduled_message(self, db, supervised_agent):
        ...
        # Cancel the message
        cancelled_message = service.cancel_message(message_id=message.id)

        assert cancelled_message.status == ProactiveMessageStatus.CANCELLED.value

    def test_cannot_cancel_sent_message(self, db, supervised_agent):
        """Cannot cancel messages that are already sent."""
        service = ProactiveMessagingService(db)

        # Create a message and manually mark as sent
        message = service.create_proactive_message(
            agent_id=supervised_agent.id,
            platform="slack",
            recipient_id="C12345",
        ...
        with pytest.raises(Exception) as exc_info:
            service.cancel_message(message_id=message.id)

        assert "Cannot cancel message" in str(exc_info.value)


class TestProactiveMessageQueries:
    """Tests for querying proactive messages."""

    def test_get_pending_messages(self, db, intern_agent, supervised_agent):
        ...
        # Create approved message (SUPERVISED)
    ...
    def test_get_message_history_by_agent(self, db, intern_agent):
        """Test filtering message history by agent."""
        service = ProactiveMessagingService(db)
        ...
        history = service.get_message_history(agent_id=intern_agent.id)

        assert len(history) == 2
        assert all(msg.agent_id == intern_agent.id for msg in history)

    def test_get_message_history_by_platform(self, db, intern_agent):
        """Test filtering message history by platform."""
        service = ProactiveMessagingService(db)

        # Create messages on different platforms
        service.create_proactive_message(
            agent_id=intern_agent.id,
            platform="slack",
            recipient_id="C12345",
        ...
        # Get history for Slack only
        slack_history = service.get_message_history(platform="slack")

        assert len(slack_history) == 1
        assert slack_history[0].platform == "slack"
...
class TestScheduledMessages:
    ...
    def test_create_scheduled_message(self, db, supervised_agent):
        """Test creating a message scheduled for future delivery."""
        service = ProactiveMessagingService(db)
    ...
    def test_send_now_with_approved_agent(self, db, autonomous_agent):
        ...
        # Message should be approved and ready to send
        # (Actual sending happens in background task)
        assert message.status == ProactiveMessageStatus.APPROVED.value
        assert message.approved_at is not None
