...
# =========================================================================== #
# 5. core/governance/policy_engine.py
# =========================================================================== #
class TestPolicyEngine:
    def _engine(self, **cfg):
        from core.governance.policy_engine import PolicyEngine, PolicyEngineConfig
        return PolicyEngine(PolicyEngineConfig(**cfg))

    def test_rule_operators(self):
        from core.governance.policy_engine import PolicyOperator, PolicyRule
        R = PolicyRule
        assert R(field="a", operator=PolicyOperator.EQUALS, value=1).evaluate({"a": 1})
        assert R(field="a", operator=PolicyOperator.NOT_EQUALS, value=1).evaluate({"a": 2})
        assert R(field="a", operator=PolicyOperator.GREATER_THAN, value=1).evaluate({"a": 2})
        assert not R(field="a", operator=PolicyOperator.GREATER_THAN, value=1).evaluate({"a": "x"})
        assert R(field="a", operator=PolicyOperator.LESS_THAN, value=2).evaluate({"a": 1})
        assert R(field="a", operator=PolicyOperator.GREATER_EQUAL, value=2).evaluate({"a": 2})
        assert R(field="a", operator=PolicyOperator.LESS_EQUAL, value=2).evaluate({"a": 2})
        assert R(field="a", operator=PolicyOperator.IN, value=[1, 2]).evaluate({"a": 1})
        assert not R(field="a", operator=PolicyOperator.IN, value="notcoll").evaluate({"a": 1})
        assert R(field="a", operator=PolicyOperator.NOT_IN, value=[3]).evaluate({"a": 1})
        assert not R(field="a", operator=PolicyOperator.NOT_IN, value=5).evaluate({"a": 5})
        assert R(field="a", operator=PolicyOperator.CONTAINS, value="ell").evaluate({"a": "hello"})
        assert not R(field="a", operator=PolicyOperator.CONTAINS, value="x").evaluate({"a": 5})
        assert R(field="a", operator=PolicyOperator.MATCHES, value=r"h\w+").evaluate({"a": "hello"})
        assert not R(field="a", operator=PolicyOperator.MATCHES, value=r"\d+").evaluate({"a": "hello"})
        assert R(field="a", operator=PolicyOperator.EXISTS).evaluate({"a": None})
        assert not R(field="zz", operator=PolicyOperator.EXISTS).evaluate({"a": 1})
        # unknown operator
        rule = R(field="a", operator=PolicyOperator.EQUALS)
        rule.operator = "bogus"
        assert rule.evaluate({"a": 1}) is False
        # evaluate() raising -> False
        rule2 = PolicyRule(field="a", operator=PolicyOperator.MATCHES, value="(")  # invalid regex
        assert rule2.evaluate({"a": "hello"}) is False

    def _policy(self, pid, **kw):
        from core.governance.policy_engine import GovernancePolicy
        return GovernancePolicy(policy_id=pid, name=pid, **kw)

    def test_scope_matching(self):
        p = self._policy("p", action_filter=["write"], layer_filter=["exec"],
                         agent_filter={"dept": "fin"})
        assert p.matches_scope("a", "write", "exec", {"dept": "fin"})
        assert not p.matches_scope("a", "read", "exec", {"dept": "fin"})
        assert not p.matches_scope("a", "write", "plan", {"dept": "fin"})
        assert not p.matches_scope("a", "write", "exec", {"dept": "ops"})
        assert self._policy("q").matches_scope("a", "x", "y", {})

    def test_evaluate_rules_and_or(self):
        from core.governance.policy_engine import PolicyOperator, PolicyRule
        r_true = PolicyRule(rule_id="r1", field="a", operator=PolicyOperator.EQUALS, value=1)
        r_false = PolicyRule(rule_id="r2", field="b", operator=PolicyOperator.EQUALS, value=2)
        p_and = self._policy("and", rules=[r_true, r_false], match_all_rules=True)
        assert p_and.evaluate_rules({"a": 1, "b": 9}) == (False, 1)
        assert p_and.evaluate_rules({"a": 1, "b": 2}) == (True, 2)
        p_or = self._policy("or", rules=[r_true, r_false], match_all_rules=False)
        assert p_or.evaluate_rules({"a": 1, "b": 9}) == (True, 1)
        assert p_or.evaluate_rules({"a": 0, "b": 9}) == (False, 0)
        assert self._policy("empty", rules=[]).evaluate_rules({}) == (True, 0)

    def test_register_unregister_indexes_and_stats(self):
        from core.governance.policy_engine import PolicyPriority
        eng = self._engine(enable_cache=False)
        p1 = self._policy("p1", priority=PolicyPriority.HIGH, tags=["sec", "fin"])
        p2 = self._policy("p2", priority=PolicyPriority.LOW, tags=["sec"])
        eng.register_policy(p1)
        eng.register_policy(p2)

        assert eng.get_policies_by_tag("sec") == [p1, p2]
        assert eng.get_policies_by_tag("none") == []
        stats = eng.get_statistics()
        assert stats["total_policies"] == 2 and stats["total_rules"] == 0
        assert stats["priority_distribution"]["HIGH"] == 1
        assert stats["cache_size"] == 0

        assert eng.unregister_policy("p1") is True
        assert eng.unregister_policy("missing") is False
        assert eng.get_policies_by_tag("fin") == []

    def test_evaluate_flow_priority_cache_defaults(self):
        from core.governance.policy_engine import (PolicyOperator, PolicyPriority,
                                                   PolicyRule)
        eng = self._engine()  # cache on, short-circuit on
        deny = self._policy(
            "deny-1", priority=PolicyPriority.CRITICAL, outcome="deny",
            rules=[PolicyRule(field="risk", operator=PolicyOperator.GREATER_THAN, value=5)],
            action_filter=["deploy"], conditions=["needs approval"], confidence=0.9)
        advisory = self._policy(
            "adv-1", priority=PolicyPriority.ADVISORY, outcome="condition",
            rules=[PolicyRule(field="risk", operator=PolicyOperator.EQUALS, value=0)])
        inactive = self._policy(
            "off", priority=PolicyPriority.HIGH, outcome="deny",
            rules=[PolicyRule(field="risk", operator=PolicyOperator.EXISTS)])
        inactive.active = False
        for p in (advisory, inactive, deny):
            eng.register_policy(p)

        res = eng.evaluate("agent", "deploy", "exec", {"risk": 9})
        assert res.matched and res.outcome == "deny" and res.policy_id == "deny-1"
        assert res.rules_matched == 1 and res.confidence == 0.9
        assert "deny-1" in res.reasoning

        # cached second call (same key, within TTL)
        res2 = eng.evaluate("agent", "deploy", "exec", {"risk": 9})
        assert res2 is res
        assert eng._policy_cache  # cache populated

        # priority_limit excludes ADVISORY -> default allow
        res3 = eng.evaluate("agent", "read", "exec", {"risk": 0}, priority_limit=PolicyPriority.MEDIUM)
        assert res3.matched is False and res3.outcome == "allow"

        # expired cache entry re-evaluates
        res4 = eng.evaluate("agent", "deploy", "exec", {"risk": 9})
        cached = eng._policy_cache[list(eng._policy_cache)[0]]
        from datetime import datetime as dt, timedelta
        cached.evaluated_at = dt.now() - timedelta(seconds=eng.config.cache_ttl_seconds + 10)
        res5 = eng.evaluate("agent", "deploy", "exec", {"risk": 9})
        assert res5.policy_id == "deny-1"

        # unregister clears cache for that policy
        eng.unregister_policy("deny-1")
        assert all(r.policy_id != "deny-1" for r in eng._policy_cache.values())

        # no short-circuit: with short_circuit disabled the engine still falls
        # through to the default-allow result (documented current behavior).
        eng2 = self._engine(short_circuit=False)
        eng2.register_policy(advisory)
        res6 = eng2.evaluate("agent", "read", "exec", {"risk": 0})
        assert res6.matched is False and res6.outcome == "allow"

    def test_factory(self):
        import core.governance.policy_engine as pe
        eng = pe.get_policy_engine()
        assert eng is pe.get_policy_engine()
