...
def mock_cache():
    ...
    cache.verify_citations_batch = AsyncMock(return_value=[
        MagicMock(
            exists=True,
...
class TestCacheStatsEndpoint:
    ...
    def test_get_cache_stats_success(self, client, mock_user, mock_cache):
        """Test getting cache statistics successfully"""
        with patch('api.admin.jit_verification_routes.get_current_user', return_value=mock_user):
            with patch('api.admin.jit_verification_routes.require_role'):
    ...
    def test_get_cache_stats_unauthorized(self, client):
        ...
        # Don't mock user, will fail auth
        response = client.get("/api/admin/governance/jit/cache/stats")
...
class TestCacheClearEndpoint:
    ...
    def test_clear_cache_success(self, client, mock_user, mock_cache):
...
class TestVerifyCitationsEndpoint:
    ...
    def test_verify_citations_success(self, client, mock_user, mock_cache):
    ...
    def test_verify_citations_force_refresh(self, client, mock_user, mock_cache):
    ...
    def test_verify_citations_empty_list(self, client, mock_user, mock_cache):
...
class TestWorkerMetricsEndpoint:
    ...
    def test_get_worker_metrics_success(self, client, mock_user, mock_worker):
...
class TestWorkerStartEndpoint:
    ...
    def test_start_worker_success(self, client, mock_user, mock_worker):
    ...
    def test_start_worker_already_running(self, client, mock_user, mock_worker):
        """Test starting worker when already running"""
        mock_worker._running = True

        with patch('api.admin.jit_verification_routes.get_current_user', return_value=mock_user):
            with patch('api.admin.jit_verification_routes.require_role'):
                with patch('api.admin.jit_verification_workers.get_jit_verification_worker', return_value=mock_worker):
                    response = client.post("/api/admin/governance/jit/worker/start")
...
class TestWorkerStopEndpoint:
    ...
    def test_stop_worker_success(self, client, mock_user):
...
class TestVerifyFactCitationsEndpoint:
    ...
    def test_verify_fact_citations_success(self, client, mock_user, mock_worker):
        """Test verifying fact citations successfully"""
        with patch('api.admin.jit_verification_routes.get_current_user', return_value=mock_user):
            with patch('api.admin.jit_verification_routes.require_role'):
                with patch('api.admin.jit_verification_routes.get_jit_verification_worker', return_value=mock_worker):
                    response = client.post("/api/admin/governance/jit/worker/verify-fact/fact-123")
    ...
    def test_verify_fact_not_found(self, client, mock_user, mock_worker):
        ...
        with patch('api.admin.jit_verification_routes.get_current_user', return_value=mock_user):
            with patch('api.admin.jit_verification_routes.require_role'):
                with patch('api.admin.jit_verification_routes.get_jit_verification_worker', return_value=mock_worker):
                    response = client.post("/api/admin/governance/jit/worker/verify-fact/nonexistent-fact")
...
class TestTopCitationsEndpoint:
    ...
    def test_get_top_citations_default_limit(self, client, mock_user, mock_worker):
        """Test getting top citations with default limit"""
        with patch('api.admin.jit_verification_routes.get_current_user', return_value=mock_user):
            with patch('api.admin.jit_verification_routes.require_role'):
                with patch('api.admin.jit_verification_routes.get_jit_verification_worker', return_value=mock_worker):
                    response = client.get("/api/admin/governance/jit/worker/top-citations")
    ...
    def test_get_top_citations_custom_limit(self, client, mock_user, mock_worker):
        """Test getting top citations with custom limit"""
        with patch('api.admin.jit_verification_routes.get_current_user', return_value=mock_user):
            with patch('api.admin.jit_verification_routes.require_role'):
                with patch('api.admin.jit_verification_routes.get_jit_verification_worker', return_value=mock_worker):
                    response = client.get("/api/admin/governance/jit/worker/top-citations?limit=1")
    ...
    def test_get_top_citations_max_limit_enforced(self, client, mock_user, mock_worker):
        """Test that max limit is enforced"""
        with patch('api.admin.jit_verification_routes.get_current_user', return_value=mock_user):
            with patch('api.admin.jit_verification_routes.require_role'):
                with patch('api.admin.jit_verification_routes.get_jit_verification_worker', return_value=mock_worker):
                    # Try to request more than max (100)
                    response = client.get("/api/admin/governance/jit/worker/top-citations?limit=200")
...
class TestHealthEndpoint:
    ...
    def test_get_health_healthy(self, client, mock_user, mock_cache, mock_worker):
        """Test getting health status when system is healthy"""
        with patch('api.admin.jit_verification_routes.get_current_user', return_value=mock_user):
            with patch('api.admin.jit_verification_routes.require_role'):
                with patch('api.admin.jit_verification_routes.get_jit_verification_cache', return_value=mock_cache):
                    with patch('api.admin.jit_verification_routes.get_jit_verification_worker', return_value=mock_worker):
                        response = client.get("/api/admin/governance/jit/health")
    ...
    def test_get_health_degraded(self, client, mock_user, mock_cache, mock_worker):
        """Test getting health status when system is degraded"""
        # Simulate degraded state
        mock_worker.get_metrics.return_value = {
            "running": False,  # Worker not running
            "verified_count": 0,
            "failed_count": 0,
            "stale_facts": 10,  # Stale facts
            "outdated_facts": 0,
            "last_run_time": None,
            "last_run_duration": 0,
...
class TestCacheWarmEndpoint:
    ...
    def test_warm_cache_success(self, client, mock_user, mock_cache):
        ...
        with patch('api.admin.jit_verification_routes.get_current_user', return_value=mock_user):
            with patch('api.admin.jit_verification_routes.require_role'):
                with patch('api.admin.jit_verification_routes.get_jit_verification_cache', return_value=mock_cache):
                    with patch('api.admin.jit_verification_routes.WorldModelService') as mock_wm_class:
                        mock_wm = MagicMock()
                        mock_wm.list_all_facts = AsyncMock(return_value=mock_facts)
                        mock_wm_class.return_value = mock_wm

                        response = client.post("/api/admin/governance/jit/cache/warm?limit=100")
    ...
    def test_warm_cache_custom_limit(self, client, mock_user, mock_cache):
            ...
            with patch('api.admin.jit_verification_routes.require_role'):
                with patch('api.admin.jit_verification_routes.get_jit_verification_cache', return_value=mock_cache):
                    with patch('api.admin.jit_verification_routes.WorldModelService') as mock_wm_class:
                        ...
                        response = client.post("/api/admin/governance/jit/cache/warm?limit=500")
...
class TestConfigEndpoint:
    ...
    def test_get_config_success(self, client, mock_user, mock_cache, mock_worker):
        """Test getting configuration successfully"""
        with patch('api.admin.jit_verification_routes.get_current_user', return_value=mock_user):
            with patch('api.admin.jit_verification_routes.require_role'):
                with patch('api.admin.jit_verification_routes.get_jit_verification_cache', return_value=mock_cache):
                    with patch('api.admin.jit_verification_routes.get_jit_verification_worker', return_value=mock_worker):
                        response = client.get("/api/admin/governance/jit/config")
...
class TestErrorHandling:
    ...
    def test_verify_citations_error(self, client, mock_user, mock_cache):
    ...
    def test_worker_metrics_error(self, client, mock_user, mock_worker):
