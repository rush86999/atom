...
class TestAgentStart:
    ...
    def test_start_success(self, agent_control_client: TestClient):
        """Test successful daemon start with default parameters."""
        with patch('api.agent_control_routes.DaemonManager') as mock_dm:
            mock_dm.is_running.return_value = False
            mock_dm.start_daemon.return_value = 12345
    ...
    def test_start_with_custom_port(self, agent_control_client: TestClient):
        ...
        with patch('api.agent_control_routes.DaemonManager') as mock_dm:
            ...
            mock_dm.start_daemon.assert_called_once_with(
                port=9000,
                host="0.0.0.0",
                workers=1,
                host_mount=False,
    ...
    def test_start_with_custom_host(self, agent_control_client: TestClient):
    ...
    def test_start_with_workers(self, agent_control_client: TestClient):
    ...
    def test_start_with_host_mount(self, agent_control_client: TestClient):
    ...
    def test_start_with_dev_mode(self, agent_control_client: TestClient):
    ...
    def test_start_already_running(self, agent_control_client: TestClient):
        """Test start returns 400 when daemon already running."""
        with patch('api.agent_control_routes.DaemonManager') as mock_dm:
            mock_dm.is_running.return_value = True
            mock_dm.get_pid.return_value = 9999
    ...
    def test_start_returns_dashboard_url(self, agent_control_client: TestClient):
    ...
    def test_start_boundary_port_values(self, agent_control_client: TestClient):
    ...
    def test_start_runtime_error(self, agent_control_client: TestClient):
        """Test start returns 500 on RuntimeError from start_daemon."""
        with patch('api.agent_control_routes.DaemonManager') as mock_dm:
            mock_dm.is_running.return_value = False
            mock_dm.start_daemon.side_effect = RuntimeError("Port already in use")
    ...
    def test_start_io_error(self, agent_control_client: TestClient):
        ...
        with patch('api.agent_control_routes.DaemonManager') as mock_dm:
            mock_dm.is_running.return_value = False
            mock_dm.start_daemon.side_effect = IOError("Cannot write PID file")
...
class TestAgentStop:
    ...
    def test_stop_success(self, agent_control_client: TestClient):
        """Test successful daemon stop."""
        with patch('api.agent_control_routes.DaemonManager') as mock_dm:
            mock_dm.is_running.return_value = True

            response = agent_control_client.post("/api/agent/stop", json={})
            ...
            assert "message" in data
            mock_dm.stop_daemon.assert_called_once()

    def test_stop_not_running(self, agent_control_client: TestClient):
    ...
    def test_stop_calls_stop_daemon(self, agent_control_client: TestClient):
        """Test stop endpoint calls DaemonManager.stop_daemon()."""
        with patch('api.agent_control_routes.DaemonManager') as mock_dm:
            mock_dm.is_running.return_value = True

            agent_control_client.post("/api/agent/stop", json={})

            mock_dm.stop_daemon.assert_called_once()

    def test_stop_response_format(self, agent_control_client: TestClient):
    ...
    def test_stop_exception_handling(self, agent_control_client: TestClient):
        """Test stop returns 500 on unexpected exception."""
        with patch('api.agent_control_routes.DaemonManager') as mock_dm:
            mock_dm.is_running.return_value = True
            mock_dm.stop_daemon.side_effect = Exception("Unexpected error")
    ...
    def test_stop_when_already_stopped(self, agent_control_client: TestClient):
        ...
        with patch('api.agent_control_routes.DaemonManager') as mock_dm:
            ...
            # First stop
            response1 = agent_control_client.post("/api/agent/stop", json={})
            assert response1.status_code == 400

            # Second stop should also return 400
            response2 = agent_control_client.post("/api/agent/stop", json={})
...
class TestAgentRestart:
    ...
    def test_restart_when_running(self, agent_control_client: TestClient):
        """Test restart stops then starts daemon when running."""
        with patch('api.agent_control_routes.DaemonManager') as mock_dm:
            mock_dm.is_running.return_value = True
            mock_dm.start_daemon.return_value = 54321
            ...
            mock_dm.stop_daemon.assert_called_once()
            mock_dm.start_daemon.assert_called_once()

    def test_restart_when_not_running(self, agent_control_client: TestClient):
        ...
        with patch('api.agent_control_routes.DaemonManager') as mock_dm:
            ...
            mock_dm.stop_daemon.assert_not_called()
            mock_dm.start_daemon.assert_called_once()

    def test_restart_with_new_config(self, agent_control_client: TestClient):
    ...
    def test_restart_was_running_true(self, agent_control_client: TestClient):
    ...
    def test_restart_was_running_false(self, agent_control_client: TestClient):
    ...
    def test_restart_returns_new_pid(self, agent_control_client: TestClient):
    ...
    def test_restart_exception_handling(self, agent_control_client: TestClient):
        """Test restart returns 500 on unexpected exception."""
        with patch('api.agent_control_routes.DaemonManager') as mock_dm:
            mock_dm.is_running.side_effect = Exception("Daemon error")
...
class TestAgentStatus:
    ...
    def test_status_when_running(self, agent_control_client: TestClient):
        ...
        with patch('api.agent_control_routes.DaemonManager') as mock_dm:
            mock_dm.get_status.return_value = test_status
    ...
    def test_status_when_not_running(self, agent_control_client: TestClient):
    ...
    def test_status_includes_pid(self, agent_control_client: TestClient):
    ...
    def test_status_includes_uptime(self, agent_control_client: TestClient):
    ...
    def test_status_includes_memory(self, agent_control_client: TestClient):
    ...
    def test_status_includes_cpu(self, agent_control_client: TestClient):
    ...
    def test_status_exception_handling(self, agent_control_client: TestClient):
        """Test status returns 500 on get_status() exception."""
        with patch('api.agent_control_routes.DaemonManager') as mock_dm:
            mock_dm.get_status.side_effect = Exception("Status error")
...
class TestAgentControlErrorPaths:
    ...
    def test_start_permission_error(self, agent_control_client: TestClient):
        """Test start handles permission errors gracefully."""
        with patch('api.agent_control_routes.DaemonManager') as mock_dm:
            mock_dm.is_running.return_value = False
            mock_dm.start_daemon.side_effect = PermissionError("Permission denied")

            response = agent_control_client.post("/api/agent/start", json={})

            assert response.status_code == 500

    def test_concurrent_start_requests(self, agent_control_client: TestClient):
    ...
    def test_very_long_host_name(self, agent_control_client: TestClient):
    ...
    def test_maximum_workers(self, agent_control_client: TestClient):
    ...
    def test_minimum_workers(self, agent_control_client: TestClient):
    ...
    def test_boundary_port_values(self, agent_control_client: TestClient):
...
class TestAgentControlEdgeCases:
    ...
    def test_restart_loop_protection(self, agent_control_client: TestClient):
    ...
    def test_concurrent_stop_requests(self, agent_control_client: TestClient):
    ...
    def test_status_with_stale_pid_file(self, agent_control_client: TestClient):
