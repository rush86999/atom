"""
API Route Registration Tests (Standalone)
...
This is a standalone test that doesn't use integration conftest.py
    ...
    def test_all_api_routes_registered(self, test_app: FastAPI):
        ...
        for route in test_app.routes:
            if hasattr(route, 'path'):
                # Don't strip '/' for root path, but strip for others
                if route.path == '/':
