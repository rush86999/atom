# -*- coding: utf-8 -*-
"""Coverage wave 84 — core/policy_search_service (standalone, LLMService
mocked, real in-memory SQLite for GovernanceDocument).

- _generate_query_embedding: happy path + exception → zero-vector fallback.
...
from core.models import GovernanceDocument, User
...
@pytest.fixture()
def svc(db):
    with patch("core.llm_service.LLMService") as llm_cls:
        llm = llm_cls.return_value
        llm.generate_embedding = AsyncMock(return_value=[1.0, 0.0])
...
class TestGenerateQueryEmbedding:
    ...
    def test_exception_falls_back_to_zero_vector(self, svc):
        svc.llm_service.generate_embedding = AsyncMock(side_effect=RuntimeError("llm down"))
        embedding = asyncio.run(svc._generate_query_embedding("hello"))
...
class TestSearch:
    ...
    def test_excludes_pending_status(self, db, svc):
        _make_doc(db, "d1", status="pending", embedding=[1.0, 0.0])
        assert asyncio.run(svc.search(query="x", verification_status=None)) == []

    def test_excludes_deleted(self, db, svc):
        _make_doc(db, "d1", is_deleted=True, embedding=[1.0, 0.0])
        assert asyncio.run(svc.search(query="x", verification_status=None)) == []

    def test_excludes_expired(self, db, svc):
        _make_doc(db, "d1", expiration_date=_aware(days_ago=1), embedding=[1.0, 0.0])
        assert asyncio.run(svc.search(query="x", verification_status=None)) == []

    def test_includes_non_expiring(self, db, svc):
        _make_doc(db, "d1", expiration_date=None, embedding=[1.0, 0.0])
        assert len(asyncio.run(svc.search(query="x", verification_status=None))) == 1

    def test_domain_filter(self, db, svc):
        _make_doc(db, "d1", category="hr", embedding=[1.0, 0.0])
        _make_doc(db, "d2", category="finance", embedding=[1.0, 0.0])
    ...
    def test_verified_filter(self, db, svc):
        _make_doc(db, "d1", last_verified=_aware(hours_ago=1), embedding=[1.0, 0.0])
        _make_doc(db, "d2", last_verified=None, embedding=[1.0, 0.0])
