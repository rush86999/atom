"""Coverage wave 105 — integrations/zendesk_routes.py (TDD, 0% baseline).
...
from integrations import zendesk_routes as zr


@pytest.fixture
def user():
    u = MagicMock(spec=User)
    u.id = "zendesk105-user"
    u.email = "zendesk105@x.com"
    return u
...
class TestAuthUrl:
    def test_success(self, anon_client):
        response = anon_client.get("/api/zendesk/auth/url")
        assert response.status_code == 200
        body = response.json()
        assert "url" in body and "timestamp" in body
        assert "zendesk.com" in body["url"]


class TestCallback:
    def test_success(self, anon_client):
        response = anon_client.get("/api/zendesk/callback", params={"code": "abc"})
        assert response.status_code == 200
        body = response.json()
...
class TestStatus:
    def test_success(self, anon_client):
        response = anon_client.get("/api/zendesk/status")
        assert response.status_code == 200
        body = response.json()
        assert body["ok"] is True
        assert body["service"] == "zendesk"
        assert body["status"] == "connected"
        assert body["user_id"] == "test_user"

    def test_custom_user_id(self, anon_client):
        response = anon_client.get("/api/zendesk/status", params={"user_id": "u1"})
        assert response.json()["user_id"] == "u1"


class TestSearch:
    def test_success(self, client):
        response = client.post("/api/zendesk/search", json={"query": "orders"})
        assert response.status_code == 200
        body = response.json()
        assert body["ok"] is True
        assert body["query"] == "orders"
        assert body["results"][0]["title"] == "Zendesk Result - orders"
        assert "timestamp" in body

    def test_custom_user_id_default(self, client):
        response = client.post("/api/zendesk/search",
                               json={"query": "x", "user_id": "u9"})
        assert response.status_code == 200
        assert response.json()["query"] == "x"

    def test_missing_query_422(self, client):
        response = client.post("/api/zendesk/search", json={})
        assert response.status_code == 422

    def test_anonymous_401(self, anon_client):
        response = anon_client.post("/api/zendesk/search", json={"query": "orders"})
        assert response.status_code == 401


class TestItems:
    def test_success(self, client):
        response = client.get("/api/zendesk/items")
        assert response.status_code == 200
        body = response.json()
        assert body["ok"] is True
    ...
    def test_custom_user_id(self, client):
        response = client.get("/api/zendesk/items", params={"user_id": "u2"})
        assert response.status_code == 200

    def test_anonymous_401(self, anon_client):
        response = anon_client.get("/api/zendesk/items")
        assert response.status_code == 401
