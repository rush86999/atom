...
class TestAITriggerCoordinator(unittest.TestCase):
    ...
    def test_evaluate_data_disabled_setting(self):
        """Test that disabled setting prevents triggering"""
        self.coordinator._enabled = False
        
        result = asyncio.run(self.coordinator.evaluate_data(
    ...
    def test_evaluate_data_triggers_agent(self):
...
class TestUserSettingsToggle(unittest.TestCase):
    ...
    def test_setting_enabled(self, mock_session):
        """Test enabled setting allows coordinator"""
        mock_db = MagicMock()
        mock_session.return_value.__enter__.return_value = mock_db
        
        with patch("core.user_preference_service.UserPreferenceService") as mock_pref:
            ...
            enabled = asyncio.run(coordinator.is_enabled())
            self.assertTrue(enabled)
    
    @patch("core.database.get_db_session")
    def test_setting_disabled(self, mock_session):
