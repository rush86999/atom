"""
Unit Tests for BYOK Cache Pre-seeding Service (FIXED)
...
class TestPreseedAllCaches:
    ...
    @pytest.mark.asyncio
    async def test_preseeds_all_cache_types(self):
...
class TestPreseedCognitiveModels:
    ...
    async def test_counts_validated_models(self):
        """RED: Test that model validation is counted correctly across all tiers."""
        with patch('core.byok_cache_preseeding.CognitiveClassifier') as mock_classifier, \
             patch('core.byok_cache_preseeding.get_pricing_fetcher') as mock_get_fetcher:

            classifier_instance = Mock()
            # 5 tiers, 2 models each = 10 total models
            classifier_instance.get_tier_models.side_effect = [
                ["gpt-4o-mini", "haiku"],
            ...
            # Should count all models from all tiers: 2 models × 5 tiers = 10
    ...
    async def test_identifies_missing_models(self):
        ...
        with patch('core.byok_cache_preseeding.CognitiveClassifier') as mock_classifier, \
            ...
            # Return different models for each tier (5 tiers total)
            # First tier has 1 missing model, others have all available
            tier_index = [0]
            def get_tier_models_side_effect(tier):
                if tier_index[0] == 0:
                    tier_index[0] += 1
                    return ["gpt-4o", "missing-model"]  # First tier: 1 missing
                else:
                    return ["gpt-4o", "sonnet"]  # Other tiers: all available

            classifier_instance.get_tier_models.side_effect = get_tier_models_side_effect
            mock_classifier.return_value = classifier_instance
            ...
            # Should have 1 missing model (from first tier)
...
class TestPreseedGovernanceCache:
    ...
    @pytest.mark.asyncio
    async def test_caches_common_actions(self):
        """RED: Test that common agent actions are cached."""
        with patch('core.byok_cache_preseeding.SessionLocal') as mock_session_local, \
             patch('core.byok_cache_preseeding.get_governance_cache') as mock_get_cache:
            ...
            # Create dummy agents
            mock_agents = [
                Mock(id="agent-1", status=AgentStatus.SUPERVISED),
                Mock(id="agent-2", status=AgentStatus.AUTONOMOUS),
                Mock(id="agent-3", status=AgentStatus.INTERN),
            ]
            mock_db.query.return_value.limit.return_value.all.return_value = mock_agents
    ...
    @pytest.mark.asyncio
    async def test_caches_common_directories(self):
    ...
    @pytest.mark.asyncio
    async def test_returns_cache_stats(self):
...
class TestPreseedCacheAwareRouter:
    ...
    @pytest.mark.asyncio
    async def test_seeds_baseline_probabilities(self):
    ...
    @pytest.mark.asyncio
    async def test_establishes_fifty_percent_baseline(self):
    ...
    @pytest.mark.asyncio
    async def test_tracks_history_size(self):
        """RED: Test that history size is tracked."""
        # Mock the CacheAwareRouter to return an instance with empty history
        with patch('core.byok_cache_preseeding.CacheAwareRouter') as mock_router_class, \
             patch('core.byok_cache_preseeding.get_pricing_fetcher'):

            # Create a mock router instance
            mock_router = Mock()
            mock_router.cache_hit_history = {}  # Empty history
            mock_router_class.return_value = mock_router
            ...
            # After seeding, history should have entries (one per prompt in SAMPLE_PROMPTS)
            prompts_seeded = result.get("prompts_seeded", 0)
            assert result["cache_history_size"] == prompts_seeded
...
class TestMaybePreseedOnStartup:
    ...
    async def test_preseeds_when_enabled(self):
        ...
        with patch.dict(os.environ, {"PRESEED_CACHE_ON_STARTUP": "true"}, clear=True):
            with patch('core.byok_cache_preseeding.preseed_all_caches') as mock_preseed:
    ...
    async def test_skips_when_disabled(self):
        ...
        with patch.dict(os.environ, {"PRESEED_CACHE_ON_STARTUP": "false"}, clear=True):
            result = await maybe_preseed_on_startup()
...
class TestCacheAPIEndpoints:
    ...
    def test_preseed_endpoint_accepts_pricing(self, client, app):
        ...
        with patch('api.admin.cache_routes.preseed_pricing_cache') as mock_pricing:
            # The endpoint wraps this in a dict with "pricing" key
            # and needs to add started_at/completed_at at top level for PreseedResponse
            ...
            response = client.post(
                "/api/v1/admin/cache/preseed",
                json={"cache_type": "pricing", "workspace_id": "default"}
            )

            assert response.status_code == 200
            data = response.json()
            assert data["pricing"]["success"] is True
            assert data["pricing"]["models_loaded"] == 1523
    ...
    def test_health_endpoint_returns_status(self, client, app):
        ...
        with patch('api.admin.cache_routes.get_governance_cache') as mock_gov, \
            ...
            mock_gov_instance = Mock()
            mock_gov_instance.get_stats.return_value = {"size": 156, "hit_rate": 94.5}
            mock_gov.return_value = mock_gov_instance

            # Create pricing cache with >1000 models to get OK status
            pricing_cache = {f"model-{i}": {} for i in range(1500)}
            mock_pricing_instance = Mock()
            mock_pricing_instance.pricing_cache = pricing_cache
            mock_pricing_instance.last_fetch = datetime.now()
            mock_pricing.return_value = mock_pricing_instance
