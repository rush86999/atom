...
async def forecast(
    ...
    **Governance**: Requires SUPERVISED maturity. Forecast results should
    ...
    # Validate params before codegen (B2 — blocks code injection).
    from tools.data_analysis_tool import _validate_identifier

    id_err = _validate_identifier(target_column, "target_column")
    if id_err:
        return {"success": False, "error": id_err}
    if date_column:
        id_err = _validate_identifier(date_column, "date_column")
        if id_err:
            return {"success": False, "error": id_err}
    if isinstance(periods, bool) or not isinstance(periods, int) or not (1 <= periods <= 365):
        return {"success": False, "error": "periods must be an int between 1 and 365"}
...
try:
    # statsmodels' additive-trend ExponentialSmoothing requires >2 points
    # (B11). Guard explicitly so a short series deterministically falls back
    # to simple EMA instead of depending on a statsmodels-version-specific
    # error message.
    if len(target) < 3:
...
"""
    ...
    if result.get("success"):
        return {
            ...
            # B13: machine-readable HITL surface — results are PENDING human
            # review, not fit for autonomous downstream action.
            "governance": {
                "requires_review": True,
                "review_status": "PENDING",
                "reason": (
                    "Financial forecasting results must pass through HITL "
                    "review before deployment or use for business decisions."
                ),
            },
        }
    return result
...
async def run_model(
    ...
    **Governance**: Requires SUPERVISED maturity. Model results should be
    ...
    for col in feature_columns or []:
        id_err = _validate_identifier(col, "feature_columns")
        if id_err:
            return {"success": False, "error": id_err}
    if isinstance(test_size, bool) or not isinstance(test_size, (int, float)) or not (0.0 < test_size < 1.0):
        return {"success": False, "error": "test_size must be a number between 0 and 1"}
    ...
    if result.get("success"):
        return {
            ...
            "governance": {
                ...
                "review_status": "PENDING",
                "reason": (
                    "Model results must pass through HITL review before "
                    "deployment or use for business decisions."
                ),
            },
        }
    return result
