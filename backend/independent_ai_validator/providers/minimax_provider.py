        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                data = {
                    ...
                    "max_tokens": 1500,
                    "temperature": _clamp_temperature(0.1),
                }
                ...
                async with session.post(
                    ...
                    if response.status != 200:
                        ...
                        return LLMResponse(
                            ...
                            confidence=0.0,
                            reasoning=f"MiniMax API error: {response.status}",
                            provider=self.name,
                            model=self.model,
            ...
            return LLMResponse(
                ...
                confidence=0.0,
                reasoning="MiniMax API request timed out after 60 seconds",
                provider=self.name,
                model=self.model,
                response_time=time.time() - start_time,
            )
        except Exception as e:
            logger.error(f"MiniMax provider error: {str(e)}")
            return LLMResponse(
                content=f"Error: {str(e)}",
                confidence=0.0,
                reasoning=f"MiniMax provider encountered an error: {str(e)}",
                provider=self.name,
                model=self.model,
                response_time=time.time() - start_time,
            )
...
"""
        ...
        try:
                ...
                async with session.post(
                    ...
                    if response.status != 200:
                        ...
                        return LLMResponse(
                            ...
                            confidence=0.0,
                            reasoning=f"MiniMax API error: {error_text}",
                            provider=self.name,
                            response_time=time.time() - start_time,
                        )
            ...
            logger.error(f"MiniMax evidence analysis error: {str(e)}")
            return LLMResponse(
                content=f"Error: {str(e)}",
                confidence=0.0,
...
"""
        ...
        try:
            ...
            logger.error(f"MiniMax bias check error: {str(e)}")
            return LLMResponse(
                content=f"Error: {str(e)}",
                confidence=0.0,
    ...
    def extract_reasoning(self, content: str) -> str:
        ...
        for line in lines:
            ...
            if (
                ...
                or line.startswith("Overall Assessment:")
                or line.startswith("Recommendations:")
                or "because" in line.lower()
                or "evidence" in line.lower()
    ...
    async def health_check(self) -> bool:
        """Check if MiniMax API is accessible and working"""
        try:
            async with aiohttp.ClientSession() as session:
                headers = {
                    "Authorization": f"Bearer {self.api_key}",
