#!/usr/bin/env python3
"""
STEP 1: Build User Interface Components - Fixed
...
def create_ui_components():
    """Create all documented UI components with correct syntax"""
    ...
    for component, details in ui_components.items():
        ...
        if not os.path.exists(page_file):
            page_content = generate_page_content_fixed(component, details)
        ...
        if not os.path.exists(component_file):
            component_content = generate_component_content_fixed(component, details)
        ...
        if not os.path.exists(styles_file):
            styles_content = generate_styles_content_fixed(component)
    ...
    create_main_layout_fixed()
    create_navigation_fixed()
    
    # Create home page
    print(f"🏠 Creating home page...")
    create_home_page_fixed()
    
    # Create global styles
    print(f"🎨 Creating global styles...")
...
def generate_page_content_fixed(component, details):
    """Generate Next.js page content with fixed syntax"""
...
def generate_component_content_fixed(component, details):
    """Generate React component content with fixed syntax"""
    features_str = "', '".join(details['features'])
...
def generate_styles_content_fixed(component):
    """Generate CSS styles content with fixed syntax"""
    return f""".container {{
  max-width: 1200px;
  margin: 0 auto;
  padding: 2rem;
...
def create_main_layout_fixed():
    """Create main layout component with fixed syntax"""
...
def create_navigation_fixed():
    """Create navigation component with fixed syntax"""
...
def create_home_page_fixed():
    """Create home page with fixed syntax"""
...
def create_global_styles():
    """Create global styles"""
    styles_dir = "frontend-nextjs/styles"
    if not os.path.exists(styles_dir):
        os.makedirs(styles_dir, exist_ok=True)
    
    # Create globals.css
    globals_file = f"{styles_dir}/globals.css"
    if not os.path.exists(globals_file):
        globals_content = """/* Global Styles for ATOM Application */
...
@media (max-width: 768px) {{
  html {{
    font-size: 14px;
        ...
        print(f"   ✅ Created global styles: {globals_file}")
    
    # Create navigation.css
    nav_file = f"{styles_dir}/navigation.css"
    if not os.path.exists(nav_file):
        nav_content = """/* Navigation Styles */
...
@media (max-width: 768px) {{
  ...
  .nav-item {{
    ...
    text-align: center;
        ...
        with open(nav_file, 'w') as f:
            f.write(nav_content)
        print(f"   ✅ Created navigation styles: {nav_file}")
    
    # Create home.css
    home_file = f"{styles_dir}/home.css"
    if not os.path.exists(home_file):
        home_content = """/* Home Page Styles */
...
@media (max-width: 768px) {{
  ...
  .steps {{
    grid-template-columns: 1fr;
        ...
        with open(home_file, 'w') as f:
            f.write(home_content)
        print(f"   ✅ Created home styles: {home_file}")
...
if __name__ == "__main__":
    ...
    if success:
        ...
        print("✅ Global styles and component styles created")
