#!/usr/bin/env python3
"""
Working LanceDB Conversation Storage and Retrieval Demo

This script demonstrates how to store and retrieve conversations from LanceDB
with proper schema and functionality.
...
try:
    ...
    print("Please install: pip install lancedb numpy pyarrow")
    LANCEDB_AVAILABLE = False


class WorkingLanceDBDemo:
    """Working demo class for LanceDB conversation storage and retrieval"""
    ...
    async def initialize(self) -> bool:
        ...
        try:
            ...
            # Define conversation schema using pyarrow
            schema = pa.schema(
                [
            ...
            try:
                ...
                print(f"Creating new conversations table: {e}")
                # Create empty table with schema using empty list
                empty_data = [
                    {
                        "id": "",
                        "user_id": "",
                ...
                self.conversations_table = self.db.create_table(
                    "conversations", data=empty_data, schema=schema
                )
                # Remove the empty record after table creation
                self.conversations_table.delete("id = ''")
    ...
    async def store_sample_conversations(self) -> int:
        ...
        sample_conversations = [
            ...
            },
            {
                ...
                "session_id": "session-002",
                "role": "assistant",
                "content": "You can integrate our API by following the documentation at docs.example.com.",
                "message_type": "text",
                "timestamp": datetime.now(timezone.utc).isoformat(),
                "metadata": json.dumps({"topic": "api", "resources": "documentation"}),
                "embedding": self._generate_simple_embedding(
                    "You can integrate our API by following the documentation at docs.example.com."
                ),
            },
        ]
    ...
    def search_conversations(
        ...
        try:
            ...
            for result in results:
                ...
                if similarity_score >= 0.5:  # Lower threshold for demo
...
async def run_demo():
    ...
    print("🚀 Starting Working LanceDB Conversation Storage and Retrieval Demo")
    print("=" * 60)

    # Initialize demo
    demo = WorkingLanceDBDemo()
    ...
    print("-" * 40)
    export_file = (
        f"working_demo_conversations_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
    )
    ...
    print("   - Connect to the Atom chat interface for real-time memory")
...
if __name__ == "__main__":
    if not LANCEDB_AVAILABLE:
        ...
        print("   - PyArrow is installed: pip install pyarrow")
        print("   - NumPy is installed: pip install numpy")
        sys.exit(1)
