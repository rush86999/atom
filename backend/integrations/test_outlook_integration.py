...
from backend.integrations.outlook_service import (
    OutlookService,
    OutlookEmail,
...
from backend.integrations.outlook_enhanced_api import outlook_enhanced_bp
...
class TestOutlookEnhancedAPI:
    """Test suite for Outlook enhanced API endpoints"""

    def setup_method(self):
        """Setup test fixtures"""
        self.client = outlook_enhanced_bp.test_client()

    @patch("backend.integrations.outlook_enhanced_api.get_outlook_service")
    def test_outlook_emails_list_success(self, mock_service):
        """Test successful email listing via API"""
        mock_service_instance = Mock()
        mock_service_instance.get_user_emails = AsyncMock(
            return_value=[
                {"id": "email_123", "subject": "Test Email", "is_read": False}
            ]
        )
        mock_service.return_value = mock_service_instance

        with outlook_enhanced_bp.test_client() as client:
            ...
            assert data["data"][0]["id"] == "email_123"

    @patch("backend.integrations.outlook_enhanced_api.get_outlook_service")
    def test_outlook_emails_send_success(self, mock_service):
        """Test successful email sending via API"""
        mock_service_instance = Mock()
        mock_service_instance.send_email = AsyncMock(
            return_value={"id": "sent_123", "status": "sent"}
        )
        mock_service.return_value = mock_service_instance
        ...
        with outlook_enhanced_bp.test_client() as client:
            ...
            data = response.get_json()
            assert data["success"] is True
            assert data["operation"] == "send"
            assert data["message"] == "Email sent successfully"

    @patch("backend.integrations.outlook_enhanced_api.get_outlook_service")
    def test_outlook_emails_missing_user_id(self, mock_service):
        """Test email API with missing user_id"""
        with outlook_enhanced_bp.test_client() as client:
            response = client.post(
                "/api/integrations/outlook/emails", json={"operation": "list"}
            )

            assert response.status_code == 400
            data = response.get_json()
            assert data["success"] is False
            assert "Missing required parameter: user_id" in data["error"]

    @patch("backend.integrations.outlook_enhanced_api.get_outlook_service")
    def test_outlook_emails_invalid_operation(self, mock_service):
        """Test email API with invalid operation"""
        with outlook_enhanced_bp.test_client() as client:
            response = client.post(
                "/api/integrations/outlook/emails",
                json={"user_id": "test_user", "operation": "invalid_operation"},
            )
    ...
    @patch("backend.integrations.outlook_enhanced_api.get_outlook_service")
    def test_outlook_calendar_events_list_success(self, mock_service):
        """Test successful calendar events listing via API"""
        mock_service_instance = Mock()
        mock_service_instance.get_calendar_events = AsyncMock(
            return_value=[{"id": "event_123", "subject": "Team Meeting"}]
        )
        mock_service.return_value = mock_service_instance
        ...
        with outlook_enhanced_bp.test_client() as client:
            ...
            assert data["operation"] == "list"
            assert len(data["data"]) == 1
            assert data["data"][0]["id"] == "event_123"

    @patch("backend.integrations.outlook_enhanced_api.get_outlook_service")
    def test_outlook_contacts_list_success(self, mock_service):
        """Test successful contacts listing via API"""
        mock_service_instance = Mock()
        mock_service_instance.get_user_contacts = AsyncMock(
            return_value=[{"id": "contact_123", "display_name": "John Doe"}]
        )
        mock_service.return_value = mock_service_instance

        with outlook_enhanced_bp.test_client() as client:
            response = client.post(
                "/api/integrations/outlook/contacts",
                json={
                    "user_id": "test_user",
                    "operation": "list",
                    "query": "John",
            ...
            assert data["operation"] == "list"
            assert len(data["data"]) == 1
            assert data["data"][0]["display_name"] == "John Doe"

    @patch("backend.integrations.outlook_enhanced_api.get_outlook_service")
    def test_outlook_tasks_list_success(self, mock_service):
        """Test successful tasks listing via API"""
        mock_service_instance = Mock()
        mock_service_instance.get_user_tasks = AsyncMock(
            return_value=[{"id": "task_123", "subject": "Complete documentation"}]
        )
        mock_service.return_value = mock_service_instance

        with outlook_enhanced_bp.test_client() as client:
            response = client.post(
                "/api/integrations/outlook/tasks",
                json={
                    "user_id": "test_user",
                    "operation": "list",
                    "status": "notStarted",
            ...
            assert data["operation"] == "list"
            assert len(data["data"]) == 1
            assert data["data"][0]["subject"] == "Complete documentation"

    @patch("backend.integrations.outlook_enhanced_api.get_outlook_service")
    def test_outlook_search_success(self, mock_service):
        """Test successful search via API"""
        mock_service_instance = Mock()
        mock_service_instance.search_emails = AsyncMock(
            return_value=[{"id": "email_123", "subject": "Search Result"}]
        )
        mock_service.return_value = mock_service_instance

        with outlook_enhanced_bp.test_client() as client:
            response = client.post(
                "/api/integrations/outlook/search",
                json={"user_id": "test_user", "query": "important", "max_results": 50},
            )
            ...
            assert data["success"] is True
            assert len(data["data"]) == 1
            assert data["data"][0]["subject"] == "Search Result"

    @patch("backend.integrations.outlook_enhanced_api.get_outlook_service")
    def test_outlook_service_not_available(self, mock_service):
        """Test API when Outlook service is not available"""
        mock_service.return_value = None

        with outlook_enhanced_bp.test_client() as client:
            response = client.post(
                "/api/integrations/outlook/emails",
                json={"user_id": "test_user", "operation": "list"},
            )

            assert response.status_code == 503
            data = response.get_json()
            assert data["success"] is False
            assert "Outlook service not available" in data["error"]

    def test_handle_service_error_decorator(self):
        """Test service error handling decorator"""
        from backend.integrations.outlook_enhanced_api import handle_service_error
        ...
        with outlook_enhanced_bp.test_client() as client:
            # This would be called internally by the decorator
            # We're testing that the decorator properly handles exceptions
            result = failing_function()
            assert result is not None
            assert result[1] == 500  # Status code

    def test_validate_user_id_decorator(self):
        """Test user_id validation decorator"""
        from backend.integrations.outlook_enhanced_api import validate_user_id
        ...
        with outlook_enhanced_bp.test_client() as client:
            ...
            with outlook_enhanced_bp.test_request_context(json={}):
