...
try:
    ...
    from db_oauth_linear import delete_tokens, get_tokens, save_tokens
...
async def list_issues(request: IssueRequest):
    ...
    try:
        ...
        return {
            "ok": True,
            "data": {"issues": issues_data, "total_count": len(issues_data)},
...
async def create_issue(request: CreateIssueRequest):
    ...
    try:
        ...
        result = await linear_service.search_linear(
            request.user_id, request.title, "issues", 1
        )
...
async def list_teams(request: TeamRequest):
    ...
    try:
        ...
        return {
            "ok": True,
            "data": {"teams": teams_data, "total_count": len(teams_data)},
            "endpoint": "list_teams",
            "timestamp": datetime.utcnow().isoformat(),
            "source": "linear_api",
        }
...
async def list_projects(request: ProjectRequest):
    ...
    try:
        ...
        projects = await linear_service.get_team_projects(
            request.user_id, request.team_id, request.limit
        )
        ...
        return {
            "ok": True,
            "data": {"projects": projects_data, "total_count": len(projects_data)},
...
async def list_cycles(request: CycleRequest):
    ...
    try:
        ...
        return {
            "ok": True,
            "data": {"cycles": cycles_data, "total_count": len(cycles_data)},
...
async def get_user_profile(request: UserRequest):
    ...
    try:
        ...
        return {
            ...
            "data": {
                ...
                "organization": {
                    ...
                    "name": tokens["user_info"].get("organization", {}).get("name")
        ...
        raise HTTPException(
            status_code=500, detail=f"Error getting user profile: {str(e)}"
        )
