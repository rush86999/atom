...
def list_salesforce_accounts():
    ...
    try:
        ...
        # Call service
        result = asyncio.run(
            salesforce_service.list_accounts(
                user_id=user_id,
                username=username,
                query=query,
                fields=fields,
...
def create_salesforce_account():
    ...
    try:
        ...
        # Call service
        result = asyncio.run(
            salesforce_service.create_account(
                user_id=user_id, username=username, account_data=account_data
            )
        )
...
def get_salesforce_account():
    ...
    try:
        ...
        # Call service
        result = asyncio.run(
            salesforce_service.get_account(
                user_id=user_id, account_id=account_id, username=username, fields=fields
            )
        )
...
def list_salesforce_contacts():
    ...
    try:
        ...
        # Call service
        result = asyncio.run(
            salesforce_service.list_contacts(
                user_id=user_id,
                username=username,
...
def list_salesforce_opportunities():
    ...
    try:
        ...
        # Call service
        result = asyncio.run(
            salesforce_service.list_opportunities(
                user_id=user_id,
                username=username,
                account_id=account_id,
                stage=stage,
...
def get_sales_pipeline_analytics():
    ...
    try:
        ...
        for stage in stages:
            stage_opps = [
                opp for opp in all_opportunities if opp.get("StageName") == stage
            ]
        ...
        current_quarter_opps = [
            opp
            for opp in all_opportunities
            if datetime.fromisoformat(opp.get("CloseDate", "").replace("Z", "+00:00"))
            >= current_quarter_start
        ]

        current_quarter_value = sum(
            opp.get("Amount", 0) for opp in current_quarter_opps
        )
    ...
    try:
        ...
        for status in statuses:
            ...
            status_analytics[status] = {
                "count": status_count,
                "percentage": (status_count / total_leads * 100)
                if total_leads > 0
                else 0,
        ...
        for source, data in source_analytics.items():
            conversion_rate = (
                (data["converted"] / data["count"] * 100) if data["count"] > 0 else 0
            )
...
def execute_soql_query():
    ...
    try:
        ...
        # Call service
        result = asyncio.run(
            salesforce_service.execute_soql_query(
                user_id=user_id, username=username, soql_query=soql_query, limit=limit
            )
        )
...
def get_salesforce_user_info():
    ...
    try:
        ...
        # Call service
        result = asyncio.run(
            salesforce_service.get_user_info(user_id=user_id, username=username)
        )
    ...
    try:
        status = {
            ...
            "endpoints": {
                ...
                "utilities": {"query": "/query", "user_info": "/user/info"},
            },
            "support": {
