...
files_modified:
  ...
  - backend/tests/bug_discovery/storage/reports/.gitkeep
autonomous: true

must_haves:
  truths:
    - "DiscoveryCoordinator.run_full_discovery() orchestrates all discovery methods"
    - "Fuzzing campaigns run via FuzzingOrchestrator.start_campaign()"
    - "Chaos experiments run via ChaosCoordinator.run_experiment()"
    - "Property tests run via subprocess pytest execution"
    - "Browser discovery runs via ExplorationAgent.explore_dfs()"
    - "All results aggregated, deduplicated, classified, and filed as GitHub issues"
  artifacts:
    - path: "backend/tests/bug_discovery/core/discovery_coordinator.py"
      provides: "DiscoveryCoordinator orchestration service"
      exports: ["DiscoveryCoordinator", "run_full_discovery"]
    - path: "backend/tests/bug_discovery/core/__init__.py"
      provides: "Unified imports for all core services"
      exports: ["DiscoveryCoordinator", "ResultAggregator", "BugDeduplicator", "SeverityClassifier", "DashboardGenerator"]
  key_links:
    - from: "backend/tests/bug_discovery/core/discovery_coordinator.py"
      to: "backend/tests/fuzzing/campaigns/fuzzing_orchestrator.py"
      via: "FuzzingOrchestrator import"
      pattern: "from tests.fuzzing.campaigns.fuzzing_orchestrator import FuzzingOrchestrator"
    - from: "backend/tests/bug_discovery/core/discovery_coordinator.py"
      to: "backend/tests/chaos/core/chaos_coordinator.py"
      via: "ChaosCoordinator import"
    ...
    - from: "backend/tests/bug_discovery/core/discovery_coordinator.py"
      ...
      via: "BugFilingService import"
      pattern: "from tests.bug_discovery.bug_filing_service import BugFilingService"
    - from: "backend/tests/bug_discovery/core/discovery_coordinator.py"
      to: "backend/tests/bug_discovery/core/result_aggregator.py"
      via: "ResultAggregator import"
      pattern: "from tests.bug_discovery.core.result_aggregator import ResultAggregator"
---

<objective>
Create the DiscoveryCoordinator orchestration service that runs all discovery methods, aggregates their results, deduplicates bugs, classifies severity, files unique bugs via BugFilingService, and generates weekly dashboard reports.

Purpose: DiscoveryCoordinator is the thin orchestration layer that ties together all discovery methods (fuzzing, chaos, property tests, browser discovery) into a unified pipeline. This service is what the weekly CI pipeline calls to run the complete bug discovery workflow.

Output: DiscoveryCoordinator service with run_full_discovery() method that orchestrates the complete bug discovery pipeline and returns aggregated statistics with report path.
</objective>
...
# Phase 242-01 Outputs (must exist before this plan)
@backend/tests/bug_discovery/core/result_aggregator.py - ResultAggregator with aggregate_fuzzing_results(), aggregate_chaos_results(), aggregate_property_results(), aggregate_browser_results()
@backend/tests/bug_discovery/core/bug_deduplicator.py - BugDeduplicator with deduplicate_bugs()
@backend/tests/bug_discovery/core/severity_classifier.py - SeverityClassifier with classify(), batch_classify()
...
@backend/tests/fuzzing/campaigns/fuzzing_orchestrator.py - FuzzingOrchestrator.start_campaign(), monitor_campaign(), stop_campaign()
@backend/tests/chaos/core/chaos_coordinator.py - ChaosCoordinator.run_experiment() with blast_radius checks
@backend/tests/browser_discovery/conftest.py - ExplorationAgent with explore_dfs(), explore_bfs(), explore_random()
@backend/tests/bug_discovery/bug_filing_service.py - BugFilingService.file_bug() with idempotency

# CI Pipeline Context
@.github/workflows/bug-discovery-weekly.yml - Weekly CI pipeline that will call DiscoveryCoordinator
</context>

<tasks>

<task type="auto">
  <name>Task 1: Create DiscoveryCoordinator orchestration service</name>
  <files>backend/tests/bug_discovery/core/discovery_coordinator.py</files>
  <action>
Create DiscoveryCoordinator service at backend/tests/bug_discovery/core/discovery_coordinator.py:
...
```
  ...
  <verify>
Run: python -c "from backend.tests.bug_discovery.core.discovery_coordinator import DiscoveryCoordinator; print('DiscoveryCoordinator imported successfully')"
</verify>
  <done>
DiscoveryCoordinator service created with run_full_discovery() orchestration method, _run_fuzzing_discovery() (uses FuzzingOrchestrator), _run_chaos_discovery() (uses ChaosCoordinator with blast radius checks), _run_property_discovery() (subprocess pytest), _run_browser_discovery() (Playwright headless), _file_bugs() (uses BugFilingService), and run_discovery() convenience function.
</done>
</task>

<task type="auto">
  <name>Task 2: Update core/__init__.py and create storage directory</name>
  <files>backend/tests/bug_discovery/core/__init__.py, backend/tests/bug_discovery/storage/reports/.gitkeep</files>
  <action>
Update backend/tests/bug_discovery/core/__init__.py to export all services:
...
```bash
mkdir -p backend/tests/bug_discovery/storage/reports
touch backend/tests/bug_discovery/storage/reports/.gitkeep
```
</action>
  <verify>
Run: python -c "from backend.tests.bug_discovery.core import DiscoveryCoordinator, ResultAggregator, BugDeduplicator, SeverityClassifier, DashboardGenerator; print('All core services exported successfully')"
</verify>
  <done>
core/__init__.py exports all five services (DiscoveryCoordinator, ResultAggregator, BugDeduplicator, SeverityClassifier, DashboardGenerator) and run_discovery() convenience function. Storage directory created with reports/ subdirectory for weekly HTML/JSON reports.
</done>
</task>
...
2. run_full_discovery() method orchestrates all four discovery methods
3. _run_fuzzing_discovery() uses FuzzingOrchestrator.run_campaign_with_bug_filing()
4. _run_chaos_discovery() uses ChaosCoordinator.run_experiment() with blast_radius_checks
5. _run_property_discovery() runs subprocess pytest and parses output
6. _run_browser_discovery() uses Playwright headless browser
7. _file_bugs() calls BugFilingService.file_bug() for each unique bug
8. DashboardGenerator.generate_weekly_report() creates HTML report
9. core/__init__.py exports all services for clean imports
10. storage/reports/ directory exists for report output
</verification>

<success_criteria>
1. DiscoveryCoordinator can be imported: from backend.tests.bug_discovery.core import DiscoveryCoordinator
2. DiscoveryCoordinator.run_full_discovery() executes without errors (may skip methods if dependencies unavailable)
3. All four discovery methods are called in sequence
4. Results are aggregated, deduplicated, and classified
5. Weekly report is generated in storage/reports/
</success_criteria>

<output>
After completion, create `.planning/phases/242-unified-bug-discovery-pipeline/242-02-SUMMARY.md` with:
- DiscoveryCoordinator service methods and orchestration flow
- Integration with FuzzingOrchestrator, ChaosCoordinator, BugFilingService
- Subprocess execution for property tests
- Playwright headless browser discovery
