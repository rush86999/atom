...
must_haves:
  truths:
    ...
    - "Performance benchmarks documented with latency targets and alert thresholds"
  artifacts:
    ...
    - path: "backend/docs/MONITORING_SETUP.md"
      provides: "Monitoring documentation including performance benchmarks"
...
<task type="auto">
  <name>Task 3: Create monitoring tests and documentation with performance benchmarks</name>
  ...
  <action>
    ...
    2. Create backend/docs/MONITORING_SETUP.md with:
       ...
       - **Performance benchmarks section** with:
         - Health check latency targets: /health/live <10ms, /health/ready <100ms
         - Metrics scrape targets: /health/metrics <50ms for full scrape
         - Baseline measurements from test environment
         - Alert thresholds for monitoring (e.g., p95 latency >200ms)
  ...
  <verify>
    ...
    Command: `grep -A5 "Performance Benchmarks" backend/docs/MONITORING_SETUP.md | wc -l`
    Expected: Performance benchmarks section exists with targets documented
  </verify>
  <done>
    Health routes tested, monitoring documentation created with performance benchmarks, SUMMARY.md complete
  </done>
</task>
...
6. MONITORING_SETUP.md documents setup, usage, and performance benchmarks
7. Performance benchmarks section includes latency targets (<10ms live, <100ms ready, <50ms metrics scrape)
</success_criteria>
