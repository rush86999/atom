...
must_haves:
  truths:
    - "AgentEvolutionLoop coverage increases from 18.8% to 70%+"
    - "run_evolution_cycle main path tested (lines 129-235)"
    - "Parent selection novelty calculation tested (line 291)"
    - "Directive application to clone tested (lines 375-433)"
    - "Evaluation and promotion paths tested (lines 464-526)"
    - "Trace recording tested (lines 528-591)"
  artifacts:
    - path: "backend/tests/core/test_agent_evolution_loop_coverage.py"
      provides: "Coverage-driven tests for evolution loop"
      min_lines: 300
      contains: "class TestEvolutionCycleCoverage"
  key_links:
    - from: "test_agent_evolution_loop_coverage.py"
      to: "core/agent_evolution_loop.py"
      via: "import AgentEvolutionLoop"
      pattern: "from core.agent_evolution_loop import AgentEvolutionLoop"
    - from: "test_agent_evolution_loop_coverage.py"
      to: "core/agent_evolution_loop.py:run_evolution_cycle"
      via: "method call coverage"
      pattern: "loop\\.run_evolution_cycle\\("
---

<objective>
Add coverage-driven tests for AgentEvolutionLoop to raise coverage from 18.8% to 70%+.

Purpose: AgentEvolutionLoop implements the Group-Evolving Agents (GEA) evolution cycle. Only 36/191 lines are currently covered. Focus on testing the happy paths through each major method: parent selection, directive application, evaluation, promotion, and trace recording.

Output: 300+ lines of new tests, 70%+ coverage on agent_evolution_loop.py
</objective>
...
<task type="auto">
  <name>Create EvolutionCycleResult tests</name>
  <files>backend/tests/core/test_agent_evolution_loop_coverage.py</files>
  <action>
Create new test file `backend/tests/core/test_agent_evolution_loop_coverage.py` with tests for EvolutionCycleResult class:

```python
"""
Coverage-driven tests for AgentEvolutionLoop (currently 18.8% -> target 70%+)
...
Run to verify:
```bash
cd backend && pytest tests/core/test_agent_evolution_loop_coverage.py::TestEvolutionCycleResult -v
```
  </action>
  <verify>
```bash
cd /Users/rushiparikh/projects/atom/backend && \
pytest tests/core/test_agent_evolution_loop_coverage.py::TestEvolutionCycleResult -v --tb=short
```
  </verify>
  <done>EvolutionCycleResult tests pass, cover __init__ and to_dict methods</done>
</task>

<task type="auto">
  <name>Create run_evolution_cycle tests</name>
  <files>backend/tests/core/test_agent_evolution_loop_coverage.py</files>
  <action>
Add tests for run_evolution_cycle method (lines 108-244), covering the full evolution cycle flow:

```python
# Add to test_agent_evolution_loop_coverage.py
...
Run to verify:
```bash
cd backend && pytest tests/core/test_agent_evolution_loop_coverage.py::TestRunEvolutionCycle -v
```
  </action>
  <verify>
```bash
cd /Users/rushiparikh/projects/atom/backend && \
pytest tests/core/test_agent_evolution_loop_coverage.py::TestRunEvolutionCycle -v --tb=short
```
  </verify>
  <done>run_evolution_cycle tests cover main flow, guardrail block, and success paths</done>
</task>

<task type="auto">
  <name>Create parent selection and directive tests</name>
  <files>backend/tests/core/test_agent_evolution_loop_coverage.py</files>
  <action>
Add tests for select_parent_group (lines 246-299) and _apply_directives_to_clone (lines 359-433):
...
Run to verify:
```bash
cd backend && pytest tests/core/test_agent_evolution_loop_coverage.py::TestSelectParentGroup tests/core/test_agent_evolution_loop_coverage.py::TestApplyDirectivesToClone -v
```
  </action>
  <verify>
```bash
cd /Users/rushiparikh/projects/atom/backend && \
pytest tests/core/test_agent_evolution_loop_coverage.py::TestSelectParentGroup tests/core/test_agent_evolution_loop_coverage.py::TestApplyDirectivesToClone -v --tb=short
```
  </verify>
  <done>Parent selection covers novelty calculation; Directive application covers clone and skill creation paths</done>
</task>

<task type="auto">
  <name>Create evaluation and trace recording tests</name>
  <files>backend/tests/core/test_agent_evolution_loop_coverage.py</files>
  <action>
Add tests for _evaluate_evolved_config, _promote_evolved_config, and _record_trace methods:
...
class TestEvaluateEvolvedConfig:
    ...
    async def test_evaluate_with_graduation_service(self, db_session):
        ...
        # Mock GraduationExamService
        with patch('core.agent_evolution_loop.GraduationExamService') as mock_exam_class:
            mock_exam = MagicMock()
    ...
    async def test_evaluate_fallback_proxy(self, db_session):
        ...
        with patch('core.agent_evolution_loop.GraduationExamService', side_effect=ImportError):
            score, passed = await loop._evaluate_evolved_config(
...
class TestRecordTrace:
    ...
    def test_record_trace_creates_evolution_trace(self, db_session):
        ...
        assert trace.ancestor_count == 2  # Unique parents
...
```bash
cd backend && pytest tests/core/test_agent_evolution_loop_coverage.py -v --cov=core.agent_evolution_loop --cov-report=term-missing
```
  </action>
  <verify>
```bash
cd /Users/rushiparikh/projects/atom/backend && \
pytest tests/core/test_agent_evolution_loop_coverage.py -v --cov=core.agent_evolution_loop --cov-report=term-missing --cov-branch | grep -E "(agent_evolution_loop|TOTAL)"
```
  </verify>
  <done>agent_evolution_loop.py coverage increases from 18.8% to 70%+, all major methods tested</done>
</task>

</tasks>

<verification>
Run: `cd backend && pytest tests/core/test_agent_evolution_loop_coverage.py -v --cov=core.agent_evolution_loop --cov-branch --cov-report=term`

Checks:
- [ ] test_agent_evolution_loop_coverage.py has 300+ lines
- [ ] All tests pass (no failures)
- [ ] agent_evolution_loop.py coverage >= 70%
- [ ] run_evolution_cycle method tested
- [ ] select_parent_group with novelty tested
- [ ] _apply_directives_to_clone tested
- [ ] _evaluate_evolved_config tested
- [ ] _promote_evolved_config tested
- [ ] _record_trace tested
</verification>

<success_criteria>
1. New test file test_agent_evolution_loop_coverage.py created with 300+ lines
2. Coverage for agent_evolution_loop.py increases from 18.8% to 70%+
3. All 7 major method groups tested (result, cycle, selection, directives, evaluation, promotion, trace)
4. Tests pass with no mock-related failures
5. Branch coverage considered (--cov-branch in verify)
</success_criteria>

<output>
After completion, create `.planning/phases/188-coverage-gap-closure/188-02-SUMMARY.md` with:
- Final coverage percentage for agent_evolution_loop.py
- Number of new tests added
- Missing lines still not covered (for future reference)
</output>
