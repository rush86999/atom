...
**Key Decision**: Implement automatic rollback on smoke test failure (no manual approval). Use progressive canary deployment (10% → 50% → 100% traffic) to minimize blast radius.
    ...
    ```yaml
      - name: Run smoke tests
        ...
        run: |
          ...
          curl -f "$STAGING_URL/health/live" || { echo "❌ Liveness probe failed"; exit 1; }
          echo "✅ Liveness probe passed"

          curl -f "$STAGING_URL/health/ready" || { echo "❌ Readiness probe failed"; exit 1; }
          echo "✅ Readiness probe passed"

          # Test database connectivity
          curl -f "$STAGING_URL/health/db" || { echo "❌ Database connectivity check failed"; exit 1; }
          ...
          if echo "$AGENT_RESPONSE" | jq -e '.error' > /dev/null; then
            echo "❌ Agent execution failed: $AGENT_RESPONSE"
            exit 1
          fi
          echo "✅ Agent execution endpoint passed"
          ...
          if echo "$CANVAS_RESPONSE" | jq -e '.error' > /dev/null; then
            echo "❌ Canvas presentation failed: $CANVAS_RESPONSE"
            exit 1
          fi
          echo "✅ Canvas presentation endpoint passed"
        ...
        with:
          script: |
            const issue = await github.rest.issues.create({
              ...
              title: '🚨 Deployment Rollback - Smoke Test Failed (Staging)',
              body: `
      ...
      - name: Monitor deployment metrics
        run: |
          ...
          if [ -z "$PROMETHEUS_URL" ]; then
            echo "⚠️ PROMETHEUS_URL not set, skipping metrics check"
            exit 0
          fi
          ...
          if (( $(echo "$ERROR_RATE > $STAGING_THRESHOLD" | bc -l) )); then
            echo "❌ Error rate too high: $ERROR_RATE% (threshold: $STAGING_THRESHOLD%)"

            # Send alert to Slack
            curl -X POST "${{ secrets.SLACK_WEBHOOK_URL }}" \
              -H 'Content-Type: application/json' \
              -d "{
                \"text\": \"⚠️ Staging deployment error rate high\",
                \"blocks\": [{
                  \"type\": \"section\",
                  \"text\": {
                    \"type\": \"mrkdwn\",
            ...
            echo "⚠️ Warning: Error rate exceeds threshold but deployment continues"
          else
            echo "✅ Error rate within acceptable range: $ERROR_RATE% (threshold: $STAGING_THRESHOLD%)"
          fi
          ...
          if (( $(echo "$LATENCY_MS > $LATENCY_THRESHOLD" | bc -l) )); then
            echo "⚠️ Latency high: ${LATENCY_MS}ms (threshold: ${LATENCY_THRESHOLD}ms)"
          else
            echo "✅ Latency within acceptable range: ${LATENCY_MS}ms (threshold: ${LATENCY_THRESHOLD}ms)"
          fi
    ...
    steps:
      ...
      - name: Automatic rollback on production failure
        ...
        run: |
          ...
          # But sends urgent Slack notification: 🚨 PRODUCTION ROLLBACK 🚨
    ```
...
<task type="checkpoint:human-verify">
  ...
  <how-to-verify>
    1. **Test Smoke Test Authentication**:
           ...
           # Check smoke test output for "✅ Authentication successful"
