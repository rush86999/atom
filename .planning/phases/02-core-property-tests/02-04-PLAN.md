...
must_haves:
  truths:
    - Required field validation returns 400 with field names (missing fields are rejected)
    - Paginated responses include consistent metadata (total_pages, has_next, has_prev calculable from inputs)
    - Error responses (4xx/5xx) include error_code and message fields
