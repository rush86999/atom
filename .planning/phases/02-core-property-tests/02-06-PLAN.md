...
must_haves:
  truths:
    - Events process in chronological order (timestamp-sorted regardless of arrival order)
    - Event batching handles remainders correctly (last batch may be partial but never empty)
    - Events exceeding max_retries move to DLQ permanently (retry_count >= max_retries = no more retries)
