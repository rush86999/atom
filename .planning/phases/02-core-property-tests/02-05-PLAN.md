...
must_haves:
  truths:
    - State updates merge correctly (original keys preserved, new keys added, overlapping keys updated)
    - Failed updates rollback completely (original state preserved via deep copy)
    - State sync conflicts resolve per strategy (local_wins, remote_wins, merge, error)
