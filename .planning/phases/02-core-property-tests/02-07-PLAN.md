...
must_haves:
  truths:
    - Path traversal attacks (../, %2e%2e) are blocked before file access
    - Unix permission checks validate against bit masks (Read=4, Write=2, Execute=1)
    - File uploads respect size limits (files exceeding max rejected before processing)
