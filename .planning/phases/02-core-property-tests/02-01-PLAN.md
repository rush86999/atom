...
must_haves:
  truths:
    - Governance maturity levels prevent privilege escalation (STUDENT cannot execute AUTONOMOUS-only actions)
    - Confidence scores never exceed maturity bounds (INTERN agents stay in 0.5-0.7 range)
    - Cache returns consistent governance decisions within TTL (same input = same output)
