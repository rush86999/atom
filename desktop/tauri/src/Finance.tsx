...
// Types matching backend and hook structures
interface Account {
  id: string;
  name: string;
            ...
            <Grid item xs={12} md={6} key={budget.category}>
              <Box>
                <Box display="flex" justifyContent="space-between" alignItems="center">
                  ...
                  <Badge color={budget.utilization > 100 ? "error" : "success"} badgeContent={budget.utilization > 100 ? "OVER" : `${budget.utilization.toFixed(0)}%`} />
                </Box>
                <LinearProgress
                  variant="determinate"
            ...
            <CardContent>
              ...
              <ResponsiveContainer width="100%" height={300}>
                <PieChart>
                  <Pie
                    data={[
                      { name: "Assets", value: calculateTotalAssets() },
                      { name: "Liabilities", value: calculateTotalLiabilities() }
                    ]}
                    cx="50%"
                    cy="50%"
                    outerRadius={100}
