...
pub async fn get_outlook_oauth_url(
    ...
    let client_id =
        std::env::var("OUTLOOK_CLIENT_ID").unwrap_or_else(|_| "your_outlook_client_id".to_string());
    let redirect_uri = std::env::var("OUTLOOK_REDIRECT_URI")
        .unwrap_or_else(|_| "http://localhost:3000/oauth/outlook/callback".to_string());
    let tenant_id = std::env::var("OUTLOOK_TENANT_ID").unwrap_or_else(|_| "common".to_string());
...
pub async fn exchange_outlook_oauth_code(
    ...
    let token_response = client
        ...
        .form(&[
            ...
            ("scope", &"https://graph.microsoft.com/Mail.Read https://graph.microsoft.com/Mail.Send https://graph.microsoft.com/Calendars.Read https://graph.microsoft.com/User.Read offline_access".to_string()),
        ])
        .send()
        .await
...
pub async fn get_outlook_connection(
    ...
    Ok(HashMap::from([
        ...
        (
            "error".to_string(),
            serde_json::Value::String("No valid tokens found".to_string()),
        ),
    ]))
}
...
pub async fn get_outlook_emails(
    ...
    let emails = vec![
        std::collections::HashMap::from([
            (
                "id".to_string(),
                serde_json::Value::String("mock_email_1".to_string()),
            ),
            (
                "subject".to_string(),
                serde_json::Value::String("Welcome to ATOM".to_string()),
            ),
            (
                "from".to_string(),
                serde_json::json!({"name": "ATOM Team", "address": "team@atom.ai"}),
            ),
            (
                "body".to_string(),
                serde_json::Value::String(
                    "Thank you for integrating Outlook with ATOM!".to_string(),
                ),
            ),
            (
                "receivedDateTime".to_string(),
                serde_json::Value::String("2025-11-02T10:00:00Z".to_string()),
            ),
        ...
        std::collections::HashMap::from([
            (
                "id".to_string(),
                serde_json::Value::String("mock_email_2".to_string()),
            ),
            (
                "subject".to_string(),
                serde_json::Value::String("Integration Complete".to_string()),
            ),
            (
                "from".to_string(),
                serde_json::json!({"name": "System", "address": "noreply@outlook.com"}),
            ),
            (
                "body".to_string(),
                serde_json::Value::String("Your Outlook integration is ready to use.".to_string()),
            ),
            (
                "receivedDateTime".to_string(),
                serde_json::Value::String("2025-11-02T09:30:00Z".to_string()),
            ),
...
pub async fn get_outlook_calendar_events(
    ...
    // Mock data for now
    let events = vec![std::collections::HashMap::from([
        (
            "id".to_string(),
            serde_json::Value::String("mock_event_1".to_string()),
        ),
        (
            "subject".to_string(),
            serde_json::Value::String("Team Meeting".to_string()),
        ),
        (
            "start".to_string(),
            serde_json::json!({
        ...
        (
            "location".to_string(),
            serde_json::Value::String("Conference Room".to_string()),
        ),
        (
            "body".to_string(),
            serde_json::Value::String("Weekly team sync meeting".to_string()),
        ),
    ])];
...
pub async fn create_outlook_calendar_event(
    ...
    response.insert(
        "message".to_string(),
        "Calendar event created successfully".to_string(),
    );
...
pub async fn disconnect_outlook(
    ...
    response.insert(
        "message".to_string(),
        "Outlook disconnected successfully".to_string(),
    );
...
pub async fn check_outlook_tokens(
    ...
    Ok(HashMap::from([
        ...
        (
            "message".to_string(),
            serde_json::Value::String("No tokens found".to_string()),
        ),
    ]))
}
