...
64. **Work-Time Lesson Application (Aug 31, 2026)** — closes the loop: lessons taught via `POST /api/agents/{id}/teach` + observed `human_correction` events (stored on `AgentRegistry.configuration["learning"]["log"]`) were write-only at work time (score nudge only). NEW `core/student_learning_service.py` accessors — `get_agent_lessons(db, agent_id, query, limit=5)` (fault-isolated, newest-first, keyword-relevance; works at ANY maturity tier — lessons survive graduation) + `format_lessons_block()` (bounded ≤5 lessons/~1.6k chars, "PERMANENT INSTRUCTIONS" framing). Injected at ALL THREE work-time surfaces: (1) `core/memory_context_assembler.py` `_lessons_leg` — FIRST block of RELEVANT MEMORY on every chat surface; (2) `core/generic_agent.py` `_execute_impl`→`memory_context["lessons"]`→ReAct MEMORY CONTEXT — every task execution; (3) `core/chat_canvas_editor.py` `plan_canvas_edit(lessons=)` via `ChatOrchestrator._agent_lessons` — canvas co-editor edits for every canvas app/type (NOT the send/action planner — sends stay verbatim by design). Research-grounded: Letta/MemGPT memory-blocks pattern (bounded always-in-context block vs archival recall) + CLAUDE.md-style imperative/bounded guidance; poisoning caveat → lesson writes stay human-gated. Tests: `tests/core/test_student_learning_service.py` (+9), `tests/core/test_generic_agent_worktime_lessons.py` (4), assembler/canvas-editor suites. See `docs/canvas/agent-learning.md` + `docs/architecture/AGENT_MEMORY_UNIFICATION_PLAN.md` addendum.

65. **Canvas WS Frame Contract + Audit-Authoritative Reads + Direct-Load Rescue (Aug 31–Sep 1, 2026)** — three fixes from the live "draft went missing" incident. (a) **Frame contract**: `canvas:update` frames declare semantics via `action`; event actions (`email_send`, `mini_app_state`) carry STATUS payloads and must never render as canvas content (a failed send replaced the drafted email in the open canvas). Enforced by `frontend-nextjs/lib/canvasFrame.ts` `isCanvasContentFrame` in `pages/canvas/[id].tsx` + `components/chat/canvas-host.tsx` — new event broadcasts must register in `CANVAS_EVENT_ACTIONS`. (b) **`read_canvas` scans back** to the latest CONTENT-bearing `CanvasAudit` row before any legacy `canvases.content` fallback — send-attempt rows (`email_send_attempt`, no content key) previously made reads serve the creation-era snapshot (draft "vanished" after every send). Tests: `tests/test_canvas_crud.py::TestReadCanvasAfterEventRows`. (c) **Direct-load rescue** (`pages/_app.tsx`, module scope, once per document): Next 16.2.2 dev boots direct URL loads with the router stuck `isReady:false` forever (silent — no console errors; affects /dashboard too; known upstream family vercel/next.js#35754/#35990) — after a 5s grace, `router.replace(location)` re-resolves and boots the page (no-op when healthy). Companion: `canvasId` falls back to `window.location.pathname` (mount-gated) + `suppressHydrationWarning` h1 (URL-derived text legitimately differs from the SSR shell — element-level suppression per Next.js guidance, NOT root-level). ⚠️ Turbopack dev can serve STALE page chunks across restarts — if an edit "doesn't take", verify the SERVED chunk and `rm -rf .next`. Dev diagnostic: `window.__atomErrors` error tap in `_app.tsx`. See `docs/canvas/README.md` frame-contract section.

66. **Image OCR for Attachment Ingestion (Sep 2026)** — `core/image_ocr.py`: email attachments that are IMAGES were nominally "ingestible" (`_BINARY_ATTACHMENT_EXTENSIONS` listed png/jpg/tiff) but had no working OCR — the only text layer came from **Docling**, an *optional* dependency (`requirements-docling.txt`, absent from the default image), so every image parsed to `""` and was dropped as `skipped: no_text` (never indexed). Fix: a general ladder — Docling first (existing, in `DocumentParser`), then **Tesseract** (pytesseract when importable, else the `tesseract` CLI directly: no Python dep; binary now provisioned in the Docker runner), then a **vision-LLM fallback** (a vision-capable BYOK model via `LLMService.generate_completion`, so hosts with no local engine still work). Wired at the general layer — `DocumentParser.parse_document(..., image_min_chars=)` → `AutoDocumentIngestionService.process_file_bytes` → `core.email_attachment_ingestion` — so cloud-drive syncs, uploads and the on-demand `email_attachment_get_text` read path benefit too; `.tif`/`.bmp` were also missing from the Docling format list. **Inline body images** are now OCR'd as well (`_ingest_binary_attachments` no longer blanket-skips `isInline`; real file attachments are served first so a logo strip can't consume the per-message cap), with a best-effort decorative filter (`is_likely_signature_or_logo`: filename tokens, <64px sides, ≥4:1 banners) plus an OCR-length floor (`ATOM_EMAIL_INLINE_IMAGE_MIN_TEXT_CHARS=16`) so signature blocks/logos/tracking pixels never enter memory. Content-type fallback added for nameless inline parts. Flags: `ATOM_IMAGE_OCR_ENABLED`, `ATOM_IMAGE_OCR_VISION_ENABLED`, `ATOM_IMAGE_OCR_TESSERACT_CMD/_LANGS/_TIMEOUT_SECONDS`, `ATOM_EMAIL_INLINE_IMAGE_OCR` (all in the settings catalog under *Email & Attachments*). Tests: `tests/test_image_ocr.py` (34) + inline/routing cases in `tests/test_email_attachment_memory_index.py` / `tests/test_email_attachment_tool.py`.
...
# Image OCR for ingested attachments (core/image_ocr.py; see component #66)
ATOM_IMAGE_OCR_ENABLED=true              # OCR image attachments during ingestion (kill switch)
ATOM_IMAGE_OCR_VISION_ENABLED=true       # vision-LLM fallback when no local engine (Tesseract/Docling)
ATOM_IMAGE_OCR_TESSERACT_CMD=            # explicit tesseract binary path (default: found on PATH)
ATOM_IMAGE_OCR_LANGS=                    # tesseract -l list, e.g. eng+deu (default: Tesseract default)
ATOM_IMAGE_OCR_TIMEOUT_SECONDS=30        # per-image tesseract timeout
ATOM_EMAIL_INLINE_IMAGE_OCR=true         # also OCR images pasted inline in the body
ATOM_EMAIL_INLINE_IMAGE_MIN_TEXT_CHARS=16  # inline OCR text shorter than this = signature/logo -> skip
...
# non-streaming fallback + guard regenerations); on exhaustion the turn
