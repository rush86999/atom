...
50. **Mini-Apps (design, not yet implemented)** — long-running stateful document apps (spreadsheets/docs/decks) on canvases. MVC model: `Canvas`=View, `CanvasLogic` (P7)=Controller, a new `MiniApp` manifest=Model; wraps the real office engine (`office_service` + `workbook_runtime`) rather than reimplementing it. The platform is the harness — every side effect flows through P1→P3→P4→P9; viewer rights always cap an app's declared scopes (no privilege escalation). See `docs/architecture/MINI_APPS.md`.
