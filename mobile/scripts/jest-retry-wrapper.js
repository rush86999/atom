...
/**
 * Jest Retry Wrapper for Flaky Test Detection (Mobile)
 *
 * Run Jest tests multiple times to detect intermittent failures (flaky tests).
 * This implements multi-run flaky detection for mobile tests using jest-expo,
 * matching the Python pytest-rerunfailures pattern used in the backend.
 *
 * Platform-specific adjustments for mobile:
 ...
 *
 * Usage:
 *   node scripts/jest-retry-wrapper.js --testPattern="AuthContext" --runs=3
 *   node scripts/jest-retry-wrapper.js --testPattern="DeviceContext" --runs=10 --output=test-results/mobile_flaky.json
 *
 ...
 *
 * @param {string} testPattern - Jest test name pattern (e.g., "AuthContext", "DeviceContext")
 ...
 * @param {string} platform - Platform name (e.g., 'mobile')
 */
...
function printSummary(results) {
  ...
  if (results.classification === 'flaky') {
    ...
    console.log('   Consider adding jest.retryTimes() or fixing the underlying issue.');
...
function main() {
  ...
  if (!args.testPattern) {
    ...
    console.error('Usage: node jest-retry-wrapper.js --testPattern="AuthContext" --runs=10');
    process.exit(2);
  }
