...
# Linux CI runners and Git-Bash-on-Windows. No venv → keep going with
# whatever Python is already active (the pre-existing behavior); the
# backend still starts, just not from a project venv.
if [[ -d venv ]]; then
