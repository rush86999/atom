...
# Sync Desktop Backend Script
# This script synchronizes backend files from the main project to the desktop app
...
echo "🚀 Starting desktop backend synchronization..."

# Create directories if they don't exist
mkdir -p "$DESKTOP_BACKEND_DIR"
mkdir -p "$DESKTOP_BACKEND_DIR/backend"

echo "📁 Copying backend Python API service..."
rsync -av --delete \
    --exclude="__pycache__" \
    --exclude="*.pyc" \
    --exclude=".pytest_cache" \
    "$PROJECT_ROOT/backend/python-api-service/" \
...
rsync -av --delete \
    --exclude="__pycache__" \
    --exclude="*.pyc" \
    "$PROJECT_ROOT/backend/python-api-service/_utils/" \
    "$DESKTOP_BACKEND_DIR/backend/python-api-service/_utils/"

echo "🎤 Copying wake word detector..."
rsync -av --delete \
    --exclude="__pycache__" \
    --exclude="*.pyc" \
    --exclude="tests/__pycache__" \
    "$PROJECT_ROOT/backend/python-api-service/wake_word_detector/" \
    "$DESKTOP_BACKEND_DIR/backend/python-api-service/wake_word_detector/"

echo "🔊 Copying audio utilities..."
rsync -av --delete \
    --exclude="__pycache__" \
    --exclude="*.pyc" \
    "$PROJECT_ROOT/backend/audio-utils/" \
    "$DESKTOP_BACKEND_DIR/backend/audio-utils/"

echo "📋 Copying requirements files..."
cp "$PROJECT_ROOT/backend/python-api-service/requirements.txt" \
   "$DESKTOP_BACKEND_DIR/backend-requirements.txt"

echo "🔐 Copying wake word model..."
if [ -f "$PROJECT_ROOT/desktop/tauri/src-tauri/atom_wake_word.onnx" ]; then
    cp "$PROJECT_ROOT/desktop/tauri/src-tauri/atom_wake_word.onnx" \
       "$DESKTOP_BACKEND_DIR/atom_wake_word.onnx"
    echo "✅ Wake word model copied"
else
    echo "⚠️  Wake word model not found at $PROJECT_ROOT/desktop/tauri/src-tauri/atom_wake_word.onnx"
fi

echo "📝 Creating desktop-specific requirements..."
cat > "$DESKTOP_BACKEND_DIR/requirements.txt" << 'EOF'
flask>=2.0.0,<2.3.0
...
# Note: lancedb>=0.5.3 requires Python 3.8+, using fallback implementation
EOF

echo "🚀 Creating startup script..."
cat > "$DESKTOP_BACKEND_DIR/start_backend.py" << 'EOF'
#!/usr/bin/env python3
"""
...
chmod +x "$DESKTOP_BACKEND_DIR/start_backend.py"

echo "✅ Desktop backend synchronization completed!"
echo "📋 Next steps:"
echo "   1. Run: cd $DESKTOP_BACKEND_DIR && python -m pip install -r requirements.txt"
echo "   2. Test: python start_backend.py start"
echo "   3. The backend will be available at http://localhost:8083"
