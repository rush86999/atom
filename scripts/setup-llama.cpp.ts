...
import { defaultLlamaConfig, getDefaultModelConfig } from '../config/llama.config';
...
class LlamaCppManager {
  ...
  async downloadLlamaCpp(): Promise<void> {
    ...
    try {
      ...
      response.data.on('data', (chunk: Buffer) => {
        ...
        process.stdout.write(`\rDownload progress: ${percentage.toFixed(1)}% (${(downloaded / 1024 / 1024).toFixed(1)}MB/${(totalSize / 1024 / 1024).toFixed(1)}MB) - ${speed.toFixed(1)} MB/s`);
      });
  ...
  async downloadModel(modelId: string): Promise<void> {
    ...
    const modelPath = path.join(this.config.modelsPath, path.basename(modelConfig.modelPath));
    ...
    console.log(`Size: ${(modelConfig.fileSize / 1024 / 1024 / 1024).toFixed(2)} GB`);
  ...
  async startServer(modelId?: string): Promise<void> {
    ...
    const modelConfig = this.config.models.find(m => m.modelId === targetModelId);
    if (!modelConfig) {
    ...
    if (!fs.existsSync(serverBinary)) {
      throw new Error(`Server binary not found: ${serverBinary}. Please download llama.cpp first.`);
    }

    console.log(`Starting llama.cpp server with model: ${modelConfig.modelName}`);

    const args = [
      '--model', modelPath,
      '--port', this.config.server.serverPort.toString(),
      '--ctx-size', modelConfig.contextSize.toString(),
      '--n-gpu-layers', modelConfig.gpuLayers.toString(),
      '--threads', modelConfig.threadCount.toString(),
      '--batch-size', modelConfig.batchSize.toString(),
      '--log-disable'
    ];
  ...
  async checkServerHealth(): Promise<boolean> {
    try {
      const response = await axios.get(`http://${this.config.server.host}:${this.config.server.serverPort}/health`, {
        timeout: 5000
      });
  ...
  async listModels(): Promise<void> {
    ...
    for (const model of this.config.models) {
      const modelPath = path.join(this.config.modelsPath, path.basename(model.modelPath));
      const exists = fs.existsSync(modelPath);
      const status = exists ? '✓ Downloaded' : '✗ Missing';

      console.log(`\n${model.modelName} (${model.modelId})`);
      console.log(`  Size: ${(model.fileSize / 1024 / 1024 / 1024).toFixed(2)} GB`);
