...
describe('RsiStrategy', () => {
  ...
  beforeEach(() => {
    ...
    strategy = new RsiStrategy('test-user');
    (strategy as any).tradingService = tradingService;
  });

  it('should generate a sell signal when the RSI is overbought', async () => {
    // Mock the historical data
    (axios.get as jest.Mock).mockResolvedValue({
      data: {
        'Time Series (Daily)': {
          // ... add data to make RSI > 70
        },
      },
    });
  ...
  it('should generate a buy signal when the RSI is oversold', async () => {
    // Mock the historical data
    (axios.get as jest.Mock).mockResolvedValue({
      data: {
        'Time Series (Daily)': {
          // ... add data to make RSI < 30
        },
      },
    });
