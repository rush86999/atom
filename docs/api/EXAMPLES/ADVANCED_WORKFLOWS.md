...
SESSION=$(curl -s -X POST http://localhost:8000/api/chat/message \
...
curl -s -H "Authorization: Bearer $TOKEN" \
     http://localhost:8000/api/chat/routing-stats | python3 -m json.tool
```
...
# 1. Write data
curl -s -X POST http://localhost:8000/api/v1/office/excel \
  -H "Content-Type: application/json" \
...
curl -s -X POST "http://localhost:8000/api/v1/office/excel/insert-rows?file_path=$FILE&sheet_name=Sheet1&row=2&count=1"

# 4. Read the evaluated result
curl -s "http://localhost:8000/api/v1/office/excel/formula-result?file_path=$FILE&cell_path=/Sheet1/A4"
```
...
# 1. Create
curl -s -X PUT "http://localhost:8000/api/canvas/$CANVAS?canvas_type=sheets&title=Q4+Budget" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
...
# 2. Read
curl -s -H "Authorization: Bearer $TOKEN" \
     "http://localhost:8000/api/canvas/$CANVAS" | python3 -m json.tool

# 3. Update
curl -s -X PUT "http://localhost:8000/api/canvas/$CANVAS?canvas_type=sheets" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
...
# 4. History
curl -s -H "Authorization: Bearer $TOKEN" \
     "http://localhost:8000/api/canvas/$CANVAS/history" | python3 -m json.tool

# 5. Delete
curl -s -X DELETE -H "Authorization: Bearer $TOKEN" \
     "http://localhost:8000/api/canvas/$CANVAS"
```
...
```bash
# 1. Register Ollama
PROVIDER=$(curl -s -X POST http://localhost:8000/api/local-models \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
...
# 2. Discover models
curl -s -H "Authorization: Bearer $TOKEN" \
     "http://localhost:8000/api/local-models/$PROVIDER/models" | python3 -m json.tool

# 3. Set capabilities
curl -s -X POST "http://localhost:8000/api/local-models/$PROVIDER/capabilities" \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
...
```bash
# 1. Create a DID
DID=$(curl -s -X POST http://localhost:8000/api/federation/dids \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
...
curl -s -X POST http://localhost:8000/api/federation/credentials \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
  -d "{\"issuer_did\":\"$DID\",\"credential_type\":\"AgentIdentityCredential\",\"subject_did\":\"$DID\",\"claims\":{\"name\":\"Test Agent\"}}"

# 3. Verify a request
curl -s -X POST http://localhost:8000/api/federation/verify \
  -H "Authorization: Bearer $TOKEN" \
  -H "Content-Type: application/json" \
