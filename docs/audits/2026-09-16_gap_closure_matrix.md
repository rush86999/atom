...
| Thing | Value |
|---|---|
| HEAD | `6354fdf18` (previous rounds `010b70d40`, `a3aa31ba4`, `315d60445`, `c31316004`) |
| Working tree | HEAD + uncommitted changes; the files this round touched are listed per row below |
| Backend process | pid **33683**, started **2026-09-16T14:29:46Z**, `GET /api/health` → `git_commit 6354fdf18`. Restarted four times as edits landed (2908 → 6878 → 11051 → 22091 → 33683); the last restart postdates every backend edit in this round, including the participant-lane fix (10:11:09) and the grounding-pass writer (10:11:27). |
...
| **Regression sweep (22 suites)** | **134 failed / 805 passed**, and the 134 are byte-identical to the pristine-HEAD failure list (`comm -13` on sorted `FAILED` lists: 134 = 134, **zero new**). All 134 are pre-existing stale-suite failures in five BYOK route/coverage files (auth fixtures and route contracts that predate this round). |
...
| **Semantics (adopted, and deliberately strict)** | A generation enters the denominator only when a verdict says its grounding was actually evaluated (`grounding_ok` / `grounding_passed`) or when it was flagged fabricated. A heuristic satisfaction score with no verdict is **`unevaluated`** — excluded, never called clean — and an in-band score with no verdict is **`unknown`**. A concurrent session tightened this while this round was in flight; it is stricter than the first version and was adopted rather than reverted. |
| **Consequence, stated plainly** | A `grounding_ok` writer now exists (`core/llm/learning_router_registry.py`, added by the concurrent session after this matrix's first draft). Until it actually fires on live traffic the denominator still contains only fabricated generations, so the rate is 1.0 whenever `MIN_EVENTS` is reached and `ATOM_FABRICATION_BENCH_RATE` is effectively vestigial. That is a safe failure direction (it benches sooner) but it is NOT a measured rate. Live check after the final restart: **no verdict of any kind among the 20 newest rows**, because the verification turns carried no evidence for the grounding check to run on. |
...
| The `grounding_ok` marker has not fired on real traffic | The writer exists; no live row carries the verdict yet, so the denominator is still empty of grounded-OK generations and the bench degenerates to "bench after N fabrication verdicts". Needs live traffic through the grounding path (evidence-bearing turns) before the rate means anything. |
| Test files that patched `LearningBasedRouter.__new__` | **Fixed here** (3 call sites in `test_fabrication_bench.py` and `test_chat_orchestrator.py`): `monkeypatch` restores a patched `__new__` as an OWN class attribute, after which `type.__call__` hands the constructor args to `object.__new__` and every later test in the same process dies with `TypeError: object.__new__() takes exactly one argument`. This was **pre-existing at HEAD** (reproduced in the pristine worktree) and was masquerading as "shared-DB pollution" in combined runs. Fixed by patching `_persist_feedback` instead; the 22-suite sweep went from 136 failures with 2 phantom router failures to **134 = the exact HEAD baseline**. |
