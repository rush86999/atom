...
## 4. On-the-Fly Skill Generation (Dynamic Tools)

Beyond high-level GEA directives, Atom supports a multi-layered "on-the-fly" skill creation mechanism that allows agents to build and load functional tools at runtime.

### Layer 1: Instructional Skills (Directives)
- **Mechanism**: Natural language behavior modifications generated by GEA.
- **Application**: Appended to agent system prompts via `EvolutionDirective` objects.
- **Scope**: Best for behavioral changes, policy updates, and simple logical patterns.

### Layer 2: Functional Skills (Live Scripting)
- **Mechanism**: Agents write Python code directly to the workspace `skills/` directory.
- **Loading**: `SkillDynamicLoader` detects changes via `watchdog` and hot-reloads the module within 1s.
- **Execution**: `SkillRegistryService` validates the code (security scanning) and executes it via `importlib`.
- **Hot-Reload**: Uses `sys.modules` cache clearing to prevent stale code execution.

### Layer 3: Formal Skill Packaging
- **Mechanism**: Successful "on-the-fly" scripts are stabilized into structured packages.
- **Builder**: `SkillBuilderService` generates standard `SKILL.md`, `requirements.txt`, and source directories.
- **Governance**: Promotions to `AUTONOMOUS` usually trigger formal packaging for audit and persistence.

### Technical Service Stack
| Service | Role in On-the-Fly Skills |
|---------|-------------------------|
| `SkillDynamicLoader` | Runtime module loading & hot-reload |
| `SkillBuilderService` | Structured skill project creation |
| `SkillRegistryService` | Validation, security scanning, and execution |
| `AgentExecutionService` | Orchestrating tool usage in the chat loop |
| `SkillSecurityScanner` | Scanning generated code for malicious patterns |

---

## 5. Attribution & Audit Trail
...
### The Graduation Path
| Starting Level | Promotion Trigger | New Level | Added Capabilities |
|----------------|-------------------|-----------|--------------------|
| **STUDENT** | >70% confidence, 5 successful ops | **INTERN** | Canvas presentation, basic tools |
| **INTERN** | >80% confidence, 20 successful ops | **SUPERVISED** | Advanced integrations, error recovery |
| **SUPERVISED** | >90% confidence, 50 successful ops | **AUTONOMOUS** | Shell access, self-writing skills |
...
## 8. Real-Time Governance Dashboard
