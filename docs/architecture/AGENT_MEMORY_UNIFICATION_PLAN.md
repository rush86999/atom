...
> **Status:** Adopted plan of record (2026-08-19). **P0 implemented & live-verified** (`7f55636ca`, 2026-08-19 — assembler, chat-path extraction, dropped-key renders incl. feedback, universal ingestion symmetry, GraphRAG keyword fix + property hydration); **P1.3 first slice + lexical leg done** (`74301215d`, 2026-08-19 — conversations leg in hybrid search, knowledge/conversations VFS subtree, FTS/BM25 leg live, ILIKE-fallback tokenization fix, poller persists IMs to the comms store); **P1.3 assembler swap done** (auto-injected `knowledge` block now routes through `DocumentsHybridSearch` — documents + conversations fused, bridge don't copy; the standalone comms-only leg was replaced by the unified hybrid leg). **P1.1 + P1.2 tool half done** (`5cc0f8041`, 2026-08-19 — meta agent now sees documents.search, search_communications, recall_episodes, memory_remember/forget; recall_episodes action over the canonical episodes table — the agent_episodes mirror-table merge is deferred to P2: the two episode subsystems are each self-consistent and merging requires schema mapping); comm-store blocker cleared (fastembed fallback when torch is broken — 384-dim, empty-table self-heal; Telegram normalizer bug that stored 'text' payloads as empty content). **P1.4 rerank done** (budget-gated rerank phase in the assembler: cross-encoder via HybridRetrievalService reuse probed once in `warm()`, fastembed cosine-similarity fallback when torch is broken, no-op otherwise; per-leg 250ms cap, phase only when gather < 700ms, kill switch `MEMORY_CONTEXT_RERANK`). **Priority: P1.3 remaining work** — governed by **design principle 8 (bridge, don't copy)** — the conversations leg and VFS subtree bridge to the comms store, never duplicating records into `documents`. Remaining P1: graph embeddings (P1.5). Owner: brennan.ca pilot → upstream
...
**P1.3 — Conversations leg for hybrid search (PRIORITY — first slice landed `74301215d`; assembler swap done):** `DocumentsHybridSearch` RRF-fuses `atom_communications` + `documents` + `turn_facts` legs — closing the "ingested but unreadable" loop for emails/Slack/WhatsApp. The auto-injected `knowledge` block was swapped to the hybrid path: the assembler's standalone comms-only leg was replaced by a unified `_knowledge_leg` routing through `DocumentsHybridSearch` (documents BM25+vector + conversations leg), so the injected block now surfaces documents and conversations as first-class lines. *Closes G10, G6, G3, G7.* Subject to **design principle 8 (bridge, don't copy)**: the leg appends first-class results hydrated from the comms record (`source='communication'`), and the VFS exposes `knowledge/conversations/<id>/content.lines` over the same store — nothing duplicated into `documents`, no snapshot at retrieval time. Verification gate: an email/Slack message ingested into the comms store must be (a) findable via hybrid search AND (b) `cat`-able via the VFS subtree, with no new rows written to `documents` for the same content. Known limitation: the conversations leg resolves the comms pipeline for the `default` workspace (`documents_hybrid.py:109`) while the rest of the assembler is workspace-scoped — aligned for the brennan pilot, needs workspace plumbing before multi-tenant.

**P1.4 — Rerank (done, 2026-08-19)**: cross-encoder rerank of the assembled block when budget allows — implemented in-process inside the assembler (not HTTP-only). A rerank phase runs between the leg gather and the per-block caps: each leg's candidate lines are re-ordered by relevance so the caps keep the best lines, not the first N the store returned. Tier 1: sentence-transformers cross-encoder via `HybridRetrievalService` reuse (`core/memory_context_assembler.py:_probe_cross_encoder`), probed ONCE in `warm()` so the hot path never imports torch. Tier 2 (torch broken): fastembed cosine-similarity — the same embedder the vector legs use. Tier 3: no-op. Gating: phase runs only when the gather finished < 700ms (plan §6 total < 800ms), per-leg cap 250ms via `_safe`, phase stops once budget is exhausted. Kill switch `MEMORY_CONTEXT_RERANK` (default ON).
