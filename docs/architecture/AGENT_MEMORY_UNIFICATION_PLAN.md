...
> **Status:** Adopted plan of record (2026-08-19). **P0 implemented & live-verified** (`7f55636ca`, 2026-08-19 — assembler, chat-path extraction, dropped-key renders incl. feedback, universal ingestion symmetry, GraphRAG keyword fix + property hydration); **P1.3 first slice + lexical leg done** (`74301215d`, 2026-08-19 — conversations leg in hybrid search, knowledge/conversations VFS subtree, FTS/BM25 leg live, ILIKE-fallback tokenization fix, poller persists IMs to the comms store). **Priority: P1.3 remaining work** (swap auto-injected knowledge block to the hybrid path; closes G6/G3/G7), governed by **design principle 8 (bridge, don't copy)** — the conversations leg and VFS subtree bridge to the comms store, never duplicating records into `documents`. Remaining P1 after that: meta-agent tool equality, episode unification, rerank, graph embeddings. Owner: brennan.ca pilot → upstream
...
8. **Bridge, don't copy**: every retrieval surface the agent uses to *go look* — the `knowledge/` VFS tree, `search_communications`, `documents.search`, and the P1.3 conversations leg — bridges to the system of record (comms store / `documents` / `turn_facts`) with stable, citable IDs and line-numbered content. Never snapshot or duplicate records into a second store at retrieval time. The difference is the top-5 guess vs. the deep dive: a copy degrades into "remembers only when the injected top-k happens to hit"; a bridge lets the agent always navigate to the live record and read it in full, citable, and current.
...
**P1.3 — Conversations leg for hybrid search (PRIORITY — first slice landed `74301215d`; remaining work is the priority now)**: extend `DocumentsHybridSearch` (or add a `memory.search` action) to RRF-fuse `atom_communications` + `documents` + `turn_facts` legs — closing the "ingested but unreadable" loop for emails/Slack/WhatsApp. Swap the auto-injected `knowledge` block to the hybrid path at the same time. *Closes G10, G6, G3, G7.* Subject to **design principle 8 (bridge, don't copy)**: the leg appends first-class results hydrated from the comms record (`source='communication'`), and the VFS exposes `knowledge/conversations/<id>/content.lines` over the same store — nothing duplicated into `documents`, no snapshot at retrieval time. Verification gate: an email/Slack message ingested into the comms store must be (a) findable via hybrid search AND (b) `cat`-able via the VFS subtree, with no new rows written to `documents` for the same content.
...
### P0.4 audit results — Outlook (first integration, 2026-08-19)

| Integration | Comms store (Pipeline A) | Graph entities (Pipeline B) | Turn-fact on bridge | Notes |
|---|---|---|---|---|
| **Outlook** | ✓ write paths exist (on-demand `outlook_service.fetch_recent_messages` → `ingest_message` at `outlook_service.py:1137`; calendar `:1179`; integration-webhook bridge at `core/ingestion_pipeline.py:1277`) — **real-time poller now wired (fix landed 2026-08-19)**: `CommunicationIngestionPipeline.start_outlook_poller()` (idempotent, interval config, `embed_content=True`), started on Microsoft OAuth connect (`oauth_routes._handle_callback_logic`) and on startup recovery (`main_api_app` lifespan). `_fetch_outlook_messages` (`atom_communication_ingestion_pipeline.py:1477+`) now reads the DB `IntegrationToken` via `outlook_service._get_access_token` (encrypted + auto-refreshed) instead of the deprecated, never-populated file `token_storage` (which always returned None → poller never fetched). **Remaining**: Graph push receiver `microsoft365_routes.py:258-271` `/webhook` is still a stub, and the memory webhook `/api/webhooks/communication/outlook` requires a JWT + `x-atom-webhook-secret` header that Graph push cannot send (Graph uses body `clientState`) — poller is the interim real-time path; SaaS push receiver exists at `api/routes/webhooks/ingestion_webhooks.py:647`. | ✓ conditional default-ON: `ingest_message` (`atom_communication_ingestion_pipeline.py:576-605`) spawns `knowledge_manager.process_document` + `CommunicationIntelligenceService.analyze_and_route` when `enable_out_of_workflow_automations` && `enable_automatic_knowledge_extraction` (both default True, `automation_settings.py:72,76`). Caveat: `core/ingestion_pipeline.py:1288` `continue` skips the Deep-Tier GraphRAG entity extraction for outlook/gmail/slack/teams — graph population relies solely on `process_document`. | ✗ n/a (email, not chat; G8 covers chat surfaces) | **Embedding degradation (all comms):** `ingest_message` embeds only when `embed_content` config is True (`atom_communication_ingestion_pipeline.py:564`); otherwise rows get `[0.0]*768` zero vectors (`:231`), so the `search_communications` vector leg returns junk and hybrid degrades to FTS-only. The poller sets `embed_content=True`, but on-demand ingest paths inherit config defaults. |
| Telegram | ⚠️ webhook path EXISTS (`atom_communication_memory_webhooks.py:236` → `_process_telegram_webhook` → `ingest_message` at `:511`) — **G12 needs a correction**: the asymmetry is the *poller* dispatch (`_fetch_new_messages` at `:678-691` has no telegram branch), and the webhook is likewise JWT-blocked for external pushes (Telegram bot can send a `secret_token` header but not a JWT). Graph entity ingestion runs via the same `ingest_message` hook. | ✓ (same conditional path) | ✗ | G12 restated: Telegram's comms-store gap is poller-only; the real systemic blocker is the JWT-on-webhook requirement for ALL external push senders. |
