...
- Ingestion-symmetry coverage table: per integration — comms store (Pipeline A) ✓/✗, graph entities (Pipeline B) ✓/✗, turn-fact extraction on its chat bridge ✓/✗. Any ✗ from the P0.4 audit must have a tracked follow-up before P1 ships.
  ...
  | **Slack** | ✓ on legacy/memory webhooks (`api/webhook_routes.py:41-65` → `_handle_webhook_message` at `atom_communication_ingestion_pipeline.py:578/601`; memory webhook `atom_communication_memory_webhooks.py:441`) — **✗ on the modern bridge**: `slack_webhooks.py:106` passes inner `data.get("event",{})` while `_transform_slack_payload` expects the `event_callback` envelope (`ingestion_pipeline.py:1718-1722`) → 0 records | ✓ (queue `/webhooks/slack/events` non-tiered `:1076/:1097`; legacy `knowledge_manager.process_document` `:662`) | ✓ (bridge → `ChatOrchestrator.process_chat_message` → `:492`) | 2026-08-20 | Modern bridge A/B path is a likely no-op (payload envelope mismatch) — TRACKED follow-up. |
  | **WhatsApp** | ✓ (bridge passes full payload `whatsapp_webhooks.py:100` → tiered `ingest_message` `:1329`; poller `_fetch_whatsapp_messages` `:782` → `ingest_message`) | ✓ (tiered `:1384/:1393`; whatsapp ∈ `COMMUNICATION_INTEGRATIONS` + `MULTI_ENTITY_INTEGRATIONS`) | ✓ (bridge → UCB whatsapp adapter `:40` → orchestrator `:492`) | 2026-08-20 | **Content bug (TRACKED):** `_normalize_message` WhatsApp branch reads `message_data.get("content","")` (`atom_communication_ingestion_pipeline.py:1682`) but `_transform_whatsapp_payload` emits `text` (`ingestion_pipeline.py:3321`) → messages stored with empty content / meaningless embedding. |
  | **Gmail** | **partial** — webhook `/webhooks/gmail/events` (`ingestion_webhooks.py:422`) → **non-tiered** direct LanceDB write (`ingestion_pipeline.py:1131/:1173`, bypasses `ingest_message`/normalization); poller `_fetch_gmail_messages` `:1381` → `ingest_message` `:728` ✓ (config-gated) | ✓ (non-tiered `:1076/:1097` + multi-entity `:1117`) | ✗ (no chat bridge; Gmail ∉ UCB `ADAPTERS` at `universal_communication_bridge.py:39-46`) | 2026-08-20 | Webhook A-path bypasses the comm pipeline (hardcoded direct write); no chat surface → turn-fact N/A on webhook. TRACKED follow-up. |
  | **Teams** | **partial** — bridge ✓ only via record-shape heuristic (`"teams"` ∉ `_KNOWN_COMM_INTEGRATIONS` which lists `microsoft_teams` at `:66-70`; falls through to shape check `:92-100`); queue `/webhooks/communication/teams` (`ingestion_webhooks.py:1183`) → **no A** | ✓ (tiered `:1384/:1393`; teams ∈ `COMMUNICATION_INTEGRATIONS` + `MULTI_ENTITY_INTEGRATIONS`) | ✓ (bridge only — UCB teams adapter `:43` → orchestrator `:492`) | 2026-08-20 | `teams` alias missing from the known-comm list; poller keyed on `microsoft_teams` (`:761`) never matches the bridge's `teams` string. TRACKED follow-up. |
  | **Discord** | ✓ (bridge, `"discord"` known `:68` → tiered `ingest_message` `:1329`; queue path `/webhooks/communication/discord` is non-tiered → no A) | ✓ (tiered `:1384/:1393` — **no multi-entity**, discord ∉ `COMMUNICATION_INTEGRATIONS`; queue path B ✓) | ✓ (bridge, but **interactions only** — `adapters/discord.py:56-108` rejects plain `MESSAGE_CREATE`; route requires `guild_id`, no DMs) | 2026-08-20 | No poller (`_fetch_new_messages` has no discord branch `:757-768`); bridge drops non-interaction messages. TRACKED follow-up. |
  | **Zendesk** | ✗ (no webhook, no transformer — `_transform_zendesk_sell_payload` `:2268` is CRM-shaped, not message-shaped) | ✗ (only `zendesk_sell` CRM via pm-crm queue `/webhooks/pm-crm/{id}` `ingestion_webhooks.py:1013` → non-tiered B-only; Zendesk Support itself has no graph path) | ✗ | 2026-08-20 | **Largest gap — zero inbound ingestion.** `zendesk_routes.py` is a stub (auth/callback/search/items — sample data); `atom_zendesk_integration_service.py` is outbound-only. TRACKED follow-up. |

  **Tracked follow-ups from the P0.4 audit (all ✗/partial rows above):**
  1. **WhatsApp content bug (HIGH — memory quality)**: `_normalize_message` WhatsApp branch reads `content` but the transformer emits `text` → empty content/embedding in `atom_communications`. Add a `content`/`text`/`body` fallback (mirror the generic branch at `:1717-1735`).
  2. **Slack modern bridge payload**: pass the full payload (or wrap in the `event_callback` envelope) to `WebhookBridge.process_event` so `_transform_slack_payload` matches and the modern A+B path is live, not a silent no-op.
  3. **Teams alias**: add `teams` to `_KNOWN_COMM_INTEGRATIONS` / `CommunicationAppType` (or normalize to `microsoft_teams` in the bridge) so A-path membership is explicit instead of shape-heuristic; align the poller key.
  4. **Gmail webhook A-path**: route `/webhooks/gmail/events` through the tiered `ingest_message` path instead of the hardcoded direct LanceDB write; add a chat-bridge if Gmail becomes an interactive surface.
  ...
  6. **Zendesk**: build real inbound ingestion (webhook + `_transform_zendesk_payload` + comm-list membership) so support tickets reach both pipelines — or explicitly declare it out of scope for the pilot. |
