...
| 2b | **Chat-path extraction (2026-08)** | Plain chat/IM turns (ChatOrchestrator — web chat and every messaging bridge) now also extract durable facts fire-and-forget after each response; previously only the agent ReAct path did, making chat a memory write black hole. | `chat_orchestrator.py:_dispatch_turn_fact_extraction` |
