...
|---|---|---|
| CC6.1 — Logical access | Multi-user auth, 8-role RBAC enforced at route level; OIDC SSO (SAML IdPs via an OIDC bridge) | `backend/core/rbac_service.py`, `core/security_dependencies.py`, `api/auth_routes.py`, `api/sso_oidc_routes.py` |
...
- SSO is OIDC; SAML and SCIM provisioning are on the roadmap (SAML via an OIDC-bridging IdP works today).
...
| SAML + SCIM | **Roadmap** | Still enterprise-procurement requirements; OIDC covers the immediate need. |
