...
**Round 80m follow-on (same session — BYOK surface mini-audit)**: live BYOK router (`api/byok_routes.py`, 21 routes) verified fully auth-gated with masked keys; the orphan duplicate `core/byok_endpoints.py` (21 unauth routes) is imported by prod code ONLY for `get_byok_manager`/`BYOKManager` (never its router) — documented as a landmine, not archived (moving would churn its dependents). Web journey was doubly broken: `DevStudio/BYOKManager.tsx` sent no JWT AND was imported by nothing → added `authHeaders()` to all three fetches and wired it into `pages/dev-studio.tsx` as an "API Keys" tab. B15 (round-71 oauth tokens columns) verified already fixed. BYOK suite 9/9 incl. new JWT assertion.

**Round 80l follow-on (same session — auth-forward headerless proxied fetches)**: 8 health/status proxy handlers (azure, microsoft365, notion, figma, google-workspace, teams health probes + slack/auth/start + stripe) called the backend with a single-argument fetch — no headers object — so the 80d fwdAuth merge never applied and gated backends would 401. Added `{ headers: { ...fwdAuth } }` to each; tsc clean on touched paths.
...
**Final state**: all 9 files green in ONE combined process — **245 passed / 0 failed / 0 errors**. No source files modified; app was the correct contract in all 9 cases.

| File | Before | After | Root cause → fix |
|---|---|---|---|
| `tests/api/test_multi_integration_workflow_routes.py` | 31 failed | **31 passed** | R120 auth gate: router mounted with `dependencies=[Depends(get_current_user)]` → all requests 401. Fixed `client` fixture with `app.dependency_overrides[get_current_user]` returning a `User` (super_admin/active) + `dependency_overrides.clear()` teardown — same pattern as `tests/api/conftest.py` |
| `tests/error_paths/test_agent_lifecycle_error_paths.py` | 1 failed (flake, ~30% of full-file runs) | **37 passed** ×5 runs | Thread+`patch` race in `test_graduation_with_concurrent_attempts` (and same latent pattern in `test_promotion_with_concurrent_attempts`): 5 threads each entered/exited their own `with patch('core.agent_graduation_service.get_episode_service')`; an exiting thread restored the REAL function while a sibling called it → real `EpisodeService` over Mock query data → `TypeError: 'Mock' object is not iterable` in `episode_service.calculate_readiness_metrics`. Replaced threads+`asyncio.run` with `asyncio.gather` inside ONE patch context (deterministic genuine concurrency); promotion test likewise `asyncio.gather` (also removes the "This event loop is already running" loop-race noise) |
| `tests/error_paths/test_database_error_paths.py` | 2+ (batch) | **30 passed** | Batch-order dependent — green standalone and in combined runs; no change needed |
| `tests/error_paths/test_episode_segmentation_error_paths.py` | 6 failed | **24 passed** | (1) `test_cosine_similarity_zero_vectors`/`test_zero_division_in_fallback` asserted NaN — app now guards zero-magnitude vectors (`episode_segmentation_service.py:138-139`) → assert 0.0; (2) `test_lancedb_embedding_failure` asserted `[]` — app now falls back to keyword similarity (`:106-108`) → assert `[1]`; (3) `test_commit_failure_constraint_violation` relied on NOT NULL maturity failure — episode pipeline is now dict-based with a single deferred commit (`create_episode_from_session`, `:383-425`) → patch `db.commit` to raise `IntegrityError`, assert propagation; (4) `test_segment_creation_failure` passed an ORM `Episode` — `_create_segments` now takes an episode DICT + `canvas_context` (`:662`) → dict episode, commit `OperationalError` propagates, segments rolled back; (5) `test_lancedb_archival_failure` passed ORM episode — `_archive_to_lancedb` takes a dict (`:766`); `lancedb.db=None` short-circuits → returns None |
| `tests/error_paths/test_edge_case_error_paths.py` | 1 failed | **33 passed** | `test_none_action_type_in_cache_lookup` matched AttributeError message `'lower'` — `_make_key` now calls `action_type.startswith("dir:")` first (`governance_cache.py:136`) → message is `'startswith'`; updated match + docstring |
| `tests/core/test_episode_service_bughunt2.py` | 2 (batch) | **40 passed** | Batch-order dependent — green standalone and in combined runs; no change needed |
| `tests/core/test_hybrid_join_key.py` | 1 failed | **8 passed** | Source-scan regex expected direct `self.memory_handler.add_document(...)` — file-ingest path now wraps it in `asyncio.to_thread(self.memory_handler.add_document, ...)` (`auto_document_ingestion.py:443-460`; same kwargs incl. `source_type:"file"` + `doc_id=`) → regex updated to the to_thread form |
| `tests/core/test_group_reflection_service.py` | 1 failed | **10 passed** | `KeyError: 'value'` — fake replaced `svc.llm.generate(system_prompt=...)`, app now calls `llm.generate_response(tenant_id=..., messages=[{role:system,content:system_prompt}, ...])` (`group_reflection_service.py:546-552`) → fake `generate_response`, capture `messages[0]["content"]` |
| `tests/core/test_expression_parser_bughunt.py` | 1 failed | **17 passed** | `test_negative_base_power_resolves_correctly` asserted `-2 ** 2 == 4` (old model: leading minus inside the NUMBER literal). Unary minus is an OPERATOR token and `**` binds looser than unary minus on its left operand — explicitly documented Python-standard behavior (`expression_parser.py:280`: `-2 ** 2 == -4`). Updated to assert `-2 ** 2 == -4` AND `(-2) ** 2 == 4` |

**Real bugs found**: 0 — the app was the correct contract in all 9 files; every failure was stale test expectations or test-side races.

**Verification** (all from `backend/`, `PYTHONPATH=/Users/rushiparikh/projects/atom/backend`, `-p no:cacheprovider -q`):
```bash
./venv/bin/python -m pytest tests/api/test_multi_integration_workflow_routes.py          # 31 passed
./venv/bin/python -m pytest tests/error_paths/test_agent_lifecycle_error_paths.py        # 37 passed ×5
./venv/bin/python -m pytest tests/error_paths/test_database_error_paths.py               # 30 passed
./venv/bin/python -m pytest tests/error_paths/test_episode_segmentation_error_paths.py   # 24 passed
./venv/bin/python -m pytest tests/error_paths/test_edge_case_error_paths.py              # 33 passed
./venv/bin/python -m pytest tests/core/test_episode_service_bughunt2.py                  # 40 passed
./venv/bin/python -m pytest tests/core/test_hybrid_join_key.py                           # 8 passed
./venv/bin/python -m pytest tests/core/test_group_reflection_service.py                  # 10 passed
./venv/bin/python -m pytest tests/core/test_expression_parser_bughunt.py                 # 17 passed
# Combined: 245 passed / 0 failed
```
...
**Tool selection rationale**: the literal `ls tests/ | rg -i "<name>"` rule (filename match at `tests/` root) returns device_tool/agent_guidance_canvas_tool/office_tool as "no mention" — but content-matching shows all three are already ≥97% covered (`test_desktop_device_governance.py`, `test_covpush_tools_a.py`, `test_covpush_w75c_tools_a.py`). Measured baselines instead: the 3 LARGEST tools with genuine coverage gaps are `smarthome_tool.py` (593 L, **19%**), `media_tool.py` (599 L, **43%**), `platform_management_tool.py` (585 L, **20%**); `device_tool` 97%, `agent_guidance` 100%, `office_tool` 99%, `calendar_tool` 100%, `canvas_docs_tool` 100% — already covered, so not repeated.

**Baselines**: whatsapp_service_manager **20%** (151 stmts; only reachable helper coverage from w93), whatsapp_database_setup **0%** (never imported by any test), reasoning_routes **0%** via wave files but note `tests/api/test_reasoning_routes.py` (463 L, exists since earlier waves) already covers it 100% — the wave-99 file adds the missing **anonymous-401 checks for BOTH endpoints** (the old file only 401'd POST /feedback) plus dataclass-chain GET coverage. Tools: smarthome 19%, media 43%, platform_mgmt 20%.

**Bugs fixed (TDD RED→GREEN — failing test first, source then patched)**:
1. **`core/feature_flags.py` — `SMART_HOME_CONTROL_ENABLED` missing**: `core/smarthome/hue_service.py` (8 sites: 80, 94, 137, 187, 237, 296, 353, 398) and `core/smarthome/home_assistant_service.py` (5 sites: 115, 158, 220, 272, 301) reference `FeatureFlags.SMART_HOME_CONTROL_ENABLED` unconditionally; the class attribute never existed → **every HueService/HomeAssistantService construction raised `AttributeError`** (source of 13+ pre-existing test_smarthome_tool.py failures). Added `SMART_HOME_CONTROL_ENABLED = os.getenv("SMART_HOME_CONTROL_ENABLED", "true").lower() == "true"` next to DEVICE_CAPABILITIES_ENABLED — RED: `test_smart_home_control_feature_flag_exists`. Verified post-fix: the file's remaining failures are unchanged-in-count (21) stale-suite issues (`core.smarthome.hue_service.Bridge` mock target doesn't exist, sync calls of async methods, `db=` kwarg on functions that take `agent_id`) — pre-existing, documented, NOT caused by this wave (identical 21 failed / 2 skipped with fixes reverted).
2. **`core/models.py` Tenant — 3 missing profile columns**: `tools/platform_management_tool.py::update_tenant_profile` sets `tenant.billing_email`, `tenant.budget_limit_usd`, `tenant.metadata_json` (lines 98–108) but the Tenant model declares none → every assignment raised `AttributeError` (caught → "Error: Failed to update tenant profile") OR was silently dropped while the tool reported success. Added nullable `billing_email`, `budget_limit_usd`, `metadata_json` (JSONColumn) to Tenant — RED: `TestBugPins::test_tenant_profile_fields_persist` (read-back from a fresh session before fix: AttributeError / missing values). NOTE: existing dev DBs need the guarded-DDL reconcile (`ALTER TABLE tenants ADD COLUMN billing_email ...`, `budget_limit_usd REAL`, `metadata_json JSON`) per the R71 hybrid-DB pattern.
3. **`core/models.py` User — `is_active` missing**: `manage_tenant_member` deactivate/reactivate set `user.is_active` (line 213/216) — no such column on User (it exists on UserAccount) → changes silently never persisted while the tool returned "deactivated/reactivated". Added `is_active = Column(Boolean, default=True, nullable=False)` to User — RED: `TestManageTenantMember::test_deactivate` (read-back still True before fix). NOTE: same guarded-DDL note (`ALTER TABLE users ADD COLUMN is_active BOOLEAN NOT NULL DEFAULT 1`).
4. **`tools/platform_management_tool.py::list_tenant_members` — `m.full_name` never existed**: User exposes a `name` property (first+last), not `full_name` → every call hit AttributeError and returned "Error listing tenant members" (member listing was **completely broken**). Fixed to `m.name or m.email` — RED: `TestBugPins::test_list_members_uses_name_not_full_name`. Partner stale tests updated to the corrected attribute: `test_covpush_tools_b.py:553` and `test_covpush_w75c_tools_a.py:895-897` (`full_name=` → `name=`).
5. **`tools/platform_management_tool.py::create_tenant` — missing `subdomain` (NOT NULL+unique)**: `Tenant(name=name)` always raised IntegrityError → **tenant creation was completely broken**. Now `subdomain=f"tenant-{uuid.uuid4().hex[:12]}"` — RED: `TestTenantCrud::test_create_tenant` (expected "created successfully", got "Error creating tenant").

**Coverage deltas** (measured with `--cov=<module> --cov-report=term-missing`):

| Module | Before | After (w99 alone) | Stmts | Remaining uncovered |
|---|---|---|---|---|
| `integrations/whatsapp_service_manager.py` | 20% | **100%** | 151 | — |
| `integrations/whatsapp_database_setup.py` | 0% (never imported) | **100%** | 153 | — (incl. `__main__` script block via runpy, both success + failure branches; DDL strings are not executable statements) |
| `api/reasoning_routes.py` | 0% via wave files (100% via pre-existing `tests/api/test_reasoning_routes.py`) | **100%** | 38 | — |
| `tools/smarthome_tool.py` | 19% | **100%** | 169 | — |
| `tools/media_tool.py` | 43% | **99%** | 197 | line 137 = unreachable dead code: `if agent_id:` inside `_check_media_governance` except — `agent_id` is always truthy there (the human path returns before the try); fail-open branch can never execute (harmless defensive code, left in place) |
| `tools/platform_management_tool.py` | 20% | **100%** | 358 | — |

Combined w99 run: **228 passed / 0 failed; TOTAL 1066 stmts / 1 missing / 99%**.

**Evidence** (each `cd backend && PYTHONPATH=/Users/rushiparikh/projects/atom/backend venv/bin/python -m pytest -p no:cacheprovider -q`, ONE process at a time — RAM control):
1. RED proofs: flag-existence test (AttributeError before fix), tenant profile persistence read-back (AttributeError/missing before), member-list read-back ("Error listing tenant members" before), user deactivate read-back (still active before), create_tenant ("Error creating tenant" before).
2. Combined w99 run (single process, 6 files, `--cov` all 6 modules) → 228 passed / 0 failed.
3. Partner regression: `test_covpush_tools_b.py` → 54 passed (after `full_name`→`name` in 1 stale mock), `test_covpush_w75c_tools_a.py` → 152 passed, `test_covpush_w68b_services.py` (create_all-heavy model canary) → 131 passed, `test_covpush_w93_whatsapp.py` + `test_covpush_w64_whatsapp_routes.py` → 135 passed, `test_smarthome_tool.py` → 21 failed/2 skipped — **identical to pre-wave baseline** (stale-suite: mocks `core.smarthome.hue_service.Bridge` which does not exist, sync calls of async methods, `db=` kwarg); zero FeatureFlags errors remain; NOT regressed by this wave.
...
**Bugs fixed (TDD RED→GREEN — failing test first, source then patched)**:
1. `integrations/auth_handler_zoom.py` (4 sites: `exchange_code_for_token`/`refresh_access_token`/`get_user_info`/`make_authenticated_request`) — intentional `HTTPException(400)` (and the API-status passthrough in `make_authenticated_request`) raised inside `try:` was **swallowed by the broad `except Exception`** and re-raised as **500**: every OAuth/API failure (invalid_grant, expired refresh token, 404 from the API) surfaced as a generic 500 and the 400 raise sites were dead code. `except HTTPException: raise` added — RED: `test_non_200_raises_400` + `test_non_2xx_raises` (asserted 400/passthrough-status, got 500).
2. `integrations/asana_real_service.py:26-27` — `__init__` defaulted `access_token` to a **REAL hardcoded Asana credential** (`2/1211551477617044/...`) and `workspace_gid` to a real workspace ID when env vars were unset — production token shipped in source + every process. Defaults now `os.getenv("ASANA_ACCESS_TOKEN", "")` / `os.getenv("ASANA_WORKSPACE_GID", "")` — RED: `test_no_env_token_no_hardcoded_credential` (asserted falsy, got the hardcoded token).
3. `integrations/asana_real_service.py:51-53` — `_make_request` DELETE branch returned `{"success": True}` **unconditionally, ignoring the HTTP status** — deletes reported success on 401/404/500 (fail-open). Now checks status: 200/204 → success, else `{"success": False}` — RED: `test_delete_failure` (asserted `{"success": False}`, got `{"success": True}`).
4. `integrations/asana_real_service.py` (5 sites: `create_task`/`update_task`/`delete_task`/`get_projects`/`create_project`) — the circuit-breaker `HTTPException(503)` and rate-limit `HTTPException(429)` raised inside `try:` were **swallowed by the broad `except Exception`** and converted to `None`/`[]` — guard branches were dead code and callers could never distinguish open-circuit/rate-limit from a plain failure. `except HTTPException as e: raise` added — RED: `test_circuit_breaker_open_503` / `test_rate_limited_429` × 5 methods (got `None`/`[]`, expected 503/429).
5. `integrations/auth_handler_salesforce.py` (3 sites: `exchange_code_for_token`/`refresh_access_token`/`get_user_info`) — same HTTPException-swallow class as bug 1: non-200 → 400 re-raised as 500, dead 400 sites. `except HTTPException: raise` added — RED: `test_non_200_raises_400` × 3.
6. `integrations/auth_handler_dropbox.py` (3 sites: same methods) — same bug class. `except HTTPException: raise` added — RED: `test_non_200_raises_400` × 3.
7. `integrations/shopify_routes.py:1,24` — `import datetime` + `datetime.now()` → `AttributeError: module 'datetime' has no attribute 'now'` — **`GET /auth/url` always 500'd** (dead route). Changed to `from datetime import datetime` — RED: `test_auth_url` (500 before, 200 after).
8. `integrations/shopify_routes.py:46-56` — callback created `EcommerceStore(workspace_id=...)` / set `store.workspace_id`, but **`EcommerceStore` (core/models.py:5781) has NO `workspace_id` column** (only tenant_id/platform/shop_domain/access_token/is_active/metadata_json) → `TypeError: 'workspace_id' is an invalid keyword argument` on EVERY new-store registration and `AttributeError` on the existing-store path — the store-save flow was broken. Workspace is now carried in `metadata_json["workspace_id"]` (no schema change) — RED: `test_callback_new_store` (400 before, 200 after).
9. **Auth missing (CRITICAL)**: `integrations/shopify_routes.py` — **every data route had NO authentication**: `/shop`, `/products`, `/orders`, `/customers*`, `/fulfillments*`, `/refunds*`, `/draft-orders*`, `/transactions`, `/analytics`, `/inventory`, `/locations`, `/webhooks/setup` returned live store data for any caller holding a token (token passed as a URL query param — also lands in proxy logs). `current_user: User = Depends(get_current_user)` now required on all 16 data routes (OAuth flow `/auth/url`+`/auth/callback`, `/status`, `/` stay public per the wave-93 dropbox convention) — RED: `test_*_anonymous_401` × 16 (200 before, 401 after).
10. **Auth missing (CRITICAL)**: `integrations/airtable_routes.py` — the READ endpoints `GET /records/{base}/{table}` and `GET /records/{base}/{table}/{record_id}` had **no authentication** — anonymous users could read any Airtable base/table reachable by the integration key. `get_current_user` added (POST/PATCH/DELETE and `/search` were already authed; `/status` + `/health` stay public) — RED: `test_list_records/test_get_record_anonymous_401` (200 before, 401 after).

**Coverage deltas** (measured with `--cov=<module> --cov-report=term-missing`, w98 files):

| Module | Before | After (w98 alone) | Stmts | Remaining uncovered |
|---|---|---|---|---|
| `integrations/auth_handler_zoom.py` | 0% | **100%** | 156 | — |
| `integrations/asana_real_service.py` | 0% | **100%** | 197 | — |
| `integrations/auth_handler_salesforce.py` | 0% | **100%** | 155 | — |
| `integrations/auth_handler_dropbox.py` | 0% | **100%** | 129 | — |
| `integrations/shopify_routes.py` | 0% | **100%** | 113 | — |
| `integrations/airtable_routes.py` | 0% | **100%** | 97 | — |

**Evidence** (each `cd backend && PYTHONPATH=/Users/rushiparikh/projects/atom/backend venv/bin/python -m pytest -p no:cacheprovider -q`, ONE process at a time — RAM control):
1. RED proofs: zoom/salesforce/dropbox `test_non_200_raises_400` + zoom `test_non_2xx_raises` (500 before `except HTTPException: raise`), asana hardcoded-token + DELETE-fail-open + 503/429 swallows, shopify `/auth/url` 500 (datetime module import), shopify callback 400 (`workspace_id` invalid kwarg), 16 shopify + 2 airtable anonymous-401 tests (200 before auth added).
2. New-wave combined run (single process, all 6 w98 files, `--cov` all 6 modules) → **228 passed / 0 failed**; **TOTAL 847 stmts / 0 missing / 100%**.
3. Partner-suite regression: `test_covpush_w95_zoom.py` + `test_covpush_w95_shopify.py` + `test_covpush_w8_webhooks.py` + `test_round69_async_unguarded_surfaces.py` → 255 passed / 2 failed, and the 2 failures are **pre-existing environmental** (`sqlite3.OperationalError: no such table: workflow_executions` in `TestWorkflowSchedulerCriticalNodes` — dev-DB drift in `advanced_workflow_orchestrator`, fails identically with no w98 files loaded; unrelated to this wave).
4. Webhook HMAC fail-closed (`SHOPIFY_WEBHOOK_SECRET` missing → 401) lives in `integrations/shopify_webhooks.py` / `api/routes/webhooks/shopify_webhooks.py` — already covered by waves 8/71c (`verify_shopify_webhook` fail-closed). The w98 `shopify_routes.py` `/webhooks/setup` SSRF guard is covered (`test_ssrf_blocked_400`); the module contains no webhook receiver.
...
**Bugs fixed (TDD RED→GREEN — failing test first, source then patched)**:
1. `integrations/quickbooks_service.py` + `integrations/zendesk_service.py` — **classes were uninstantiable**: neither implemented the `IntegrationService` ABC's `get_capabilities()`, so `QuickBooksService()` / `ZendeskService()` raised `TypeError: Can't instantiate abstract class ... with abstract method get_capabilities` — every live caller (`accounting/sync_manager.py`, `core/hybrid_data_ingestion.py`, `scripts/verify_all_integrations.py`) crashed at construction. Added `get_capabilities()` to both — RED: the entire w97 suite failed at `_svc()` before the fix.
2. `integrations/quickbooks_service.py:74` — `exchange_token` with missing `client_id`/`client_secret` built `auth=(None, None)` → uncaught httpx `TypeError` escaped as a 500. Now fail-closed `HTTPException(400, "Missing QuickBooks credentials")` (mirrors w95 zoom) — RED: `test_missing_credentials_fail_closed`.
3. `integrations/zoho_books_service.py:318` + `integrations/zoho_inventory_service.py:265` — module factory `get_zoho_books_service(config)` / `get_zoho_inventory_service(config)` referenced an **undefined global `tenant_id`** → `NameError` on every call (module-level singleton was created before the name ever existed). Now `ZohoBooksService(tenant_id="default", config=config)` — RED: `test_get_zoho_books_service` / `test_get_zoho_inventory_service`.
4. `integrations/zoho_books_service.py:71` + `integrations/zoho_inventory_service.py:70` — `_get_active_token` referenced `self.session_id`, which the base `IntegrationService` never sets → **`AttributeError` for any tenantless call**, making the access-token/env fallback branch unreachable dead code. Now `getattr(self, "session_id", None)` — RED: `test_tenantless_falls_back_to_access_token`.
5. `integrations/zoho_crm_service.py:252,262` + `integrations/mailchimp_service.py:197,207` — `sync_to_postgres_cache` used the **phantom `IntegrationMetric.tenant_id` column** (`filter_by(tenant_id=...)` + `IntegrationMetric(tenant_id=...)`) → `InvalidRequestError: Entity 'IntegrationMetric' has no property 'tenant_id'` on EVERY sync (same bug class as wave-95 Linear). Now uses the real `workspace_id` column — RED: `test_success_inserts_metrics` for both.
6. `integrations/zoho_crm_service.py:152,168,187,223` + `integrations/zoho_inventory_service.py:148,174` — intentional `HTTPException(401/400)` raised inside `try:` was **swallowed by the broad `except Exception`**: no-token `get_leads`/`get_deals` returned `[]` and `create_lead`/`check_stock`/`get_items` re-raised as **500** (or returned `{"error": ...}`) — 401/400 never surfaced. `except HTTPException: raise` added — RED: `test_no_token_401` / `test_no_org_400` across both modules.
7. `integrations/zoho_inventory_service.py:176` — `check_stock` error path returned **raw `str(e)`** to callers. Now generic `{"error": "Failed to check stock"}` — RED: `test_error_generic_no_leak`.
8. `str(e)` leaks → generic messages (detail kept server-side in logs, per project standard): `quickbooks_service.py` (health_check 232, execute_operation 282, sync inner 331 / outer 338), `zendesk_service.py` (health_check 270, execute_operation 328), `zoho_books_service.py` (execute_operation 67, sync inner 291 / outer 298), `zoho_crm_service.py` (execute_operation 70, sync inner 276 / outer 283), `zoho_inventory_service.py` (execute_operation 66, sync inner 238 / outer 245), `mailchimp_service.py` (health_check 57, execute_operation 109, sync inner 222 / outer 229) — RED: one `no_leak`/`generic` test per site.

**Coverage deltas** (measured with `--cov=<module> --cov-report=term-missing`, w97 file alone):

| Module | Before | After (w97 alone) | Stmts | Remaining uncovered |
|---|---|---|---|---|
| `integrations/quickbooks_service.py` | 0% | **100%** | 188 | — |
| `integrations/zendesk_service.py` | 0% | **100%** | 153 | — |
| `integrations/zoho_books_service.py` | 0% | **100%** | 193 | — |
| `integrations/zoho_crm_service.py` | 0% | **100%** | 199 | — |
| `integrations/zoho_inventory_service.py` | 0% | **100%** | 159 | — |
| `integrations/mailchimp_service.py` | 0% | **100%** | 127 | — |

**Evidence** (each `cd backend && PYTHONPATH=/Users/rushiparikh/projects/atom/backend venv/bin/python -m pytest -p no:cacheprovider -q`, ONE process at a time — RAM control):
1. RED proofs: quickbooks abstract-class `TypeError` (whole suite failed at `_svc()`), `test_missing_credentials_fail_closed` (uncaught TypeError → 500 before, clean 400 after), `test_get_zoho_books_service`/`test_get_zoho_inventory_service` (NameError before), `test_tenantless_falls_back_to_access_token` (AttributeError before), CRM/mailchimp `test_success_inserts_metrics` (InvalidRequestError on phantom `tenant_id` before), CRM/inventory `test_no_token_401` + `test_no_org_400` (swallowed 401/400 before `except HTTPException: raise`), inventory `test_error_generic_no_leak` (str(e) before), one leak-test per str(e) site.
2. New-wave combined run (single process, all 6 w97 files, `--cov` all 6 modules) → **240 passed / 0 failed**; **TOTAL 1019 stmts / 0 missing / 100%**. Batched runs (3 files each): 122 + 118 passed / 0 failed.
3. Partner-suite regression: `test_covpush_zendesk.py` (85 tests, covers the related `atom_zendesk_integration_service`) → **85 passed / 0 failed**. Other suites referencing these classes (`test_covpush_bigfour.py`, `test_covpush_universal.py`, `test_covpush_w21_workflow_engine.py`, `test_covpush_w29_workflow_engine_graph.py`, `test_covpush_crm_services.py`) all `patch`/`monkeypatch` the classes — verified no dependency on changed internals.
4. mypy A/B vs HEAD on the 6 edited modules: **217 → 214 errors** (all pre-existing legacy noise in these files + transitive imports; no new errors introduced; 3 resolved by the edits).
5. No live caller depended on the old leak strings or factory NameErrors — `rg` over non-test code shows callers only construct the services (`accounting/sync_manager.py`, `core/hybrid_data_ingestion.py`, `core/workflow_engine.py`, `sales/order_to_cash.py`, `scripts/verify_all_integrations.py`), all of which were previously broken by the abstract-class bug for QuickBooks/Zendesk.
...
**Bugs fixed (TDD RED→GREEN — failing test first, source then patched)**:
1. `core/models.py` — **missing `NotionToken` model**: `integrations/notion_routes.py` imports `NotionToken` from `core.models` where it does NOT exist → the module was **unimportable** (`ImportError` at collection; every lazy-registry load of the "notion" integration crashed). Model added (columns mirror every field the router uses: `user_id`, `workspace_id`, `status`, `access_token`, `refresh_token`, `notion_user_id`, `workspace_name`, `workspace_icon`, `token_type`, `owner_type`, `expires_at`, `last_used`) — RED: whole w96_notion_routes suite failed collection with ImportError before the fix.
2. `integrations/telegram_routes.py:52-59` — **webhook FAIL-OPEN**: with the secret env unset the handler logged a warning and ACCEPTED all requests — the opposite of its own comment ("Fails closed: rejects … if not configured at all") and of the R45/R69 convention (teams/gmail/whatsapp 401/503 unconfigured). Also read the non-canonical `TELEGRAM_WEBHOOK_SECRET_TOKEN`. Now reads `ATOM_TELEGRAM_WEBHOOK_SECRET` (canonical) with legacy fallback, and **fails closed 401 when unconfigured** — RED: `test_webhook_no_secret_fails_closed_401` (200 before fix, 401 after).
3. `integrations/telegram_routes.py` — **no auth on the router** (mounted live at `/api/v1/integrations/telegram` in `main_api_app.py`): `/send`, `/send-photo`, `/send-poll`, `/send-keyboard`, `/edit-keyboard`, `/answer-callback`, `/answer-inline`, `/send-chat-action`, `/get-chat-info/{id}`, `/workspaces/{user_id}` (cross-user data), `/status`, `/capabilities` all answered anonymous callers. `get_current_user` added to every endpoint except `/webhook` (secret-token-authed) and `/health` (public health convention) — RED: 12 parametrized `TestAuthGates::test_anonymous_401` (200 before, 401 after).
4. `integrations/telegram_routes.py` (message branch) — `asyncio.create_task(...)` used **without `import asyncio` in scope** (it was only imported inside the callback/inline branches) → **every message webhook crashed with NameError → 500 "Processing failed"** — RED: `test_message_success_bridges_and_audits` (500 before, 200 after).
5. `integrations/telegram_routes.py` — bridge call was fire-and-forget (`create_task` never awaited, no done-callback): bridge failures were **silently swallowed** ("Task exception was never retrieved") and the handler's own 500 + failure-audit branch was dead code. Now awaited (matches `whatsapp_routes.py:104` convention) so failures surface as 500 + failure audit — RED: `test_bridge_failure_500_audits_failure` (200 before, 500 after).
6. `integrations/obsidian_service.py:44-47` — the constructor's own **SSRF guard rejected the documented default `plugin_url="http://localhost:27123"`** (`ValueError: Hostname 'localhost' resolves to blocked IP`) → `ObsidianService()` (and the default route headers in `obsidian_routes.py`) was **unusable out of the box**; the Local REST API's loopback target is the intended destination, not an SSRF vector. `_is_loopback_url()` now exempts loopback literals/hostname; all other URLs still validated (private IPs/metadata IPs stay blocked) — RED: `test_default_loopback_constructor_ok` (ValueError before fix).

**Coverage deltas** (measured with `--cov=<module> --cov-report=term-missing`, w96 file alone):

| Module | Before | After (w96 alone) | Stmts | Remaining uncovered |
|---|---|---|---|---|
| `integrations/notion_routes.py` | 0% (unimportable) | **100%** | 162 | — |
| `integrations/box_service.py` | 0% | **98%** | 153 | lines 251-253 (`list_files` except — try body is pure static dict, provably unreachable) |
| `integrations/atom_communication_memory_production_api.py` | 0% | **100%** | 190 | — |
| `integrations/telegram_routes.py` | 0% | **100%** | 182 | — |
| `integrations/obsidian_service.py` | 0% | **100%** | 143 | — |

**Evidence** (each `cd backend && PYTHONPATH=/Users/rushiparikh/projects/atom/backend venv/bin/python -m pytest -p no:cacheprovider -q`, ONE process at a time — RAM control):
1. RED proofs: notion whole-suite collection `ImportError: cannot import name 'NotionToken' from 'core.models'`; telegram `test_webhook_no_secret_fails_closed_401` 200→401, 12 anonymous-401s 200→401, message-webhook NameError 500→200, bridge-failure 200→500; obsidian default-constructor `ValueError: Hostname 'localhost' resolves to blocked IP: ::1`.
2. New-wave combined run (single process, all 5 w96 files, `--cov` all 5 modules) → **190 passed / 0 failed**; **TOTAL 830 stmts / 3 missing / 99%** (run twice for stability). `main_api_app` import + lazy-registry notion/telegram router loads verified OK.
3. Partner-suite regressions (all green): notion partners `test_bughunt_notion_oauth.py` + `test_covpush_w94_notion.py` + `test_covpush_w33_notion_service.py` → 93 passed; telegram partners `test_covpush_w65d_telegram_discovery.py` + `test_covpush_adapters_telegram.py` → 189 passed; `test_covpush_intgr_c.py` → 275 passed; `test_bughunt_intgr_c.py` + `test_covpush_w92_comm_memory_api.py` → 112 passed; `test_covpush_w94_comm_webhooks.py` + box partners `test_box_integration.py` + `test_box_oauth_flow.py` → 55 passed. `test_round18_integration_security.py` → 10 passed / 6 failed — the 6 are the **pre-existing** `TestChannelRoutesRequireAuth` `ImportError: cannot import name 'channel_routes'` drift (module doesn't exist in the repo; documented in wave-94, untouched by this wave).
4. mypy: the only new-file errors are the 2 **pre-existing** `send_poll` bool|None arg-type errors in telegram_routes.py (lines 449-450, untouched by this wave; repo baseline 4279 errors / 373 files).
...
**Method**: full `npx jest --coverage --watchAll=false --maxWorkers=2` baseline (266s), then per-file targeted runs. New tests co-located in `components/<dir>/__tests__/` per convention (MSW for fetch, per-file `jest.mock` for use-toast/sonner/hooks). 84 new tests across 10 suites.

| Component | Before | After | New tests |
|---|---|---|---|
| `components/Onboarding/CognitiveTierWizard.tsx` | 0% | **100%** | 7 |
| `components/Onboarding/CostCalculator.tsx` | 0% | **100%** | 6 |
| `components/Automations/LogsSidebar.tsx` | 16.2% | **97.3%** | 11 |
| `components/supervision/OutputPreview.tsx` | 43.2% | **97.3%** | 11 |
| `components/finance/SubscriptionTracker.tsx` | 0% | **97.2%** | 6 |
| `components/finance/FinanceOverview.tsx` | 0% | **100%** | 8 |
| `components/dashboard/MetricsCard.tsx` | 0% | **100%** | 13 |
| `components/admin/jit-verification/LatencyDisplay.tsx` | 17.1% | **91.4%** | 10 |
| `components/admin/business-facts/CitationViewer.tsx` | 27.3% | **100%** | 7 |
| `components/canvas/MatchConfidenceReviewer.tsx` | 0% | **96.9%** | 11 |

**Component bug fixed (TDD red→green)**: `components/Onboarding/CostCalculator.tsx` — `requestsPerDay` state was an array `[100]` passed to the `Slider` (`components/ui/slider.tsx`, interface declares `value: number`; `onValueChange` emits `Number(e.target.value)`). Moving the slider set state to a scalar number → `requestsPerDay[0]` = `undefined` → label rendered "Requests Per Day: undefined" and the monthly estimate became NaN. Changed state to scalar `number` (line 15), `monthlyRequests = requestsPerDay * 30` (line 27), label `{requestsPerDay}` (line 56). Failing test: `components/Onboarding/__tests__/CostCalculator.test.tsx` "updates the estimate and label when the slider moves to 200 requests/day" (was red, now green).

**Notable repo conventions discovered**: (1) `tests/setup.ts:458` globally mocks `@/components/ui/use-toast` (passthrough ToastProvider — real toasts never render under test; per-file `jest.mock('@/components/ui/use-toast', () => ({ useToast: () => ({ toast: mockToast }) }))` is the pattern, assert on the mock). (2) fetch must go through the shared MSW server (`server.use(rest.get(...))` from `@/tests/mocks/server`) — `global.fetch = jest.fn()` breaks MSW interception (setup.ts:87 comment). (3) lucide-react 0.553 renamed icon classes: `lucide-check-circle-2`→`lucide-circle-check`, `lucide-x-circle`→`lucide-circle-x`, `lucide-loader-2`→`lucide-loader-circle`. (4) testing-library `getByText` matches only direct text-node children, so mixed `{<span>Label:</span> value}` markup needs per-segment assertions.

**Verification**: `npx jest components/Onboarding/__tests__/CognitiveTierWizard.test.tsx components/Onboarding/__tests__/CostCalculator.test.tsx components/Automations/__tests__/LogsSidebar.test.tsx components/supervision/__tests__/OutputPreview.test.tsx components/finance/__tests__/SubscriptionTracker.test.tsx components/finance/__tests__/FinanceOverview.test.tsx components/dashboard/__tests__/MetricsCard.test.tsx components/admin/jit-verification/__tests__/LatencyDisplay.test.tsx components/admin/business-facts/__tests__/CitationViewer.test.tsx components/canvas/__tests__/MatchConfidenceReviewer.test.tsx --coverage --collectCoverageFrom='components/Onboarding/CognitiveTierWizard.tsx' --collectCoverageFrom='components/Onboarding/CostCalculator.tsx' --collectCoverageFrom='components/Automations/LogsSidebar.tsx' --collectCoverageFrom='components/supervision/OutputPreview.tsx' --collectCoverageFrom='components/finance/SubscriptionTracker.tsx' --collectCoverageFrom='components/finance/FinanceOverview.tsx' --collectCoverageFrom='components/dashboard/MetricsCard.tsx' --collectCoverageFrom='components/admin/jit-verification/LatencyDisplay.tsx' --collectCoverageFrom='components/admin/business-facts/CitationViewer.tsx' --collectCoverageFrom='components/canvas/MatchConfidenceReviewer.tsx' --watchAll=false --maxWorkers=2` → **84 passed / 0 failed**, aggregate 97.98% stmts.
...
| File | Lines | Before stmts | After stmts | After lines | New tests |
|---|---|---|---|---|---|
| `hooks/chat/useChatInterface.ts` | 542 | 66.18% | **95.34%** | 95.63% | 20 |
| `lib/hubspotApi.ts` | 498 | 71.65% | **100%** | 100% | 30 |
| `hooks/useWhatsAppWebSocketEnhanced.ts` | 416 | 75.49% | **100%** | 100% | 17 |
| `lib/api-admin.ts` | 352 | 70.37% | **100%** | 100% | 21 |
| `hooks/test-helpers.ts` | 334 | 36.70% | **100%** | 100% | 25 |
| `lib/auth.ts` | 304 | 75.80% | **97.58%** | 97.24% | 9 |
| `lib/boards-api.ts` | 258 | 67.46% | **100%** | 100% | 30 |
| `hooks/useBoard.ts` | 119 | 77.46% | **97.18%** | 100% | 15 |
| `hooks/useLiveFinance.ts` | 71 | 70.96% | **100%** | 100% | 4 |
| `lib/logger.ts` | 67 | 62.96% | **100%** | 100% | 8 |

**Real bugs fixed (TDD red→green)**:
1. `lib/boards-api.ts:217-226` — `DecomposeNeedsKeyError extends Error` broke under tsconfig `target: es5`: the transpiled `super()` returns a raw `Error` instance whose prototype is `Error.prototype`, so `err instanceof DecomposeNeedsKeyError` was ALWAYS false — `components/boards/TaskDetailDrawer.tsx:242` (the "needs a BYOK key" UX branch) was dead code. Fixed with `Object.setPrototypeOf(this, DecomposeNeedsKeyError.prototype)`. Failing test: `boards-api.test.ts` "throws DecomposeNeedsKeyError on 424".
2. `hooks/chat/useChatInterface.ts:361-372` — `handleRegenerate` restore path was dead code: `handleSend` swallows all errors internally, so the `try/catch` never caught and a failed regenerate silently replaced the user's exchange with the error banner (lost conversation, no toast). `handleSend` now returns `Promise<boolean>` (false on any failure/early-return); `handleRegenerate` restores `originalMessages` + error toast on `false`. Failing test: "restores the original exchange when regeneration fails".

**Test-infra landmines found & fixed**: (1) `hooks/__tests__/useLiveFinance.test.ts` old tests used `jest.spyOn(global, 'setInterval')` under fake timers — spying the fake-timer setInterval corrupts timer state for EVERY later test in the file (new fetch tests never settled). Replaced with behavior-based assertions (`jest.getTimerCount()`, MSW-handler fetch counting). (2) `global.localStorage = mock` silently fails in jsdom (getter-only accessor) — `Object.defineProperty(window, 'localStorage', ...)` required (`api-admin.test.ts`); jsdom Storage methods are also un-spyable (Proxy). (3) `useWhatsAppWebSocketEnhanced.test.ts` mocked `WebSocket` lacked the `CONNECTING/OPEN/...` statics, so the hook's readyState guards compared against `undefined` — restored the constants so guards are exercised honestly. (4) `useChatInterface.test.ts` toast mock must be a module-scope stable `jest.fn()` (per-render `jest.fn()` changes identity, hiding calls after re-renders).
...
1. **Intent-as-features (feature request)**: predictor feature contract 10 → **16** (`FeatureExtractor.feature_names` in `routellm_trainer.py`, derived from `INTENT_CATEGORIES` so it can't drift). `_extract_request_features` now emits six `intent_*` one-hots (from `request.intent` or `conversation_context["intent"]`); `_task_default_features` emits zeros. Intent is a *feature*, not a key dimension — no schema change, no feedback-pipeline change, train/serve consistent via the stashed `prompt_features`. Value proven by test: a LR predictor trained on coding-satisfied / conversation-unsatisfied separates them (>0.2 probability gap) while an intent-blind predictor is indifferent (<0.05 gap).
2. **Live-path dead bug (NEW FINDING, same class as R97)**: `_rerank_with_learning` / `_stash_decision_features` build a synthetic request object without `conversation_context`, which `_extract_request_features` accessed directly → AttributeError → swallowed → live re-rank + feature stash silently no-op'd in production. Fixed with defensive `getattr` reads; intent threaded into the synthetic request. Verified with real (unmocked) feature extraction through the re-rank path.
3. **EMA cold-start gating (verification finding)**: `_rerank_with_learning` early-returned on `per_model is None` *before* evaluating the EMA term, so EMA could never route during full cold start (the documented "EMA carries while predictors are cold" handoff was dead on the live path). Gating now only skips when neither predictors nor EMA could contribute; EMA-only re-ranking covered by test.
4. **EMA correctness verified**: recurrence `S = αx + (1-α)S` correct with per-metric `n`; regime-shift test (10 successes → 5 failures decays 0.89 → ~0.29); `success` = `success AND quality_satisfied` pinned; `ATOM_EMA_ALPHA` clamp (0→0.2, 2.5→1.0, junk→0.2); cold-fleet `_ema_quality_term` returns None; stale "bias-corrected" comments in `_ema_quality_term`/`_ema_normalization_baselines`/`get_routing_statistics` removed (bias correction was already stripped from code). Old 10-dim persisted `.pkl` predictors keep serving via `n_features_in_` truncation in `predict_satisfaction` (no ValueError).
...
- `frontend-nextjs/tests/pages/approvals-page.test.tsx` NEW (6 tests, Red-following Green): pins the R82 Approvals page legs — HITL queue + Training Proposals (STUDENT→INTERN) render both with Bearer auth, approve/reject POST to `/api/maturity/training/proposals/{id}/approve|reject`, 403 surfaces the supervisor notice, empty states.
...
Automated sweep: extracted all 347 browser-level `/api/*` fetch/client calls from the frontend and diffed against the backend route table + Next proxy + frontend handlers. Genuine journey misses found + fixed:
- `pages/dashboard/forensics.tsx`: called `/api/business-health/forensics/{vendor-drift,pricing-opportunities,subscription-waste}` — the forensics SERVICE exists at `/api/forensics/*` (api/forensics_api.py, auth-gated); the three dashboard fetches were permanently 404. Repointed to the real endpoints (same `{data}` success shape). Tests updated (12/12 green).
- `pages/dashboard/owner.tsx` (Owner Cockpit): fetched `/api/business-health/dashboard` which no backend route serves → page always errored. Now composes the LIVE `/api/business-health/priorities` surface into the briefing panel; metrics grid renders only when present (guarded). Tests updated.
