...
const WakeWordProvider = ({ children }) => {
  ...
  const connectWebSocket = (0, react_1.useCallback)(() => {
    ...
    const timeout = setTimeout(() => {
      if (ws.readyState === WebSocket.CONNECTING) {
        ...
        setWakeWordError("WebSocket connection timed out after 10 seconds");
      }
    }, 10000);
    ws.onopen = () => {
    ...
    ws.onmessage = (event) => {
      ...
      if (
        ...
        setWakeWordError(
          message.error || "Unknown error from audio processor.",
        );
    ...
    ws.onerror = (error) => {
      ...
      setWakeWordError(
        "WebSocket connection error. Ensure the audio processor is running.",
      );
    ...
    ws.onclose = (event) => {
      ...
      if (isWakeWordEnabled && !event.wasClean) {
        // If enabled and not closed cleanly
        setWakeWordError(
      ...
      if (webSocketRef.current === ws) {
        // Avoid issues if a new WS was created quickly
  ...
  const startAudioProcessing = (0, react_1.useCallback)(async () => {
    if (!isWakeWordEnabled || isListening) {
      console.log(
        "Not starting audio processing: disabled or already listening.",
      );
      return;
    }
      ...
      processorNodeRef.current.onaudioprocess = (event) => {
        if (
          ...
          for (let i = 0; i < inputData.length; i++) {
            // Handle potential audio clipping and NaN values
            if (isNaN(inputData[i])) continue;
  ...
  const toggleWakeWord = (0, react_1.useCallback)(
    (forceEnable) => {
      const targetState =
        typeof forceEnable === "boolean" ? forceEnable : !isWakeWordEnabled;
      console.log(
        `Toggling wake word. Current: ${isWakeWordEnabled}, Target: ${targetState}`,
      );
    ...
    if (isWakeWordEnabled && !isListening && !wakeWordError) {
      ...
      const resumeAudio = async () => {
        ...
        if (ctx.state === "suspended") {
          console.log(
            "AudioContext is suspended, attempting to resume for wake word.",
          );
          try {
            await ctx.resume();
            ...
            setWakeWordError(
              "Audio system could not be started. Please interact with the page and try again.",
            );
          }
        } else {
          startAudioProcessing();
  ...
  return (0, jsx_runtime_1.jsx)(WakeWordContext.Provider, {
    value: {
      isWakeWordEnabled,
      isListening,
      wakeWordError,
      toggleWakeWord,
      startListening: startAudioProcessing,
      stopListening: stopAudioProcessing,
    },
    children: children,
  });
};
