...
const GmailAuthCallbackPage = () => {
  ...
  const [handleCallbackMutation, { loading: mutationInProgress }] = (0,
  client_1.useMutation)(HANDLE_GMAIL_AUTH_CALLBACK_MUTATION);
  const isMounted = (0, react_1.useRef)(true);
  const navigationTimeout = (0, react_1.useRef)(null);
  (0, react_1.useEffect)(() => {
    ...
    if (googleError) {
      const description =
        googleErrorDescription || googleError || "Unknown Google error.";
      ...
      if (isMounted.current) {
        navigationTimeout.current = setTimeout(() => {
          router.push("/Settings/UserViewCalendarAndContactIntegrations");
        }, 5000);
      }
      return;
    }
    if (typeof code === "string" && code) {
      ...
      handleCallbackMutation({ variables: { code } })
        .then((response) => {
          const { success, message } =
            response.data?.handleGmailAuthCallback || {};
          if (success) {
            ...
            throw new Error(
              message || "Failed to process Gmail authorization with backend.",
            );
          }
        })
        .catch((err) => {
          ...
          setStatusMessage(
            `Error: ${err.message || "Could not complete Gmail connection."}`,
          );
          toast({
            title: "Gmail Connection Failed",
            description:
              err.message ||
              "An unexpected error occurred while connecting your Gmail account.",
            status: "error",
            duration: 9000,
            isClosable: true,
          });
      ...
      toast({
        title: "Gmail Connection Error",
        description:
          "Invalid callback state. No authorization code provided by Google.",
        status: "error",
        duration: 9000,
        isClosable: true,
      });
  ...
  return (0, jsx_runtime_1.jsxs)(Box_1.default, {
    flex: 1,
    ...
    children: [
      (0, jsx_runtime_1.jsx)(Text_1.default, {
        variant: "header",
        mb: "l",
