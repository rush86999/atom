...
export default async function handler(
  ...
  try {
    ...
    const response = await fetch(`${backendUrl}/api/auth/jira/callback`, {
      method: 'POST',
      headers: {
    ...
    if (response.ok) {
      ...
      // Redirect to Jira integration page with success message
      res.redirect('/integrations/jira?success=true');
    } else {
