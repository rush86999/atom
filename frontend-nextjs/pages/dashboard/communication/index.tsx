...
export default function UnifiedCommunicationDashboard() {
  const [activeTab, setActiveTab] = useState<
    "overview" | "services" | "statistics" | "settings"
  >("services");
  const [servicesStatus, setServicesStatus] =
    useState<UnifiedServicesStatus | null>(null);
  ...
  const fetchDashboardData = async () => {
    try {
      ...
      const statusResponse = await fetch(
        `${apiConfig.baseUrl}${apiConfig.endpoints.implementations}`,
      );
      ...
      const statsResponse = await fetch(
        `${apiConfig.baseUrl}${apiConfig.endpoints.statistics}`,
      );
      if (statsResponse.ok) {
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-white border border-gray-200 rounded-lg p-6">
          ...
          <div className="space-y-3">
            {servicesStatus?.services &&
              Object.entries(servicesStatus.services).map(([name, config]) => (
