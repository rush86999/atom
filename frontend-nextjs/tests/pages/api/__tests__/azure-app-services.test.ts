...
import handler from "@/pages/api/integrations/azure/app-services";
...
describe("pages/api/integrations/azure/app-services", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it("proxies a plain GET list request to the backend with the user id header", async () => {
    mockFetch.mockResolvedValue(
      jsonResponse(true, 200, { value: [{ name: "app-1" }] }),
    );
  ...
  it("sends the request body for non-GET methods", async () => {
    mockFetch.mockResolvedValue(jsonResponse(true, 201, { name: "app-new" }));
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      "http://127.0.0.1:8000/api/azure/app-services",
      {
        ...
        body: JSON.stringify({ name: "app-new" }),
      },
    );
  });
  ...
  it("honours PYTHON_API_SERVICE_BASE_URL when configured", async () => {
    ...
    await invoke("GET");
    expect(mockFetch).toHaveBeenCalledWith(
      "http://python:6000/api/azure/app-services",
      expect.anything(),
    );
  });
  ...
  it("mirrors backend error status and payload", async () => {
    mockFetch.mockResolvedValue(
      jsonResponse(false, 404, { error: "app service not found" }),
    );
    const res = await invoke("GET", { id: "missing" });
    expect(res._getStatusCode()).toBe(404);
    expect(res._getJSONData()).toEqual({ error: "app service not found" });
  });
  ...
  it("returns 500 with the error message when fetch rejects", async () => {
    ...
    expect(res._getJSONData()).toEqual({
      error: "Failed to fetch Azure app services",
