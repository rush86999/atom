...
describe("pages/api/auth/microsoft/callback", () => {
  ...
  it("exchanges the code with the backend and redirects on success", async () => {
    ...
    expect(res._getRedirectUrl()).toBe(
      "/integrations?success=true&provider=microsoft",
    );

    expect(mockFetch).toHaveBeenCalledTimes(1);
    const [url, init] = mockFetch.mock.calls[0];
    expect(url).toBe("http://localhost:8001/api/auth/microsoft/callback");
  ...
  it("redirects with token_exchange_failed and logs when the backend rejects the code", async () => {
    mockFetch.mockResolvedValue(backendResponse(false, "invalid_grant", 400));
