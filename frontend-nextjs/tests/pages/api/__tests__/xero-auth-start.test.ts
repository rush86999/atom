...
import handler from "@/pages/api/integrations/xero/auth/start";

const mockSession = { user: { id: "user-1", email: "u@example.com" } };

describe("pages/api/integrations/xero/auth/start", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it("requests the authorization URL from the backend and redirects", async () => {
    mockFetch.mockResolvedValue({
      ok: true,
      json: async () => ({ authorization_url: "https://login.xero.com/identity/connect/authorize" }),
    });
    ...
    expect(mockFetch).toHaveBeenCalledWith("http://backend.test/api/auth/xero/start", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({
        user_id: "user-1",
        redirect_uri: "https://fe.atom.test/api/integrations/xero/auth/callback",
      }),
    });
  });
  ...
  it("uses the default backend URL when the env var is unset", async () => {
    ...
    mockFetch.mockResolvedValue({
      ok: true,
      json: async () => ({ authorization_url: "https://xero.example/auth" }),
    });
    const res = await invoke();
    expect(res._getStatusCode()).toBe(302);
