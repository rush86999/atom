  ...
  describe(describeName, () => {
    ...
    it("proxies a plain GET list request to the backend", async () => {
      mockFetch.mockResolvedValue(
        jsonResponse(true, 200, { results: [{ id: "item-1" }] }),
      );
      const res = await invoke("GET");
      expect(res._getStatusCode()).toBe(200);
      expect(res._getJSONData()).toEqual({ results: [{ id: "item-1" }] });
    ...
    it("appends the id path segment for a single id", async () => {
      ...
      expect(mockFetch.mock.calls[0][0]).toBe(
        `http://localhost:5058${backendPath}/item-1`,
      );
    });
...
runProxySuite(
  "pages/api/integrations/jira/issues",
  jiraIssuesHandler,
  "/api/jira/enhanced/issues",
  "Failed to fetch Jira issues",
);

runProxySuite(
  "pages/api/integrations/jira/projects",
  jiraProjectsHandler,
...
runProxySuite(
  "pages/api/integrations/linear/issues",
  linearIssuesHandler,
