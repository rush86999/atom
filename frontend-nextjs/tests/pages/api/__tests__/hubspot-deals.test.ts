...
import handler from "@/pages/api/hubspot/deals";
...
describe("pages/api/hubspot/deals", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it("proxies a plain GET list request to the backend", async () => {
    mockFetch.mockResolvedValue(
      jsonResponse(true, 200, { results: [{ id: "dl-1" }] }),
    );
  ...
  it("sends the request body for non-GET methods", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      "http://127.0.0.1:8000/api/hubspot/deals",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ dealname: "Big deal" }),
      },
    );
  });
  ...
  it("honours PYTHON_API_SERVICE_BASE_URL when configured", async () => {
    ...
    await invoke("GET");
    expect(mockFetch).toHaveBeenCalledWith(
      "http://python:6000/api/hubspot/deals",
      expect.anything(),
    );
  });
  ...
  it("mirrors backend error status and payload", async () => {
    mockFetch.mockResolvedValue(
      jsonResponse(false, 404, { error: "deal not found" }),
    );
    const res = await invoke("GET", { id: "missing" });
    expect(res._getStatusCode()).toBe(404);
    expect(res._getJSONData()).toEqual({ error: "deal not found" });
  });
