...
import handler from "@/pages/api/integrations/linear/auth/callback";

const mockSession = { user: { id: "user-1", email: "u@example.com" } };

describe("pages/api/integrations/linear/auth/callback", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it("exchanges the code with the backend and redirects on success", async () => {
    ...
    expect(res._getRedirectUrl()).toBe("/integrations/linear?success=true");
    expect(mockFetch).toHaveBeenCalledWith(
      "http://backend.test/api/integrations/linear/auth/callback",
      {
        method: "POST",
        ...
        body: JSON.stringify({
          ...
          state: "st-1",
          user_id: "user-1",
          redirect_uri: "https://fe.atom.test/api/integrations/linear/auth/callback",
        }),
      },
    );
  });
  ...
  it("returns 400 with the backend message when token exchange fails", async () => {
    mockFetch.mockResolvedValue({
      ok: false,
      json: async () => ({ message: "invalid state" }),
    });
    const res = await invoke({ code: "bad" });
    expect(res._getStatusCode()).toBe(400);
    expect(res._getJSONData()).toEqual({
      error: "Failed to complete Linear OAuth",
  ...
  it("returns 500 with the error message when fetch throws", async () => {
    mockFetch.mockRejectedValue(new Error("backend 504"));
    const res = await invoke({ code: "c" });
    expect(res._getStatusCode()).toBe(500);
    expect(res._getJSONData()).toEqual({
      error: "Failed to complete Linear OAuth flow",
      message: "backend 504",
    });
