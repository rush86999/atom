...
import handler from "@/pages/api/hubspot/campaigns";
...
describe("pages/api/hubspot/campaigns", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it("proxies a plain GET list request to the backend", async () => {
    mockFetch.mockResolvedValue(
      jsonResponse(true, 200, { results: [{ id: "cmp-1" }] }),
    );
  ...
  it("sends the request body for non-GET methods", async () => {
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      "http://127.0.0.1:8000/api/hubspot/campaigns",
      {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ name: "Spring campaign" }),
      },
    );
  });
  ...
  it("honours PYTHON_API_SERVICE_BASE_URL when configured", async () => {
    ...
    await invoke("GET");
    expect(mockFetch).toHaveBeenCalledWith(
      "http://python:6000/api/hubspot/campaigns",
      expect.anything(),
    );
  });
  ...
  it("mirrors backend error status and payload", async () => {
    mockFetch.mockResolvedValue(
      jsonResponse(false, 404, { error: "campaign not found" }),
    );
    const res = await invoke("GET", { id: "missing" });
    expect(res._getStatusCode()).toBe(404);
    expect(res._getJSONData()).toEqual({ error: "campaign not found" });
  });
  ...
  it("returns 500 with the error message when fetch rejects", async () => {
    ...
    expect(res._getJSONData()).toEqual({
      error: "Failed to fetch HubSpot campaigns",
