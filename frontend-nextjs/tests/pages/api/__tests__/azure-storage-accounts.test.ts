...
describe("pages/api/integrations/azure/storage-accounts", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it("proxies a plain GET list request to the backend with the user id header", async () => {
    mockFetch.mockResolvedValue(
      jsonResponse(true, 200, { value: [{ name: "stg-1" }] }),
    );
    const res = await invoke("GET");
    expect(res._getStatusCode()).toBe(200);
    expect(res._getJSONData()).toEqual({ value: [{ name: "stg-1" }] });
    expect(mockFetch).toHaveBeenCalledWith(
      "http://127.0.0.1:8000/api/azure/storage-accounts",
      {
        method: "GET",
  ...
  it("appends the id path segment for a single id", async () => {
    mockFetch.mockResolvedValue(jsonResponse(true, 200, { name: "stg-1" }));
    await invoke("GET", { id: "stg-1" });
    expect(mockFetch).toHaveBeenCalledWith(
      "http://127.0.0.1:8000/api/azure/storage-accounts/stg-1",
      expect.anything(),
    );
  });
  ...
  it("ignores array ids and forwards remaining query params", async () => {
    ...
    expect(url).toContain("/api/azure/storage-accounts");
  ...
  it("sends the request body for non-GET methods", async () => {
    mockFetch.mockResolvedValue(jsonResponse(true, 201, { name: "stg-new" }));
    const res = await invoke("POST", {}, { name: "stg-new" });
    ...
    expect(mockFetch).toHaveBeenCalledWith(
      "http://127.0.0.1:8000/api/azure/storage-accounts",
      {
        ...
        body: JSON.stringify({ name: "stg-new" }),
      },
    );
  });
  ...
  it("honours PYTHON_API_SERVICE_BASE_URL when configured", async () => {
    ...
    await invoke("GET");
    expect(mockFetch).toHaveBeenCalledWith(
      "http://python:6000/api/azure/storage-accounts",
      expect.anything(),
    );
  });
  ...
  it("mirrors backend error status and payload", async () => {
    mockFetch.mockResolvedValue(
      jsonResponse(false, 404, { error: "storage account not found" }),
    );
    const res = await invoke("GET", { id: "missing" });
    expect(res._getStatusCode()).toBe(404);
    expect(res._getJSONData()).toEqual({ error: "storage account not found" });
  });
  ...
  it("returns 500 with the error message when fetch rejects", async () => {
    ...
    expect(res._getJSONData()).toEqual({
      error: "Failed to fetch Azure storage accounts",
