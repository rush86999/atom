...
import handler from "@/pages/api/integrations/jira/auth/callback";

const mockSession = { user: { id: "user-1", email: "u@example.com" } };

describe("pages/api/integrations/jira/auth/callback", () => {
  beforeEach(() => {
    jest.clearAllMocks();
  ...
  it("exchanges the code with the backend and redirects on success", async () => {
    ...
    expect(res._getRedirectUrl()).toBe("/integrations/jira?success=true");
    expect(mockFetch).toHaveBeenCalledWith("http://backend.test/api/auth/jira/callback", {
      method: "POST",
  ...
  it("returns 400 with the backend message when token exchange fails", async () => {
    mockFetch.mockResolvedValue({
      ok: false,
      json: async () => ({ message: "expired code" }),
    });
  ...
  it("returns 500 with the error message when fetch throws", async () => {
    mockFetch.mockRejectedValue(new Error("tls handshake failed"));
    const res = await invoke({ code: "c" });
    expect(res._getStatusCode()).toBe(500);
    expect(res._getJSONData()).toEqual({
      error: "Failed to complete Jira OAuth flow",
      message: "tls handshake failed",
    });
    expect(console.error).toHaveBeenCalledWith(
      "Jira OAuth callback error:",
      expect.any(Error),
    );
  });
